...
DynamicObject::~DynamicObject()
{
	...
	while(jtr != jend)
	{
		target = *jtr;
		++jtr;
		target->RemoveAura(m_spellProto->m_id);
	}
...
void DynamicObject::Create(Unit * caster, Spell * pSpell, float x, float y, float z, uint32 duration, float radius)
{
	...
	m_uint32Values[OBJECT_FIELD_ENTRY] = m_spellProto->m_id;
	m_uint32Values[DYNAMICOBJECT_BYTES] = 0x01eeeeee;
	m_uint32Values[DYNAMICOBJECT_SPELLID] = m_spellProto->m_id;
...
//			++itr;
			...
			if(GetDistanceSq(target) <= radius)
			{
				...
				{
					if(m_spellProto->m_effect[i] == 27)
					{
						pAura->AddMod(m_spellProto->m_effectAura[i],
							m_spellProto->m_effectBasePoints[i]+1, m_spellProto->m_effectMiscValue[i], i);
					}
				}
