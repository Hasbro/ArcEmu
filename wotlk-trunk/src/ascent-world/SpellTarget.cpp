...
// 11 select id,name, m_implicitTargetA1 from spell where m_implicitTargetA1 = 11;
// +----+----------------------+------------------------+
// | id | name				 | m_implicitTargetA1 |
// +----+----------------------+------------------------+
...
// select id,name, m_implicitTargetA1 from spell where m_implicitTargetB1 = 9;
// +-------+----------------------------+------------------------+
// | id	| name					   | m_implicitTargetA1 |
// +-------+----------------------------+------------------------+
...
void Spell::FillTargetMap(uint32 i)
{
	...
	uint32 TypeA = m_spellInfo->m_implicitTargetA[i];
	uint32 TypeB = m_spellInfo->m_implicitTargetB[i];
...
{
	sLog.outDebug("[SPELL][TARGET] Unhandled target typeA: %u typeB: %u", m_spellInfo->m_implicitTargetA[j], m_spellInfo->m_implicitTargetB[j]);
...
void Spell::SpellTargetDefault(uint32 i, uint32 j)
{
	if(j==0 || (m_caster->IsPet() && j==1))
	{
		...
		else if( (m_spellInfo->m_effect[i] == SPELL_EFFECT_ADD_FARSIGHT
			|| m_spellInfo->m_effect[i] == SPELL_EFFECT_SUMMON_DEMON) )
...
void Spell::SpellTargetSelf(uint32 i, uint32 j)
{
	...
	//	{
	//		if(m_spellInfo->m_effect[i] != SPELL_EFFECT_LEARN_SPELL)//in talents
	//		{	
...
void Spell::SpellTargetSingleTargetEnemy(uint32 i, uint32 j)
{
	...
	TargetsList *tmpMap=&m_targetUnits[i];
	if(m_spellInfo->m_targetCreatureType && pTarget->GetTypeId()==TYPEID_UNIT)
	{		
		...
		if( cr->GetCreatureName() )
			if(!(1<<(cr->GetCreatureName()->Type-1) & m_spellInfo->m_targetCreatureType))
				return;
	}
	...
	if(p_caster && pTarget != p_caster)
	{
		...
		int x;
		for( x = 0; x < 3; ++x )
			if( m_spellInfo->m_effectAura[x] == SPELL_AURA_MOD_POSSESS ||
		...
		{	
			...
			if(	pTarget->m_auras[x] ) 
			{
				for(uint32 y = 0; y < 3; ++y)
					if ( pTarget->m_auras[x]->m_spellProto->m_effectAura[y] == SPELL_AURA_SPELL_MAGNET )
					{
	...
	if(m_spellInfo->m_effectChainTargets[i])
	{
		uint32 jumps=m_spellInfo->m_effectChainTargets[i]-1;
		float range=GetMaxRange(dbcSpellRange.LookupEntry(m_spellInfo->m_rangeIndex));//this is probably wrong
		range*=range;
...
{
	TargetsList *tmpMap=&m_targetUnits[i];
	if(m_spellInfo->m_effect[i] != SPELL_EFFECT_SUMMON_DEMON && m_spellInfo->m_effect[i] != SPELL_EFFECT_SUMMON_OBJECT_WILD)
...
void Spell::SpellTargetSingleTargetFriend(uint32 i, uint32 j)
{
	...
	float r= GetMaxRange(dbcSpellRange.LookupEntry(m_spellInfo->m_rangeIndex));
...
{
	TargetsList *tmpMap=&m_targetUnits[i];
	if(m_spellInfo->m_id == 12938)
	{
		//FIXME:this ll be immortal targets
		FillAllTargetsInArea(i,m_targets.m_destX,m_targets.m_destY,m_targets.m_destZ,GetRadius(i));
	}
	if(m_spellInfo->m_id == 30427)
	{
		uint32 cloudtype;
		Creature *creature;
...
void Spell::SpellTargetChainTargeting(uint32 i, uint32 j)
{
	...
	uint32 jumps=m_spellInfo->m_effectChainTargets[i];
	if(m_spellInfo->m_spellClassMask && u_caster)
	{
		SM_FIValue(u_caster->SM_FAdditionalTargets,(int32*)&jumps,m_spellInfo->m_spellClassMask);
	}
