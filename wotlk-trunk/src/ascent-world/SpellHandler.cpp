...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
	...
	recvPacket >> tmp1 >> slot >> tmp3 >> cn >> item_guid >> WOTLK;
	Item* tmpItem = NULL;
	...
	if(!spellInfo)
	{
		sLog.outError("SpellHandler.cpp(86): unknown spell id %i\n", spellId);
		return;
	}

	if (spellInfo->m_auraInterruptFlags & AURA_INTERRUPT_ON_STAND_UP)
	{
	...
	{
		_player->SendCastResult(spellInfo->m_id, SPELL_FAILED_NOT_READY, cn, 0);
		return;
	}

	if(_player->m_currentSpell)
	{
		_player->SendCastResult(spellInfo->m_id, SPELL_FAILED_SPELL_IN_PROGRESS, cn, 0);
		return;
	}
	...
	if( itemProto->ForcedPetId >= 0 )
	{
		if( itemProto->ForcedPetId == 0 )
		{
			...
			{
				_player->SendCastResult(spellInfo->m_id, SPELL_FAILED_BAD_TARGETS, cn, 0);
				return;
			}
		}
		else
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
	...
	recvPacket >> WOTLK >> spellId  >> cn; // I don't think order matters but just incase
	// check for spell id
	SpellEntry *spellInfo = dbcSpell.LookupEntryForced(spellId);

	if(!spellInfo || !sHookInterface.OnCastSpell(_player, spellInfo))
	{
		sLog.outError("SpellHandler.cpp(193): unknown spell id %i\n", spellId);
		return;
	}

	sLog.outDetail("WORLD: got cast spell packet, spellId - %i (%s), data length = %i, rangeindex = %i, allowed targets = %i",
		spellId, spellInfo->m_name_lang[0], recvPacket.size(), spellInfo->m_rangeIndex, spellInfo->m_targets);
	...
	if( !GetPlayer()->HasSpell(spellId) || spellInfo->m_attributes & ATTRIBUTES_PASSIVE )
	{
	...
	{
		//autoshot 75
		if((spellInfo->m_attributesExB & FLAGS3_ACTIVATE_AUTO_SHOT) /*spellInfo->Attributes == 327698*/)	// auto shot..
		{
			//sLog.outString( "HandleSpellCast: Auto Shot-type spell cast (id %u, name %s)" , spellInfo->m_id , spellInfo->Name );
		...
		if(name)
			...
			MSG_COLOR_SUBWHITE, name, MSG_COLOR_YELLOW, spellInfo->m_spellClassMask, spellInfo->m_spellClassSet);*/
