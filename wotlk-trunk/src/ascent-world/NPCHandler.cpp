...
void WorldSession::SendTrainerList(Creature* pCreature)
{
	...
	for(vector<TrainerSpell>::iterator itr = pTrainer->Spells.begin(); itr != pTrainer->Spells.end(); ++itr)
	{
		...
		if( pSpell->pCastRealSpell != NULL )
			data << pSpell->pCastRealSpell->m_id;
		else if( pSpell->pLearnSpell )
			data << pSpell->pLearnSpell->m_id;
		else
			continue;
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvPacket)
{
	...
	for(vector<TrainerSpell>::iterator itr = pTrainer->Spells.begin(); itr != pTrainer->Spells.end(); ++itr)
	{
		if( ( itr->pCastRealSpell && itr->pCastRealSpell->m_id == TeachingSpellID ) ||
			( itr->pLearnSpell && itr->pLearnSpell->m_id == TeachingSpellID ) )
		{
			pSpell = &(*itr);
		}
	}
	...
	if( pSpell->pLearnSpell )
	{
		...
		// add the spell
		_player->addSpell( pSpell->pLearnSpell->m_id );

		uint32 i;
		for( i = 0; i < 3; ++i)
		{
			if(pSpell->pLearnSpell->m_effect[i] == SPELL_EFFECT_PROFICIENCY || pSpell->pLearnSpell->m_effect[i] == SPELL_EFFECT_LEARN_SPELL ||
				pSpell->pLearnSpell->m_effect[i] == SPELL_EFFECT_WEAPON)
			{
		...
		for( i = 0; i < 3; ++i)
		{
			if( pSpell->pLearnSpell->m_effect[i] == SPELL_EFFECT_SKILL )
			{
				uint32 skill = pSpell->pLearnSpell->m_effectMiscValue[i];
				uint32 val = (pSpell->pLearnSpell->m_effectBasePoints[i]+1) * 75;
				if( val > 375 )
					val = 375;
	...
	if(pSpell->DeleteSpell)
	{
		// Remove old spell.
		if( pSpell->pLearnSpell )
			_player->removeSpell(pSpell->DeleteSpell, true, true, pSpell->pLearnSpell->m_id);
		else if(pSpell->pCastSpell)
			_player->removeSpell(pSpell->DeleteSpell, true, true, pSpell->pCastRealSpell->m_id);
		else
...
uint8 WorldSession::TrainerGetSpellStatus(TrainerSpell* pSpell)
{
	...
	if( pSpell->pCastRealSpell && (_player->HasSpell(pSpell->pCastRealSpell->m_id) || _player->HasDeletedSpell(pSpell->pCastRealSpell->m_id)) )
		return TRAINER_STATUS_ALREADY_HAVE;

	if( pSpell->pLearnSpell && (_player->HasSpell(pSpell->pLearnSpell->m_id) || _player->HasDeletedSpell(pSpell->pLearnSpell->m_id)) )
		return TRAINER_STATUS_ALREADY_HAVE;
