...
Player::Player( uint32 guid ) : m_mailBox(guid)
{
	...
	SDetector =  new SpeedDetector( SPDT_WINDOW_SIZE );
#endif
}
...
bool Player::Create(WorldPacket& data )
{
	...
	if(!info)
	{
		...
		sLog.outDetail("tried to create invalid player with race %u and class %u", race, class_);
	...
	memset(m_taximask, 0, sizeof(uint32)*10);
	...
	if(!myRace || !myClass)
	{
		...
		sLog.outDetail("tried to create invalid player with race %u and class %u, dbc info not found", race, class_);
		m_session->Disconnect();
		return false;
	}
	...
	SetUInt32Value(UNIT_FIELD_MAXPOWER7, 1000); // death knight	runic power
   ...
   // SetUInt32Value(PLAYER_BYTES_2, (facialHair | (0xEE << 8) | (0x01 << 16) | (0x02 << 24)));
	...
	else
	{
		...
		SetUInt32Value(PLAYER_CHARACTER_POINTS1, 45);
		SetUInt32Value(UNIT_FIELD_POWER7, 1000); // Actually converts to 100 :S
	...
	*/
	...
	if(levelup)
	{
        ...
        SendLevelupInfo(
			...
			lvlinfo->Stat[6] - oldlevel->Stat[6]);
...
void Player::_SavePet(QueryBuffer * buf)
{
	...
	if(m_Summon&&m_Summon->IsInWorld()&&m_Summon->GetPetOwner()==this)	// update PlayerPets array with current pet's info
	{
		...
		if(!m_Summon->Summon)	   // is a pet
		{
			...
			{
				if(buf == NULL)
					CharacterDatabase.Execute("INSERT INTO playerpetspells VALUES(%u, %u, %u, %u)", GetLowGUID(), pn, itr->first->m_id, itr->second);
				else
					buf->AddQuery("INSERT INTO playerpetspells VALUES(%u, %u, %u, %u)", GetLowGUID(), pn, itr->first->m_id, itr->second);
			}
		}
	}
...
{
	if (m_spellInfo->m_equippedItemSubclass != 0)
	{
		if(this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND))
		{
			if((int32)this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND)->GetProto()->Class == m_spellInfo->m_equippedItemClass)
			{
				if (m_spellInfo->m_equippedItemSubclass != 0)
				{
					if (m_spellInfo->m_equippedItemSubclass != 173555 && m_spellInfo->m_equippedItemSubclass != 96 && m_spellInfo->m_equippedItemSubclass != 262156)
					{
						if (pow(2.0,(this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND)->GetProto()->SubClass) != m_spellInfo->m_equippedItemSubclass))
							return false;
					}
				}
			}
		}
		else if(m_spellInfo->m_equippedItemSubclass == 173555)
			return false;

		if (this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_RANGED))
		{
			if((int32)this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_RANGED)->GetProto()->Class == m_spellInfo->m_equippedItemSubclass)
			{
				if (m_spellInfo->m_equippedItemSubclass != 0)
				{
					if (m_spellInfo->m_equippedItemSubclass != 173555 && m_spellInfo->m_equippedItemSubclass != 96 && m_spellInfo->m_equippedItemSubclass != 262156)
					{
						if (pow(2.0,(this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_RANGED)->GetProto()->SubClass) != m_spellInfo->m_equippedItemSubclass))							return false;
					}
				}
			}
		}
		else if
			(m_spellInfo->m_equippedItemSubclass == 262156)
			return false;
	}
	return true;
}
...
#define get_next_field fields[field_index++]
	...
	if(!info)
	{
		sLog.outError("Something baaad happened here");
		SoftDisconnect();
		return;
	}
	else
...
//#undef _language_fix_
	...
	while(true) 
	{
		...
		*end=0;
		spProto = dbcSpell.LookupEntryForced(atol(start));
		if(spProto)
			mDeletedSpells.insert(spProto->m_id);
		start = end +1;
	}
...
#ifdef EXPERIMENTAL_SPEED_DETECTOR
	//this is not the place to put this but for now it will do it. I can think of 1001 methods to speedhack this
	...
	if( apply )
	{
		...
		for( int k = 0; k < 5; k++ )
		{
			...
			if( item->GetProto()->Spells[k].Trigger == 1 )
			{
				...
				if( spells->m_shapeshiftMask )
				{
					AddShapeShiftSpell( spells->m_id );
					continue;
				}
	...
	else
	{
		...
		for( int k = 0; k < 5; k++ )
		{
			if( item->GetProto()->Spells[k].Trigger == 1 )
			{
				SpellEntry* spells = dbcSpell.LookupEntry( item->GetProto()->Spells[k].Id );
				if( spells->m_shapeshiftMask )
					RemoveShapeShiftSpell( spells->m_id );
				else
...
{	
	...
	}else if(guidtype == HIGHGUID_TYPE_GAMEOBJECT)
	{
		...
		if(!pGO)return;
		pGO->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE,0);
		pLoot=&pGO->loot;
...
int32 Player::CanShootRangedWeapon( uint32 spellid, Unit* target, bool autoshot )
{
	...
	uint8 fail = 0;
	uint32 rIndex = autoshot ? 114 : spellinfo->m_rangeIndex;
	...
	{
		SM_FFValue( this->SM_FRange, &maxr, spellinfo->m_spellClassMask );
		SM_PFValue( this->SM_PRange, &maxr, spellinfo->m_spellClassMask );
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		float spell_flat_modifers=0;
		float spell_pct_modifers=0;
		SM_FFValue(this->SM_FRange,&spell_flat_modifers,spellinfo->m_spellClassMask);
		SM_FFValue(this->SM_PRange,&spell_pct_modifers,spellinfo->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!spell range bonus mod flat %f , spell range bonus pct %f , spell range %f, spell group %u\n",spell_flat_modifers,spell_pct_modifers,maxr,spellinfo->m_spellClassMask);
#endif
	}
	...
	//if(autoshot)
	//SM_FFValue( SM_FRange, &bonusRange, dbcSpell.LookupEntry( 75 )->m_spellClassMask ); // HORRIBLE hackfixes :P
...
void Player::EventRepeatSpell()
{
	...
	int32 f = this->CanShootRangedWeapon( m_AutoShotSpell->m_id, target, true );
...
void Player::removeSpellByHashName(uint32 hash)
{
	...
	for(iter= mSpells.begin();iter != mSpells.end();)
	{
		...
		if(e->NameHash == hash)
		{
			if(info->spell_list.find(e->m_id) != info->spell_list.end())
				continue;
	...
	for (unsigned int i = 0; i < numRows; i++)		  // Loop through all talents.
	{
		...
		for (int j = 0; j < 5; j++)
		{
			if (tmpTalent->RankID[j] != 0)
			{
				...
				{
					for(int k=0;k<3;k++)
						if(spellInfo->m_effect[k] == SPELL_EFFECT_LEARN_SPELL)
						{
							//removeSpell(spellInfo->m_effectTriggerSpell[k], false, 0, 0);
							...
							spellInfo2 = dbcSpell.LookupEntry( spellInfo->m_effectTriggerSpell[k] );
							if(spellInfo2)
...
void Player::UpdateNearbyGameObjects()
{
	for (Object::InRangeSet::iterator itr = GetInRangeSetBegin(); itr != GetInRangeSetEnd(); ++itr)
	{
		if((*itr)->GetTypeId() == TYPEID_GAMEOBJECT)
		{
			...
			if((*itr)->GetByteValue(GAMEOBJECT_BYTES_1, 1) == GAMEOBJECT_TYPE_QUESTGIVER)
			{
...
void Player::ClearCooldownForSpell(uint32 spell_id)
{
	...
	for(i = 0; i < NUM_COOLDOWN_TYPES; ++i)
	{
		...
		{
			itr2 = itr++;
			if( ( i == COOLDOWN_TYPE_CATEGORY && itr2->first == spe->m_category ) ||
				( i == COOLDOWN_TYPE_SPELL && itr2->first == spe->m_id ) )
			{
...
void Player::CalculateBaseStats()
{
	...
	SetUInt32Value(UNIT_FIELD_MAXPOWER7, 1000); // death knight
}
...
{	
	...
	for (itr = mSpells.begin(); itr != mSpells.end(); ++itr)
	{
		...
		if(	info  
			&& (info->m_attributes & ATTRIBUTES_PASSIVE)  // passive
			&& !( info->c_is_flags & SPELL_FLAG_IS_EXPIREING_WITH_PET ) //on pet summon talents
			 ) 
		{
			if( info->m_shapeshiftMask )
			{
				if( !( ((uint32)1 << (GetShapeShift()-1)) & info->m_shapeshiftMask ) )
					continue;
			}
...
*/
		...
		Aura * a;
		if(sp->m_id == 8326 || sp->m_id == 9036 || sp->m_id == 20584 || sp->m_id == 15007)	// death auras
		{
			if(!isDead())
				continue;
		...
		for(uint32 x =0;x<3;x++)
        {
		    if(sp->m_effect[x]==SPELL_EFFECT_APPLY_AURA)
		    {
			    a->AddMod(sp->m_effectAura[x],sp->m_effectBasePoints[x]+1,sp->m_effectMiscValue[x],x);
		    }
        }
...
//			this->AddAuraVisual((*i).id,count_appearence-1,a->IsPositive());
	...
	if( getClass() == DKNIGHT )
	{
		// Death Knights have Blood Presence active
		if(m_FirstLogin)  // Only on the first login
			CastSpell(this, dbcSpell.LookupEntry(48266), true); // Blood Presence...

		if(!HasActiveAura(48265) || !HasActiveAura(48263) && HasActiveAura(48266)) // Default to blood, if no presence
			RemoveAura(48266);
		else
			CastSpell(this, dbcSpell.LookupEntry(48266), true);
	}
...
//	for(uint32 x=0;x<MAX_AURAS+MAX_PASSIVE_AURAS;x++)
	for(uint32 x=0;x<MAX_AURAS;x++)
	{
		if(m_auras[x])
		{
			...
			{
				if(aur->m_spellProto->m_effect[i] == SPELL_EFFECT_APPLY_AREA_AURA ||
					aur->m_spellProto->m_effect[i] == SPELL_EFFECT_ADD_FARSIGHT)
				{
					skip = true;
					break;
				}
...
/*			why these should't be save??
			...
			if(aur->GetSpellProto()->m_procTypeMask)
			{
//				sLog.outDebug("skipping aura %d because has flags %d",aur->GetSpellId(),aur->GetSpellProto()->procFlags);
			...
			if(aur->GetSpellProto()->m_procCharges)
			{
//				sLog.outDebug("skipping aura %d because has proccharges %d",aur->GetSpellId(),aur->GetSpellProto()->procCharges);
            ...
            if(aur->IsPassive() && !(aur->GetSpellProto()->m_attributesEx & 1024))
				skip = true;
			//shapeshift
//			if(m_ShapeShifted==aur->m_spellProto->m_id)
//				skip=true;
...
void Player::SetShapeShift(uint8 ss)
{
	...
	for( uint32 x = 0; x < MAX_AURAS + MAX_PASSIVE_AURAS; x++ )
	{
		if( m_auras[x] != NULL )
		{
			uint32 reqss = m_auras[x]->GetSpellProto()->m_shapeshiftMask;
			...
			if( this->getClass() == DRUID )
			{
				...
				{
					switch( m_auras[x]->GetSpellProto()->m_effectAura[y])
					{
	...
	for( itr = mSpells.begin(); itr != mSpells.end(); ++itr )
	{
		...
		if( sp->apply_on_shapeshift_change || sp->m_attributes & 64 )		// passive/talent
		{
			if( sp->m_shapeshiftMask && ((uint32)1 << (ss-1)) & sp->m_shapeshiftMask )
			{
...
void Player::Possess(Unit * pTarget)
{
	...
	for(list<AI_Spell*>::iterator itr = pTarget->GetAIInterface()->m_spells.begin(); itr != pTarget->GetAIInterface()->m_spells.end(); ++itr)
	{
		if((*itr)->agent == AGENT_SPELL)
			avail_spells.push_back((*itr)->spell->m_id);
	}
...
void Player::EventStunOrImmobilize(Unit *proc_target, bool is_victim)
{
	...
	if( t_trigger_on_stun )
	{
		...
		SM_FIValue(SM_FChanceOfSuccess,&t_trigger_on_stun_chance,spellInfo->m_spellClassMask);
		...
		if ( spellInfo->m_procTypeMask & PROC_TARGET_SELF )
...
void Player::_AddLanguages(bool All)
{
	...
	case RACE_HUMAN:
		{
			_AddSkillLine(98, 300, 300); // Common
			addSpell(668);
		}break;
	case RACE_ORC:
		{
			_AddSkillLine(109, 300, 300); // Orcish
			addSpell(669);
		}break;
	case RACE_DWARF:
		{
			_AddSkillLine(98, 300, 300);
			addSpell(668);
			
			_AddSkillLine(111, 300, 300); // Dwarven
			addSpell(672);
		}break;
	case RACE_NIGHTELF:
		{
			...
			_AddSkillLine(113, 300, 300); // Darnassian
			addSpell(671);
		}break;
	case RACE_UNDEAD:
		{
			_AddSkillLine(109, 300, 300);
			addSpell(669);

			_AddSkillLine(673, 300, 300); // Gutterspeak
			addSpell(17737);
		}break;
	case RACE_TAUREN:
		{
			...
			_AddSkillLine(115, 300, 300); // Taurahe
			addSpell(670);
		}break;
	case RACE_GNOME:
		{
			...
			_AddSkillLine(313, 300, 300); // Gnomish
			addSpell(7340);
		}break;
	case RACE_TROLL:
		{
			...
			_AddSkillLine(315, 300, 300); // Troll
			addSpell(7341);
		}break;
	case RACE_BLOODELF:
		{
			...
			_AddSkillLine(137, 300, 300); // Thalassian
			addSpell(813);
		}break;
	case RACE_DRAENEI:
		{
			...
			_AddSkillLine(759, 300, 300); // Draenei
			addSpell(29932);
		}break;
	}
...
void Player::AddShapeShiftSpell(uint32 id)
{
	...
	if( sp->m_shapeshiftMask && ((uint32)1 << (GetShapeShift()-1)) & sp->m_shapeshiftMask )
	{
...
void Player::Cooldown_Add(SpellEntry * pSpell, Item * pItemCaster)
{
	...
	if( pSpell->m_categoryRecoveryTime > 0 && pSpell->m_category )
	{
		cool_time = pSpell->m_categoryRecoveryTime;
		if( pSpell->m_spellClassMask )
		{
			SM_FIValue(SM_FCooldownTime, &cool_time, pSpell->m_spellClassMask);
			SM_PIValue(SM_PCooldownTime, &cool_time, pSpell->m_spellClassMask);
		}

		_Cooldown_Add( COOLDOWN_TYPE_CATEGORY, pSpell->m_category, mstime + cool_time, pSpell->m_id, pItemCaster ? pItemCaster->GetProto()->ItemId : 0 );
	}
	
	if( pSpell->m_recoveryTime > 0 )
	{
		cool_time = pSpell->m_recoveryTime;
		if( pSpell->m_spellClassMask )
		{
			SM_FIValue(SM_FCooldownTime, &cool_time, pSpell->m_spellClassMask);
			SM_PIValue(SM_PCooldownTime, &cool_time, pSpell->m_spellClassMask);
		...
		_Cooldown_Add( COOLDOWN_TYPE_SPELL, pSpell->m_id, mstime + cool_time, pSpell->m_id, pItemCaster ? pItemCaster->GetProto()->ItemId : 0 );
	}
}

void Player::Cooldown_AddStart(SpellEntry * pSpell)
{
	if( pSpell->m_startRecoveryTime == 0 )
		return;

	uint32 mstime = getMSTime();
	int32 atime = float2int32( float(pSpell->m_startRecoveryTime) * m_floatValues[UNIT_MOD_CAST_SPEED] );
	if( atime <= 0 )
		return;
	if( atime >= 1 )
		atime = 1; // global cooldown is decreased by spell haste, but it's not INCREASED by spell slow.

	if( pSpell->m_startRecoveryTime )		// if we have a different cool category to the actual spell category - only used by few spells
		_Cooldown_Add( COOLDOWN_TYPE_CATEGORY, pSpell->m_categoryRecoveryTime, mstime + atime, pSpell->m_id, 0 );
...
bool Player::Cooldown_CanCast(SpellEntry * pSpell)
{
	...
	if( pSpell->m_category )
	{
		itr = m_cooldownMap[COOLDOWN_TYPE_CATEGORY].find( pSpell->m_category );
	...
	itr = m_cooldownMap[COOLDOWN_TYPE_SPELL].find( pSpell->m_id );
	...
	if( pSpell->m_startRecoveryTime && m_globalCooldown )			/* gcd doesn't affect spells without a cooldown it seems */
	{
...
{
	if(sWorld.antihack_speed == false)
		return;
	
	if(sWorld.antihack_speed == true && !GetTaxiState() && m_isMoving)
	{
