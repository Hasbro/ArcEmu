...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
	...
	//update the detector
	_player->SDetector->AddSample( movement_info.x, movement_info.y, getMSTime() );
	//this is nasty here, will move it in case detector is good :P
	if( !_player->GetTaxiState() )
	{
		...
		if( _player->SDetector->GetSpeed() > speed )
		{
			printf("We might have a speedhack here our speed %f their speed %f \n",speed,_player->SDetector->GetSpeed());
		}
	}
#endif
