...
public:
    ...
    ASCENT_INLINE uint32 GetSpellId() const {return m_spellProto->m_id; }
    ASCENT_INLINE bool IsPassive(){ if(!m_spellProto) return false; return (m_spellProto->m_attributes & ATTRIBUTES_PASSIVE && !m_areaAura);}
	...
	void ModAmplitude( float factor );
...
//	void SpellAuraIncreaseHealingByInt(bool apply);
	...
	ASCENT_INLINE bool IsCombatStateAffecting()
	{
		for(uint32 x = 0; x < 3; ++x) {
			if(m_spellProto->m_effectAura[x] == SPELL_AURA_PERIODIC_DAMAGE || 
				m_spellProto->m_effectAura[x] == SPELL_AURA_PERIODIC_DAMAGE_PERCENT ||
				m_spellProto->m_effectAura[x] == SPELL_AURA_PERIODIC_TRIGGER_SPELL ||
				m_spellProto->m_effectAura[x] == SPELL_AURA_PERIODIC_LEECH ||
				m_spellProto->m_effectAura[x] == SPELL_AURA_PERIODIC_MANA_LEECH)
			{
				return true;
			}
		}
