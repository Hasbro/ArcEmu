...
void Pet::CreateAsSummon(uint32 entry, CreatureInfo *ci, Creature* created_from_creature, Unit *owner, SpellEntry* created_by_spell, uint32 type, uint32 expiretime)
{
	...
	if(type & 0x1 && created_by_spell != NULL)
		SetUInt64Value(UNIT_CREATED_BY_SPELL, created_by_spell->m_id);
	...
	if(m_uint32Values[OBJECT_FIELD_ENTRY] != WATER_ELEMENTAL)	
	{
		...
		*data << uint8(mSpells.size());
		for(PetSpellMap::iterator itr = mSpells.begin(); itr != mSpells.end(); ++itr)
			*data << uint16(itr->first->m_id) << uint16(itr->second);
	}
	*data << uint64(0);
...
void Pet::InitializeSpells()
{
	for(PetSpellMap::iterator itr = mSpells.begin(); itr != mSpells.end(); ++itr)
	{
		...
		if( info->m_attributes & ATTRIBUTES_PASSIVE )
		{
			// Cast on self..
...
{
	// Create an AI_Spell
	map<uint32,AI_Spell*>::iterator itr = m_AISpellStore.find(info->m_id);
	...
	sp->entryId = GetEntry();
	sp->floatMisc1 = 0;
	sp->maxrange = GetMaxRange(dbcSpellRange.LookupEntry(info->m_rangeIndex));
	sp->minrange = GetMinRange(dbcSpellRange.LookupEntry(info->m_rangeIndex));
	sp->Misc2 = 0;
	sp->procChance = 0;
	sp->spell = info;
	sp->spellType = STYPE_DAMAGE;
	sp->spelltargetType = TTYPE_SINGLETARGET;
	sp->cooldown = objmgr.GetPetSpellCooldown(info->m_id);
	sp->cooldowntime = 0;
	if(info->m_effect[0] == SPELL_EFFECT_APPLY_AURA || info->m_effect[0] == SPELL_EFFECT_APPLY_AREA_AURA)
		sp->spellType = STYPE_BUFF;

	if(info->m_implicitTargetA[0] == 24)
	{
		float radius = ::GetRadius(dbcSpellRadius.LookupEntry(info->m_effectRadiusIndex[0]));
		sp->maxrange = radius;
...
void Pet::AddSpell(SpellEntry * sp, bool learning)
{
	...
	if( sp->m_attributes & ATTRIBUTES_PASSIVE )
	{
		if(IsInWorld())
		{
...
void Pet::SetSpellState(SpellEntry* sp, uint16 State)
{
	...
	{
		AI_Spell * sp2 = GetAISpellForSpellId(sp->m_id);
		if(sp2)
		{
...
void Pet::SetDefaultActionbar()
{
	...
	if(mSpells.size() > 0)
	{
		...
		for(; itr != mSpells.end() && pos < 4; ++itr, ++pos)
			ActionBar[3+pos] = itr->first->m_id;
	}
...
{
	mSpells.erase(sp);
	map<uint32, AI_Spell*>::iterator itr = m_AISpellStore.find(sp->m_id);
	if(itr != m_AISpellStore.end())
	{
...
uint16 Pet::GetUsedTP()
{
	...
	if(mSpells.size() > 0)
	{
		...
		for(; itr != mSpells.end(); ++itr)
		{
			...
			if(itr->first->m_id == GetHighestRankSpell(itr->first->m_id))
				sumTP += SpellTP(itr->first->m_id);
		}
	}
	return sumTP;
}
uint32 Pet::CanLearnSpell( SpellEntry * sp )
{
	...
	if( mSpells.size() > 0 && !( sp->m_attributes & ATTRIBUTES_PASSIVE ) )
	{
		uint8 cnt = 0;
		PetSpellMap::iterator itr = mSpells.begin();
		for( ; itr != mSpells.end(); ++itr )
		{
			...
			if( !( itr->first->m_attributes & ATTRIBUTES_PASSIVE ) )
				cnt++;
		}
		if( cnt > 3 )
	...
	if( SpellTP( sp->m_id ) - SpellTP( GetHighestRankSpell( sp->m_id ) ) >  TP )
