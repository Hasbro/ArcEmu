...
//    SMT_SPEED             =12,// movement speed, while given spell is active(flat is %) // TODO CHECK! ok this is not speed and is used with 23 misc to sometimes so is odd
    ...
    SMT_GLOBAL_COOLDOWN        =21,// implemented in WotLK
...
enum SPELL_DMG_TYPE // SPELL_ENTRY_m_defenseType
{
...
enum SpellCastFlags
{
	...
	CAST_FLAG_UNKNOWN4           = 0x20000,              // wotlk
    CAST_FLAG_UNKNOWN5           = 0x80000               // wotlk
};
...
enum SPELL_ENTRY
{
    ...
    SPELL_ENTRY_m_implicitTargetA_1,
    SPELL_ENTRY_m_implicitTargetA_2,
    SPELL_ENTRY_m_implicitTargetA_3,
    SPELL_ENTRY_m_implicitTargetB_1,
    SPELL_ENTRY_m_implicitTargetB_2,
    SPELL_ENTRY_m_implicitTargetB_3,
    ...
    SPELL_ENTRY_m_spellClassSet,
    SPELL_ENTRY_m_spellClassMask,
    SPELL_ENTRY_unkne,
    SPELL_ENTRY_MaxTargets,
    SPELL_ENTRY_m_defenseType,
    SPELL_ENTRY_FG,
    SPELL_ENTRY_FH,
...
{
    SPELL_TYPE_NONE                 = 0x00000000, // 0
    SPELL_TYPE_SEAL                 = 0x00000001, // 1
    SPELL_TYPE_ASPECT               = 0x00000002, // 2
    SPELL_TYPE_BLESSING             = 0x00000004, // 3
    SPELL_TYPE_CURSE                = 0x00000008, // 4
    SPELL_TYPE_STING                = 0x00000010, // 5
    SPELL_TYPE_ARMOR                = 0x00000020, // 6
    SPELL_TYPE_AURA                 = 0x00000040, // 7
    //hmm these could be named simply incompatible spells. One active at a time
    SPELL_TYPE_MARK_GIFT            = 0x00000080, // 8
    SPELL_TYPE_TRACK                = 0x00000100, // 9
    SPELL_TYPE_HUNTER_TRAP          = 0x00000200, // 10
    SPELL_TYPE_MAGE_INTEL           = 0x00000400, // 11
    SPELL_TYPE_MAGE_MAGI            = 0x00000800, // 12
    SPELL_TYPE_MAGE_WARDS           = 0x00001000, // 13
    SPELL_TYPE_PRIEST_SH_PPROT      = 0x00002000, // 14
    SPELL_TYPE_SHIELD               = 0x00004000, // 15
    SPELL_TYPE_FORTITUDE            = 0x00008000, // 16
    SPELL_TYPE_SPIRIT               = 0x00010000, // 17
    SPELL_TYPE_MAGE_AMPL_DUMP       = 0x00020000, //18
	...
	SPELL_TYPE_DK_PRESENCE			= 0x00800000, // wotlk Death Knight presence
};
...
ASCENT_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
    ...
    if( sp->m_effect[0]==SPELL_EFFECT_APPLY_AURA ||
       sp->m_effect[0]==SPELL_EFFECT_APPLY_AREA_AURA)
    {
        switch (sp->m_effectAura[0])
        {
    ...
    if( sp->m_effectAura[1]==SPELL_EFFECT_APPLY_AURA ||
        sp->m_effect[1]==SPELL_EFFECT_APPLY_AREA_AURA)
    {
        switch (sp->m_effectAura[1])
        {
            case 3://SPELL_AURA_PERIODIC_DAMAGE:
            case 43://SPELL_AURA_PROC_TRIGGER_DAMAGE:
            case 89://SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
    ...
    if( sp->m_effect[2]==SPELL_EFFECT_APPLY_AURA ||
        sp->m_effect[2]==SPELL_EFFECT_APPLY_AREA_AURA)
    {
        switch (sp->m_effectAura[2])
        {
            case 3://SPELL_AURA_PERIODIC_DAMAGE:
            case 43://SPELL_AURA_PROC_TRIGGER_DAMAGE:
            case 89://SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
...
ASCENT_INLINE bool IsHealingSpell(SpellEntry *sp)
{
    ...
    if( sp->m_effect[1] == SPELL_EFFECT_APPLY_AURA ||
        sp->m_effect[1] == SPELL_EFFECT_APPLY_AREA_AURA )
    {
        switch (sp->m_effectAura[1])
        {
...
{
	if(
		sp->m_implicitTargetA[0]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetA[1]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetA[2]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetB[0]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetB[1]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetB[2]==EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS ||
		sp->m_implicitTargetA[0]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetA[1]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetA[2]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetB[0]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetB[1]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetB[2]==EFF_TARGET_ALL_ENEMIES_AROUND_CASTER ||
		sp->m_implicitTargetA[0]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetA[1]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetA[2]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetB[0]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetB[1]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetB[2]==EFF_TARGET_ALL_ENEMY_IN_AREA_CHANNELED ||
		sp->m_implicitTargetA[0]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
		sp->m_implicitTargetA[1]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
		sp->m_implicitTargetA[2]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
		sp->m_implicitTargetB[0]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
		sp->m_implicitTargetB[1]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
		sp->m_implicitTargetB[2]==EFF_TARGET_ALL_ENEMY_IN_AREA_INSTANT 
		)
		return 1;
...
#define POWER_TYPE_RUNES 5 // Depletes specific runes
#define POWER_TYPE_RUNIC_POWER 6 // *10
...
public:
	...
	// This returns SPELL_ENTRY_m_defenseType where 0 = SPELL_DMG_TYPE_NONE, 1 = SPELL_DMG_TYPE_MAGIC, 2 = SPELL_DMG_TYPE_MELEE, 3 = SPELL_DMG_TYPE_RANGED
	// It should NOT be used for weapon_damage_type which needs: 0 = MELEE, 1 = OFFHAND, 2 = RANGED
	ASCENT_INLINE uint32 GetType() { return ( m_spellInfo->m_defenseType == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->m_defenseType ); }
    ...
    uint32 GetDuration()
    {
        ...
        if(m_spellInfo->m_durationIndex)
        {
            SpellDuration *sd=dbcSpellDuration.LookupEntry(m_spellInfo->m_durationIndex);
            if(sd)
            {
                ...
                {
                    SM_FIValue(u_caster->SM_FDur,(int32*)&this->Dur,m_spellInfo->m_spellClassMask);
                    SM_PIValue(u_caster->SM_PDur,(int32*)&this->Dur,m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
					int spell_flat_modifers=0;
					int spell_pct_modifers=0;
					SM_FIValue(u_caster->SM_FDur,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
					SM_FIValue(u_caster->SM_PDur,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
					if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
						printf("!!!!!spell duration mod flat %d , spell duration mod pct %d , spell duration %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,Dur,m_spellInfo->m_spellClassMask);
#endif
                }
				// DEATH KNIGHT: Runic Power (duration modifier...)
				// RUNIC POWER:
				// The duration of any ability that costs Runic Power is affected by the Death Knight's current Runic Power
				// at the time of cast. 100 RP means the spell has the full duration, whereas 0 RP (technically uncastable) means the spell has no duration.
				...
				{
					this->Dur = float2int32( (float)u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) / 1000.0f * (float)this->Dur );
    ...
    ASCENT_INLINE float GetRadius(uint32 i)
    {
        ...
        bRadSet[i]=true;
        Rad[i]=::GetRadius(dbcSpellRadius.LookupEntry(m_spellInfo->m_effectRadiusIndex[i]));
        if(m_spellInfo->m_spellClassMask && u_caster)
        {
            SM_FFValue(u_caster->SM_FRadius,&Rad[i],m_spellInfo->m_spellClassMask);
            SM_PFValue(u_caster->SM_PRadius,&Rad[i],m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
			float spell_flat_modifers=0;
			float spell_pct_modifers=1;
			SM_FFValue(u_caster->SM_FRadius,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
			SM_PFValue(u_caster->SM_PRadius,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
			if(spell_flat_modifers!=0 || spell_pct_modifers!=1)
				printf("!!!!!spell radius mod flat %f , spell radius mod pct %f , spell radius %f, spell group %u\n",spell_flat_modifers,spell_pct_modifers,Rad[i],m_spellInfo->m_spellClassMask);
#endif
        }
