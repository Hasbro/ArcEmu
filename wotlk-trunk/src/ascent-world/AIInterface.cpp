...
//				if(m_nextSpell->spell->CastingTimeIndex != 1)
				...
				SpellCastTime *sd = dbcSpellCastTime.LookupEntry(m_nextSpell->spell->m_castingTimeIndex);
				if(GetCastTime(sd) != 0)
					StopMovement(0);
...
/*					if(m_nextSpell->procCount)
					...
					{	
						Pet * pPet = static_cast<Pet*>(m_Unit);
						if(pPet && Rand(10))
							pPet->AddPetSpellToOwner(spellInfo->m_id);
					}
					m_nextSpell = NULL;
				}
				else // Target out of Range -> Run to it
...
SpellEntry *AIInterface::getSpellEntry(uint32 spellId)
{
	...
	if(!spellInfo)
	{
		sLog.outError("AIInterface.cpp(2944): unknown spell id %i\n", spellId);
		return NULL;
	}
		...
		for(list<AI_Spell*>::iterator itr = m_spells.begin(); itr != m_spells.end();)
		{
			...
			if(sp->agent == AGENT_SPELL)
			{
				if (sp->spellType == STYPE_BUFF)
				{
					...
					{
						if(!m_Unit->HasActiveAura(sp->spell->m_id))
						{
							return sp;
						}
					}
				...
				else
				{
					...
					if(sp->procChance >= 100 || Rand(sp->procChance))
					{
						...
						switch(sp->spell->m_powerType)
						{
						case POWER_TYPE_MANA:
							if(m_Unit->GetUInt32Value(UNIT_FIELD_POWER1) < sp->spell->m_manaCost)
								continue;
							break;

						case POWER_TYPE_FOCUS:
							if(m_Unit->GetUInt32Value(UNIT_FIELD_POWER3) < sp->spell->m_manaCost)
								continue;
							break;
						}
						def_spell = sp;
...
void AIInterface::addSpellToList(AI_Spell *sp)
{
	...
	m_Unit->m_SpellList.insert(sp->spell->m_id); // add to list
}
