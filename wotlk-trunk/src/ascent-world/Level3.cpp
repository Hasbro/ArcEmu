...
bool ChatHandler::HandleLearnCommand(const char* args, WorldSession *m_session)
{
	...
	if(stricmp(args, "all")==0)
	{
		...
		static uint32 spellarray[DRUID+1][9999] = {
			...
			{ 48721, 49939, 49940, 49941, 49998, 49999, 45463, 49923, 49924, 45902, 49926, 49927, 49928, 49929, 49930, 48680, 49913, 49914, 49915, 49916, 47802, 48743, 49934, 45524, 48263, 48266, 47528, 49910, 49911, 49912, 45477, 49896, 49903, 49904, 49909, 47568, 46584, 43265, 49936, 49937, 49938, 45462, 49917, 49918, 49919, 49920, 49921, 47541, 49892, 49893, 49894, 49895, 45843, 49932, 49933, 48265, 42650, 48707, 48778 },		// CLASS 6
	...
	if(!plr->GetSession()->HasGMPermissions() && (sp->m_effect[0]==SPELL_EFFECT_INSTANT_KILL||sp->m_effect[1]==SPELL_EFFECT_INSTANT_KILL||sp->m_effect[2]==SPELL_EFFECT_INSTANT_KILL))
	{
...
bool ChatHandler::HandleCastAllCommand(const char* args, WorldSession* m_session)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (info->m_effect[i] == 36) //SPELL_EFFECT_LEARN_SPELL - 36
		{
...
void SendItemLinkToPlayer(ItemPrototype * iProto, WorldSession * pSession, bool ItemCount)
{
	if(!iProto || !pSession)
		return;
	...
	if(ItemCount)
		sChatHandler.SystemMessage(pSession,"Item %u |%s|Hitem:%u:0:0:0:0:0:0:0|h[%s]|h|r  count %u", iProto->ItemId ,q.c_str(),iProto->ItemId,iProto->Name1, pSession->GetPlayer()->GetItemInterface()->GetItemCount(iProto->ItemId, true));
	else
...
bool ChatHandler::HandleGORotate(const char * args, WorldSession * m_session)
{
	...
	go->ModFloatValue(GAMEOBJECT_PARENTROTATION+1, rad);
	go->ModFloatValue(GAMEOBJECT_PARENTROTATION+2, rad);
	go->ModFloatValue(GAMEOBJECT_PARENTROTATION+3, rad);
	go->SaveToDB();
...
bool ChatHandler::HandleAIAgentDebugBegin(const char * args, WorldSession * m_session)
{
	...
	{
		result = WorldDatabase.Query("SELECT * FROM ai_agents WHERE spell = %u", (*itr)->m_id);
		ASSERT(result);
		spell_thingo t;
		t.type = result->Fetch()[6].GetUInt32();
		t.target = result->Fetch()[7].GetUInt32();
		delete result;
		aiagent_extra[(*itr)->m_id] = t;
	}
...
bool ChatHandler::HandleAIAgentDebugContinue(const char * args, WorldSession * m_session)
{
	...
	for(uint32 i = 0; i < count; ++i)
	{
		...
		BlueSystemMessage(m_session, "Casting %u, "MSG_COLOR_SUBWHITE"%u remaining.", sp->m_id, aiagent_spells.size());

		map<uint32, spell_thingo>::iterator it = aiagent_extra.find(sp->m_id);
...
bool ChatHandler::HandleShowItems(const char * args, WorldSession * m_session)
{
	...
	for(; !itr.End(); itr.Increment())
	{
		...
		SendItemLinkToPlayer((*itr)->GetProto(), m_session, true);
	}
	itr.EndSearch();
...
bool ChatHandler::HandleAddTrainerSpellCommand( const char * args, WorldSession * m_session )
{
	...
	if( pSpell->m_effect[0] == SPELL_EFFECT_INSTANT_KILL || pSpell->m_effect[1] == SPELL_EFFECT_INSTANT_KILL || pSpell->m_effect[2] == SPELL_EFFECT_INSTANT_KILL )
	{
	...
	SystemMessage(m_session, "Added spell %u (%s) to trainer.", pSpell->m_id, pSpell->m_name_lang);
	...
	WorldDatabase.Execute("INSERT INTO trainer_spells VALUES(%u, %u, %u, %u, %u, %u, %u, %u, %u, %u)", 
		pCreature->GetEntry(), (int)0, pSpell->m_id, cost, reqspell, (int)0, (int)0, reqlevel, delspell, (int)0);
