...
void Spell::SpellEffectInstantKill(uint32 i)
{
	...
	*/
	...
	//m_caster->SpellNonMeleeDamageLog(unitTarget, m_spellInfo->m_id, unitTarget->GetUInt32Value(UNIT_FIELD_HEALTH), true);
...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	if(unitTarget->SchoolImmunityList[m_spellInfo->m_schoolMask])
	{
	...
	if(m_spellInfo->m_effectChainTargets[i])//chain
	{
		int32 reduce = (int32)(m_spellInfo->m_effectChainAmplitude[i] * 100.0f);
		
		if(reduce && chaindamage)
		{
			if(m_spellInfo->m_spellClassMask && u_caster)
			{
				SM_FIValue(u_caster->SM_PJumpReduce,&reduce,m_spellInfo->m_spellClassMask);
			}
	...
	else 
	{
		...
		case 0xCBC738B8:	// Bloodthirst
			{
                dmg = u_caster->GetAP()*(m_spellInfo->m_effectBasePoints[0]+1) / 100;
			}break;
		...
		// REPLACE THIS INTO NAMEHASH SWITCH WHEN WE GET AN UPDATED 3.0.1 NAMEHASH LIST!
		switch( m_spellInfo->m_id )
		{
			...
			case 49949:
			case 49941:
				// Causes all disease effects on targets within 30 yards to painfully erupt,
				// consuming the diseases, dealing 1 to 0 damage, and forcing the target to attack
				// the Death Knight for 3 sec.
				
				// Wow, this is interesting, what happens if there is no disease?
				bool has_disease = false;
				for( uint32 x = MAX_POSITIVE_AURAS ; x <= MAX_AURAS ; x ++ )
				{
					if( unitTarget->m_auras[x] && unitTarget->m_auras[x]->m_spellProto->m_dispelType == DISPEL_DISEASE )
					{
						has_disease = true;
						break;
					}
	...
	**************************************************************************/
	...
	if(m_spellInfo->m_speed > 0)
	{
		...
		float time = ((dist*1000.0f)/m_spellInfo->m_speed);
		if(time <= 100)
			m_caster->SpellNonMeleeDamageLog(unitTarget,m_spellInfo->m_id, dmg, pSpellId==0);
		else
		{
			damageToHit = dmg;
			/*sEventMgr.AddEvent(m_caster, &Object::SpellNonMeleeDamageLog,
				unitTarget,m_spellInfo->m_id,dmg, EVENT_SPELL_DAMAGE_HIT, uint32(time), 1);*/
			sEventMgr.AddEvent(m_caster, &Object::EventSpellDamage, unitTarget->GetGUID(), 
				m_spellInfo->m_id, dmg, EVENT_SPELL_DAMAGE_HIT, uint32(time), 1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
	}
	else
	{
		...
		{
			m_caster->SpellNonMeleeDamageLog( unitTarget, m_spellInfo->m_id, dmg, pSpellId == 0, static_damage );
		}
		else 
		{
			if( u_caster != NULL )
	...
	case 28332:
		{
			...
			rss->chance = m_spellInfo->m_procChance;
			rss->spellId = m_spellInfo->m_id;
	...
	case 13043:
		{
			...
			ReflectSpellSchool *rss = new ReflectSpellSchool;
			rss->chance = m_spellInfo->m_effectBasePoints[0];
			rss->spellId = m_spellInfo->m_id;
	...
	case 35729:
		{
			...
			for(uint32 i = MAX_POSITIVE_AURAS; i < MAX_AURAS; ++i)
			{
				...
				if( pAura != NULL && !pAura->IsPassive() && !pAura->IsPositive() && !(pAura->GetSpellProto()->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY) &&
					pAura->GetSpellProto()->m_schoolMask != 0 )
				{
					pAura->Remove();
				}
			}
	...
	case 18350:
		{
			...
			uint32 cost = float2int32( float( float(sp->m_manaCost) * 0.6f ) );
...
//			u_caster->SpellNonMeleeDamageLog(unitTarget,spellId,damage,false);
	...
	case 27222:
		...
		uint32 damage = (((m_spellInfo->m_effectBasePoints[i]+1)*(100+playerTarget->m_lifetapbonus))/100)+((playerTarget->GetDamageDoneMod(m_spellInfo->m_schoolMask)*80)/100);
	...
	case 27172: //Judgement of Command
		{
			uint32 SpellID = m_spellInfo->m_effectBasePoints[i]+1;
	...
	*********************************/
	...
	/* DEATH COIL */
		...
		case 49895:
			/* 
			Unleashes all available runic power, causing up to ${$m1+$AP*0.25} Shadow damage to an enemy target
			or healing up to ${($m1+$AP*0.25)*1.5} damage from a friendly Undead target.

			AP * 0.25 already handled by core (SpellEntry::fixed_apcoefficient)
			*/
			if( !p_caster || !unitTarget)
				return;

			if(isAttackable( p_caster, unitTarget)) // if its an enemy
			{
				p_caster->SpellNonMeleeDamageLog( unitTarget , m_spellInfo->m_id , damage , true , true , false );
			}
			else // if its friendly
			{
				p_caster->Heal( unitTarget , m_spellInfo->m_id , float2int32((float)damage * 1.5f) );
			}
...
void Spell::SpellEffectApplyAura(uint32 i)  // Apply Aura
{
	...
	if ( !playerTarget )
	{
		...
		{
			switch(m_spellInfo->m_effectAura[i])
			{
			case 5:  // confuse
			case 6:  // charm
			case 7:  // fear
	...
	std::map<uint32,Aura*>::iterator itr=unitTarget->tmpAura.find(m_spellInfo->m_id);
	...
	if(itr==unitTarget->tmpAura.end())
	{
		...
		if(!(m_spellInfo->m_attributes & ATTRIBUTES_PASSIVE)) // Passive
		...
		// death knight shizz, amplitude
		// WARNING: SLOW SHIT INCOMING! =/
		if( m_spellInfo->m_powerType == POWER_TYPE_RUNIC_POWER )
			pAura->ModAmplitude( (float)u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) / 1000.0f );

		pAura->pSpellId = pSpellId; //this is required for triggered spells
		
		unitTarget->tmpAura[m_spellInfo->m_id] = pAura;		
	}
	else
	{
		 pAura=itr->second;
	} 
	pAura->AddMod(m_spellInfo->m_effectAura[i],damage,m_spellInfo->m_effectMiscValue[i],i);
}
...
void Spell::SpellEffectPowerDrain(uint32 i)  // Power Drain
{
	...
	uint32 powerField = UNIT_FIELD_POWER1+m_spellInfo->m_effectMiscValue[i];
	uint32 curPower = unitTarget->GetUInt32Value(powerField);
	uint32 amt=damage+((u_caster->GetDamageDoneMod(m_spellInfo->m_schoolMask)*80)/100);
	if(amt>curPower)
	{
		amt=curPower;
	}
	unitTarget->SetUInt32Value(powerField,curPower-amt);
	uint32 m=u_caster->GetUInt32Value(UNIT_FIELD_MAXPOWER1+m_spellInfo->m_effectMiscValue[i]);
	...
	SendHealManaSpellOnPlayer(u_caster, u_caster, amt, m_spellInfo->m_effectMiscValue[i]);
}
...
void Spell::SpellEffectHealthLeech(uint32 i) // Health Leech
{
	...
	m_caster->DealDamage(unitTarget, damage, 0, 0, m_spellInfo->m_id);
...
void Spell::SpellEffectHeal(uint32 i) // Heal
{
	if(m_spellInfo->m_effectChainTargets[i])//chain
	{
		...
		{
			int32 reduce=m_spellInfo->m_effectDieSides[i]+1;
			if(m_spellInfo->m_spellClassMask && u_caster)
			{
				SM_FIValue(u_caster->SM_PJumpReduce,&reduce,m_spellInfo->m_spellClassMask);
			}
		...
		case 31616:
			{
				if(unitTarget && unitTarget->IsPlayer() && pSpellId && unitTarget->GetHealthPct()<30)
				{
					...
					if(spellInfo)
					{
						...
						int32 value = 0;
						int32 basePoints = spellInfo->m_effectBasePoints[i]+1;//+(m_caster->getLevel()*basePointsPerLevel);
						int32 randomPoints = spellInfo->m_effectDieSides[i];
						if(randomPoints <= 1)
							value = basePoints;
						else
		...
		case 18562: //druid - swiftmend
			{
				if( unitTarget )
				{
					...
					{
						uint32 amplitude = taura->GetSpellProto()->m_effectAmplitude[1] / 1000;
						if( !amplitude )
							amplitude = 3;
					...
					else
					{
						...
						if( taura != NULL && taura->GetSpellProto() != NULL )
						{
							uint32 amplitude = taura->GetSpellProto()->m_effectAmplitude[0] / 1000;
							if( !amplitude )
								amplitude = 3;
...
void Spell::SpellEffectWeapondamageNoschool(uint32 i) // Weapon damage + (no School)
{
	...
	if( GetType() == SPELL_DMG_TYPE_RANGED && m_spellInfo->m_speed > 0.0f )
	{
		float time = (m_caster->CalcDistance(unitTarget) * 1000.0f) / m_spellInfo->m_speed;
		if(time <= 100.0f)
...
void Spell::SpellEffectCreateItem(uint32 i) // Create item 
{
	...
	for(int j=0; j<3; j++) // now create the Items
	{
		...
		m_itemProto = ItemPrototypeStorage.LookupEntry( m_spellInfo->m_effectItemType[j] );
		if (!m_itemProto)
			 continue;

		if(m_spellInfo->m_effectItemType[j] == 0)
			continue;

		uint32 item_count = 0;
		if (m_itemProto->Class != ITEM_CLASS_CONSUMABLE || m_spellInfo->m_spellClassSet != 3) //m_spellClassSet 3 is mage
			item_count = damage;
		else if(p_caster->getLevel() >= m_spellInfo->m_spellLevel)
			item_count = ((p_caster->getLevel() - (m_spellInfo->m_spellLevel-1))*damage);
		...
		if (skill)
		{
			...
			for(; itr != objmgr.ProfessionDiscoveryTable.end(); itr++)
			{
				...
				if(!pf) continue; // should never happen, oh well
				if(pf->SkillID != skill->Id) continue;
				if(p_caster->HasSpell(pf->SpellID)) continue;
				
				if(Rand(pf->pctChance))
				{
					discovered_recipe = pf->SpellID;
					break;
				}
			}
		}
		...
		slot = 0;
		add = p_caster->GetItemInterface()->FindItemLessMax(m_spellInfo->m_effectItemType[j],1, false);
		if (!add)
		{
			...
			newItem =objmgr.CreateItem(m_spellInfo->m_effectItemType[i],p_caster);
			...
			{
				/*WorldPacket data(45);
				p_caster->GetSession()->BuildItemPushResult(&data, p_caster->GetGUID(), 1, item_count, m_spellInfo->m_effectItemType[i] ,0,0xFF,1,0xFFFFFFFF);
			...
			// If something discovered learn p_caster that recipe and broadcast message him
			if ( discovered_recipe )
			{
				...
				if ( se != NULL )
				{
					...
					WorldPacket * data;
					char msg[256];
					sprintf( msg, "%sDISCOVERY! %s has discovered how to create %s.|r", MSG_COLOR_GOLD, p_caster->GetName(), se->m_name_lang );
...
void Spell::SpellEffectWeapon(uint32 i)
{
	...
	default:
		{
			skill = 0;
			sLog.outDebug("WARNING: Could not determine skill for spell id %d (SPELL_EFFECT_WEAPON)", this->m_spellInfo->m_id);
		}break;
	}
...
void Spell::SpellEffectPersistentAA(uint32 i) // Persistent Area Aura
{
	...
	if(u_caster)
	if(m_spellInfo->m_channelInterruptFlags > 0)
	{
...
void Spell::SpellEffectSummon(uint32 i) // Summon
{
	...
	CreatureInfo * ci = CreatureNameStorage.LookupEntry(m_spellInfo->m_effectMiscValue[i]);
	CreatureProto * cp = CreatureProtoStorage.LookupEntry(m_spellInfo->m_effectMiscValue[i]);
	if( !ci || !cp )
		return;


	if(m_spellInfo->m_effectMiscValue[i] == 510)	// Water Elemental
	{
		Pet *summon = objmgr.CreatePet();
		summon->SetInstanceID(m_caster->GetInstanceID());
		summon->CreateAsSummon(m_spellInfo->m_effectMiscValue[i], ci, NULL, p_caster, m_spellInfo, 1, 45000);
...
void Spell::SpellEffectLeap(uint32 i) // Leap
{
	...
	for(uint32 i = MAX_POSITIVE_AURAS; i < MAX_AURAS; ++i)
	{
		if( p_caster->m_auras[i] != NULL )
		{
			...
			{
				if( p_caster->m_auras[i]->GetSpellProto()->m_effectAura[j] == SPELL_AURA_MOD_STUN || 
					p_caster->m_auras[i]->GetSpellProto()->m_effectAura[j] == SPELL_AURA_MOD_ROOT )
				{
...
void Spell::SpellEffectEnergize(uint32 i) // Energize
{
	...
	uint32 POWER_TYPE=UNIT_FIELD_POWER1+m_spellInfo->m_effectMiscValue[i];
	...
	if(m_spellInfo->m_id==30824)
		modEnergy = damage*GetUnitTarget()->GetAP()/100;
	//paladin illumination
	else if(m_spellInfo->m_id==20272 && ProcedOnSpell)
	{
		SpellEntry *motherspell=dbcSpell.LookupEntry(pSpellId);
		if(motherspell)
			modEnergy = (motherspell->m_effectBasePoints[0]+1)*ProcedOnSpell->m_manaCost/100;
	}
	//paladin - Spiritual Attunement 
	else if(m_spellInfo->m_id==31786 && ProcedOnSpell)
	{
		SpellEntry *motherspell=dbcSpell.LookupEntry(pSpellId);
		if(motherspell)
		{
			...
			uint32 healamt=0;
			if(ProcedOnSpell->m_effect[0]==SPELL_EFFECT_HEAL || ProcedOnSpell->m_effect[0]==SPELL_EFFECT_SCRIPT_EFFECT)
				healamt=ProcedOnSpell->m_effectBasePoints[0]+1;
			else if(ProcedOnSpell->m_effect[1]==SPELL_EFFECT_HEAL || ProcedOnSpell->m_effect[1]==SPELL_EFFECT_SCRIPT_EFFECT)
				healamt=ProcedOnSpell->m_effectBasePoints[1]+1;
			else if(ProcedOnSpell->m_effect[2]==SPELL_EFFECT_HEAL || ProcedOnSpell->m_effect[2]==SPELL_EFFECT_SCRIPT_EFFECT)
				healamt=ProcedOnSpell->m_effectBasePoints[2]+1;
			modEnergy = (motherspell->m_effectBasePoints[0]+1)*(healamt)/100;
		}
	}
	else if (m_spellInfo->m_id==2687){
		modEnergy = damage;
		if( p_caster != NULL )
		{
	...
	SendHealManaSpellOnPlayer(u_caster, unitTarget, modEnergy, m_spellInfo->m_effectMiscValue[i]);
...
void Spell::SpellEffectWeaponDmgPerc(uint32 i) // Weapon Percent damage
{
	...
	if( GetType() == SPELL_DMG_TYPE_MAGIC )
	{
		...
		u_caster->SpellNonMeleeDamageLog(unitTarget, m_spellInfo->m_id, dmg, false, false, false);
	}
	else
	{
		uint32 _type;
...
void Spell::SpellEffectTriggerMissile(uint32 i) // Trigger Missile
{
	...
	uint32 spellid = m_spellInfo->m_effectTriggerSpell[i];
	if(spellid == 0)
		return;
...
void Spell::SpellEffectOpenLock(uint32 i) // Open Lock
{
	...
	uint32 locktype=m_spellInfo->m_effectMiscValue[i];
	switch(locktype)
	{
			...
			else if(gameObjTarget)
			{
				...
				for(int i=0;i<5;i++)
				{
					if(lock->locktype[i] == 2 && lock->minlockskill[i] && lockskill >= lock->minlockskill[i])
					{
						...
						gameObjTarget->SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
...
void Spell::SpellEffectOpenLockItem(uint32 i)
{
	...
	gameObjTarget->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);	
...
{
	uint32 skill = 0;
	skilllinespell* skillability = objmgr.GetSpellSkill(m_spellInfo->m_id);
	if (skillability)
...
void Spell::SpellEffectLearnSpell(uint32 i) // Learn Spell
{
	...
	if( m_spellInfo->m_id == 483 )		// "Learning"
	{
	...
	if(playerTarget)
	{
		...
		uint32 spellToLearn = m_spellInfo->m_effectTriggerSpell[i];
		...
		for(uint32 i=0;i<3;i++)
			if(spellinfo->m_effect[i] == SPELL_EFFECT_WEAPON ||
			   spellinfo->m_effect[i] == SPELL_EFFECT_PROFICIENCY ||
			   spellinfo->m_effect[i] == SPELL_EFFECT_DUAL_WIELD )
			{
...
void Spell::SpellEffectLearnPetSpell(uint32 i)
{
	/*if(unitTarget && m_caster->GetTypeId() == TYPEID_PLAYER)
	{
		...
		{
			static_cast< Player* >(m_caster)->AddPetSpell(m_spellInfo->m_effectTriggerSpell[i], unitTarget->GetEntry());
		}
	}*/
	...
	if(unitTarget && unitTarget->IsPet() && p_caster)
	{
		...
		if(pPet->IsSummon())
			p_caster->AddSummonSpell(unitTarget->GetEntry(), m_spellInfo->m_effectTriggerSpell[i]);
		
		pPet->AddSpell( dbcSpell.LookupEntry( m_spellInfo->m_effectTriggerSpell[i] ), true );

		// Send Packet
		WorldPacket data(SMSG_SET_AURA_SINGLE, 22);
		data << pPet->GetGUID() << uint8(0) << uint32(m_spellInfo->m_effectTriggerSpell[i]) << uint32(-1) << uint32(0);
		p_caster->GetSession()->SendPacket(&data);
	}
}
...
void Spell::SpellEffectDispel(uint32 i) // Dispel
{
	...
	if(unitTarget->m_auras[x])
	{
		...
		if(!aur->IsPassive() && !(aur->GetSpellProto()->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
		{
			uint32 UARank = 0;
			if(m_spellInfo->m_dispelType == DISPEL_ALL)
			{
			...
			else if(aur->GetSpellProto()->m_dispelType == m_spellInfo->m_effectMiscValue[i])
			{
				unitTarget->HandleProc( PROC_ON_PRE_DISPELL_AURA_VICTIM , u_caster , m_spellInfo, aur->GetSpellId() );
				...
				data << aur->GetSpellId();
				m_caster->SendMessageToSet(&data,true);
				unitTarget->RemoveAllAuras(aur->GetSpellProto()->m_id,aur->GetCasterGUID());
				if(!--damage)
					return;
			}	
			if( UARank != 0 )
...
void Spell::SpellEffectSummonWild(uint32 i)  // Summon Wild
{
	...
	uint32 cr_entry=m_spellInfo->m_effectMiscValue[i];
	CreatureProto * proto = CreatureProtoStorage.LookupEntry(cr_entry);
	CreatureInfo * info = CreatureNameStorage.LookupEntry(cr_entry);
	if(!proto || !info)
	{
		sLog.outDetail("Warning : Missing summon creature template %u used by spell %u!",cr_entry,m_spellInfo->m_id);
		return;
	}
...
void Spell::SpellEffectSkillStep(uint32 i) // Skill Step
{
	...
	uint32 skill = m_spellInfo->m_effectMiscValue[i];
	if( skill == 242 )
	...
	else
	{
		...
		if( skill == SKILL_RUNEFORGING )
			target->_AddSkillLine( skill, 1, 1 );
		else
	...
	case SKILL_RUNEFORGING:
		target->addSpell( 53323 ); // Rune of Swordbreaking
		target->addSpell( 53331 ); // Rune of Scourgebane
		target->addSpell( 53341 ); // Rune of Cinderglacier
		target->addSpell( 53342 ); // Rune of Spellbreaking
		target->addSpell( 53343 ); // Rune of Icefever
		break;
	};
}
...
void Spell::SpellEffectSummonObject(uint32 i)
{
	...
	uint32 entry = m_spellInfo->m_effectMiscValue[i];
	...
	if( entry == GO_FISHING_BOBBER )
	{
		...
		go->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value( GAMEOBJECT_STATE, 0 );
	...
	{//portal, lightwell
		...
		go->SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE, 1);		
		go->SetUInt64Value(OBJECT_FIELD_CREATED_BY,m_caster->GetGUID());
		go->PushToWorld(m_caster->GetMapMgr());	  
		...
		{//enable it for party only
			go->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value( GAMEOBJECT_STATE,0);
			//disable by default
			WorldPacket *pkt = go->BuildFieldUpdatePacket(GAMEOBJECT_BYTES_1, 0); //wotlk not sure?
		...
		else if(entry == 36727 || entry == 177193) // Portal of Summoning and portal of doom
		{
			...
			go->m_ritualspell = m_spellInfo->m_id;	 
		}
...
void Spell::SpellEffectEnchantItem(uint32 i) // Enchant Item Permanent
{
	...
	EnchantEntry * Enchantment = dbcEnchant.LookupEntry(m_spellInfo->m_effectMiscValue[i]);
...
void Spell::SpellEffectSummonPet(uint32 i) //summon - pet
{
	...
	//uint32 entryId = m_spellInfo->m_effectMiscValue[i];
	...
	if(ci)
	{
		...
		Pet *summon = objmgr.CreatePet();
		summon->SetInstanceID(m_caster->GetInstanceID());
		summon->CreateAsSummon(m_spellInfo->m_effectMiscValue[i], ci, NULL, u_caster, m_spellInfo, 1, 0);
	}
}
...
void Spell::SpellEffectWeapondamage( uint32 i ) // Weapon damage +
{
	...
	if( m_spellInfo->m_effect[2] == SPELL_EFFECT_WEAPON_PERCENT_DAMAGE || m_spellInfo->m_effect[1] == SPELL_EFFECT_WEAPON_PERCENT_DAMAGE )
	{
		add_damage += damage;
		return;
	}
...
void Spell::SpellEffectPowerBurn(uint32 i) // power burn
{
	...
	m_caster->SpellNonMeleeDamageLog(unitTarget,m_spellInfo->m_id, (uint32)(mana * m_spellInfo->m_effectMultiple[i]), pSpellId==0,true);   
}
...
void Spell::SpellEffectThreat(uint32 i) // Threat
{
	...
	bool chck = unitTarget->GetAIInterface()->modThreatByPtr(u_caster,m_spellInfo->m_effectBasePoints[i]);
	if(chck == false)
...
void Spell::SpellEffectTriggerSpell(uint32 i) // Trigger Spell
{
	...
	Spell*sp=new Spell(m_caster,dbcSpell.LookupEntry(m_spellInfo->m_effectTriggerSpell[i]),true,NULL);
	SpellCastTargets tgt(unitTarget->GetGUID());
	sp->prepare(&tgt);

	//if(m_spellInfo->m_effectTriggerSpell[i] != 0)
	  //  TriggerSpellId = m_spellInfo->m_effectTriggerSpell[i];	
}
...
void Spell::SpellEffectInterruptCast(uint32 i) // Interrupt Cast
{
	...
	{
		school=unitTarget->GetCurrentSpell()->m_spellInfo->m_schoolMask;
	}
...
void Spell::SpellEffectSummonObjectWild(uint32 i)
{
	...
	// spawn a new one
	GameObject *GoSummon = u_caster->GetMapMgr()->CreateGameObject(m_spellInfo->m_effectMiscValue[i]);
	if(!GoSummon->CreateFromProto(m_spellInfo->m_effectMiscValue[i],
	...
	GoSummon->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
...
void Spell::SpellEffectScriptEffect(uint32 i) // Script Effect
{
	...
	case 25345://Rank 8
	{
		...
		if(m_spellInfo->m_effectBasePoints[0] < 0)
			m_tmpAffect->SetNegative();

		m_tmpAffect->SetPeriodicTriggerSpell(m_spellInfo->m_effectTriggerSpell[0],m_spellInfo->m_effectAmplitude[0],damage);
	}break;
	*/
...
void Spell::SpellEffectAddComboPoints(uint32 i) // Add Combo Points
{
	...
	if(pSpellId && m_spellInfo->m_id==14157)
	{
...
void Spell::SpellEffectSummonTotem(uint32 i) // Summon Totem
{
	...
	SpellEntry * TotemSpell = ObjectMgr::getSingleton().GetTotemSpell(m_spellInfo->m_id);
	if(TotemSpell == 0) 
	{
	...
	pTotem->SetUInt32Value(UNIT_CREATED_BY_SPELL, m_spellInfo->m_id);
	...
	for( j = 0; j < 3; ++j )
	{
		if( TotemSpell->m_effect[j] == SPELL_EFFECT_APPLY_AREA_AURA || TotemSpell->m_effect[j] == SPELL_EFFECT_PERSISTENT_AREA_AURA || TotemSpell->m_effectAura[j] == SPELL_AURA_PERIODIC_TRIGGER_SPELL )
		{
			break;
		}
	}
...
void Spell::SpellEffectSelfResurrect(uint32 i)
{
	...
	case 27240:
		{
			health = m_spellInfo->m_effectMiscValue[i];
			mana = -damage;
		}break;
...
void Spell::SpellEffectSummonCritter(uint32 i)
{
	...
	uint32 SummonCritterID = m_spellInfo->m_effectMiscValue[i];

	// m_spellInfo->m_effectDieSides[i] has something to do with dismissing our critter
...
void Spell::SpellEffectKnockBack(uint32 i)
{
	...
	float dx, dy;
	float value1 = float(m_spellInfo->m_effectBasePoints[i]+1);
	float value2 = float(m_spellInfo->m_effectMiscValue[i]);
	float proportion;
...
void Spell::SpellEffectFeedPet(uint32 i)  // Feed Pet
{
	...
	SpellEntry *spellInfo = dbcSpell.LookupEntry(m_spellInfo->m_effectTriggerSpell[i]);
...
void Spell::SpellEffectReputation(uint32 i)
{
	...
	//playerTarget->modReputation(m_spellInfo->m_effectMiscValue[i], damage, true);
	playerTarget->ModStanding(m_spellInfo->m_effectMiscValue[i], damage);
}
...
void Spell::SpellEffectSummonObjectSlot(uint32 i)
{
	...
	uint32 slot=m_spellInfo->m_effect[i] - SPELL_EFFECT_SUMMON_OBJECT_SLOT1;
	...
	// spawn a new one
	GoSummon = u_caster->GetMapMgr()->CreateGameObject(m_spellInfo->m_effectMiscValue[i]);
   if(! GoSummon->CreateFromProto(m_spellInfo->m_effectMiscValue[i],
		m_caster->GetMapId(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), m_caster->GetOrientation() ))
   {
		delete GoSummon;
...
void Spell::SpellEffectDispelMechanic(uint32 i)
{
	...
	uint32 sMisc = m_spellInfo->m_effectMiscValue[i];
	...
	*/
	unitTarget->RemoveAllAurasByMechanic( m_spellInfo->m_effectMiscValue[i] , m_spellInfo->m_effectBasePoints[i] , true );
...
void Spell::SpellEffectDestroyAllTotems(uint32 i)
{
	...
	for(uint32 x=0;x<4;x++)
	{
		...
		if(p_caster->m_TotemSlots[x])
		{
			...
			float pts = float(m_spellInfo->m_effectBasePoints[i]+1) / 100.0f;
			RetreivedMana += float(sp->m_manaCost) * pts;
...
{
	if(!p_caster || p_caster->GetShapeShift() != FORM_DEMON ) 
		return;
	Pet *pPet = p_caster->GetSummon();
	if(pPet)
	{
	...
	if(ci)
	{
		...
		pPet->SetInstanceID(p_caster->GetInstanceID());
		pPet->CreateAsSummon(m_spellInfo->m_effectMiscValue[i], ci, NULL, p_caster, m_spellInfo, 1, 300000);
	}
	//Create Enslave Aura if its inferno spell
	if(m_spellInfo->m_id == 1122)
	{
...
void Spell::SpellEffectResurrect(uint32 i) // Resurrect (Flat)
{
	if(!playerTarget)
	{
		if(!corpseTarget)
		{
			...
			if(unitTarget)
			{
				...
				{
					uint32 hlth = ((uint32)m_spellInfo->m_effectBasePoints[i] > unitTarget->GetUInt32Value(UNIT_FIELD_MAXHEALTH)) ? unitTarget->GetUInt32Value(UNIT_FIELD_MAXHEALTH) : (uint32)m_spellInfo->m_effectBasePoints[i];
					uint32 mana = ((uint32)m_spellInfo->m_effectBasePoints[i] > unitTarget->GetUInt32Value(UNIT_FIELD_MAXPOWER1)) ? unitTarget->GetUInt32Value(UNIT_FIELD_MAXPOWER1) : (uint32)m_spellInfo->m_effectBasePoints[i];
	...
	uint32 health = m_spellInfo->m_effectBasePoints[i];
	uint32 mana = m_spellInfo->m_effectMiscValue[i];
...
void Spell::SpellEffectSkill(uint32 i)
{
	...
	if ( !p_caster || p_caster->_GetSkillLineMax(m_spellInfo->m_effectMiscValue[i]) >= uint32( damage * 75 ) )
		return;
	
	if ( p_caster->_HasSkillLine( m_spellInfo->m_effectMiscValue[i]) )
		p_caster->_ModifySkillMaximum( m_spellInfo->m_effectMiscValue[i], uint32( damage * 75 ) );
	else
		p_caster->_AddSkillLine( m_spellInfo->m_effectMiscValue[i], 1, uint32( damage * 75 ) );
}
...
void Spell::SpellEffectDummyMelee( uint32 i ) // Normalized Weapon damage +
{
	...
	else if( m_spellInfo->NameHash == SPELL_HASH_DEVASTATE)
	{
		...
		spell->pSpellId=m_spellInfo->m_id;
	...
	for (uint32 x =0;x<3;x++)
		if(m_spellInfo->m_effect[x] == SPELL_EFFECT_WEAPON_PERCENT_DAMAGE)
		{
			add_damage = damage * (m_spellInfo->m_effectBasePoints[x]+1) /100;
			break;
		}
	switch( m_spellInfo->m_id )
	{
		// BLOOD STRIKE (TODO: Add spell hash for this)
		case 45902:
		case 49926:
		case 49927:
		case 49928:
		case 49929:
		case 49930:
			//Log.Notice( "DK" , "Blood Strike Dummy" );
		// Death Knight: Blood Strike (extra damage per disease on the target)
			// FORMULA: $s1*$s2/100 (EffectBasePoints0 * EffectBasePoints1/100)
			uint32 dk_bs_add_damage;
			add_damage = 0; // !!!! overpowered because of " rogue ambush etc" section.
			dk_bs_add_damage = float2int32( (float)( m_spellInfo->m_effectBasePoints[0] + 1 ) * (float)( m_spellInfo->m_effectBasePoints[1] + 1 ) / 100.0f );
			for( uint32 a = MAX_POSITIVE_AURAS ; a <= MAX_AURAS ; a ++ )
			{
				if( unitTarget->m_auras[a] && unitTarget->m_auras[a]->GetSpellProto()->m_dispelType == DISPEL_DISEASE )
				{
					add_damage += dk_bs_add_damage;
				}
			}
			//Log.Notice( "DK" , "add damage = %u, dk_bs_add_damage = %u" , add_damage , dk_bs_add_damage );
			return;
			break;
	}
...
void Spell::SpellEffectSpellSteal( uint32 i )
{
	...
	if(unitTarget->m_auras[x])
	{
		...
		{
			if(aur->GetSpellProto()->m_dispelType == DISPEL_MAGIC)
			{
				data.clear();
				data << m_caster->GetNewGUID();
				data << unitTarget->GetNewGUID();
...
void Spell::SpellEffectResurrectNew(uint32 i)
{
   ...
   //resurr
	...
	playerTarget->m_resurrectMana = m_spellInfo->m_effectMiscValue[i];
...
void Spell::SpellEffectTranformItem(uint32 i)
{
	...
	if(!i_caster)
		return;
	uint32 itemid=m_spellInfo->m_effectItemType[i];
	if(!itemid)
		return;
...
void Spell::SpellEffectEnvironmentalDamage(uint32 i)
{
	...
	//this is GO, not unit	
	m_caster->SpellNonMeleeDamageLog(unitTarget,m_spellInfo->m_id,damage, pSpellId==0);
...
void Spell::SpellEffectAddHonor(uint32 i)
{
	...
	HonorHandler::AddHonorPointsToPlayer( playerTarget, m_spellInfo->m_implicitTargetA[i] );
}
...
{
	if(m_spellInfo->m_effectAura[i] != 0)
		SpellEffectApplyAura(i);
}
...
void Spell::SpellEffectTriggerSpellWithValue(uint32 i)
{
	...
	SpellEntry* TriggeredSpell = dbcSpell.LookupEntryForced(m_spellInfo->m_effectTriggerSpell[i]);
	if( TriggeredSpell == NULL )
		return;

	Spell*sp=new Spell(m_caster,dbcSpell.LookupEntry(TriggeredSpell->m_id),true,NULL);

	for(uint32 x=0;x<3;x++)
	{
		sp->forced_basepoints[x] = TriggeredSpell->m_effectBasePoints[i];
	}
