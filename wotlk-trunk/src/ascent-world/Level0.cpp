...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession *m_session)
{
	...
	if(m_session->GetPlayer()->getClass() == DKNIGHT)
			m_session->GetPlayer()->SafeTeleport(609, 0, LocationVector(2487.19f, -5588.05f, 550.2f));
	else
...
bool ChatHandler::HandleInfoCommand(const char* args, WorldSession *m_session)
{
	...
	GreenSystemMessage(m_session, "Server Revision: |r%sAscent r%u/%s-%s-%s %s(www.darktidegaming.com)", MSG_COLOR_WHITE,
