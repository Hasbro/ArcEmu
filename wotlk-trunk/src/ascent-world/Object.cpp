...
uint32 Object::BuildCreateUpdateBlockForPlayer(ByteBuffer *data, Player *target)
{
	...
	{
		switch(m_uint32Values[GAMEOBJECT_BYTES_1, 1])
		{
	...
	if ((isCritter || health <= damage) )
	{
		...
		if(pVictim->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT) > 0)
		{
			if(pVictim->GetCurrentSpell())
			{
				...
				for(int i = 0; i < 3; i++)
				{
					if(spl->m_spellInfo->m_effect[i] == SPELL_EFFECT_PERSISTENT_AREA_AURA)
					{
				...
				if(spl->m_spellInfo->m_channelInterruptFlags == 48140) spl->cancel();
			}
		}
...
//------------------------------critical strike chance--------------------------------------	
			...
			if( spellInfo->is_ranged_spell )
			{
				...
				if( IsPlayer() )
				{
					...
					CritChance += (float)(pVictim->AttackerCritChanceMod[spellInfo->m_schoolMask]);
				}
				else
				{
			...
			else
			{
				...
				{
					SM_FFValue(caster->SM_CriticalChance, &CritChance, spellInfo->m_spellClassMask);
	#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
					float spell_flat_modifers=0;
					SM_FFValue(caster->SM_CriticalChance,&spell_flat_modifers,spellInfo->m_spellClassMask);
					if(spell_flat_modifers!=0)
						printf("!!!!spell critchance mod flat %f ,spell group %u\n",spell_flat_modifers,spellInfo->m_spellClassMask);
	#endif
				}
				if( pVictim->IsPlayer() )
...
//==========================================================================================
			...
			{		
				...
				if( spellInfo->m_spellClassMask )
					SM_FIValue( caster->SM_PCriticalDamage, &critical_bonus, spellInfo->m_spellClassMask );

				if( critical_bonus > 0 )
				{
					// the bonuses are halved by 50% (funky blizzard math :S)
					float b;
					if( spellInfo->m_schoolMask == 0 || spellInfo->is_melee_spell || spellInfo->is_ranged_spell )		// physical || hackfix SoCommand/JoCommand
...
//==========================================================================================
	...
	if( this->IsUnit() && allowProc && spellInfo->m_id != 25501 )
	{
...
bool Object::CanActivate()
{
	...
	case TYPEID_GAMEOBJECT:
		{
			if(static_cast<GameObject*>(this)->HasAI() && GetByteValue(GAMEOBJECT_BYTES_1, 1) != GAMEOBJECT_TYPE_TRAP)
				return true;
		}break;
	}
