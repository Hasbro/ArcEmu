...
/*	unsigned int numRows = sTalentStore.GetNumRows();
	...
	else
	{
		...
		if(!(GetPlayer( )->HasSpell(spellid)))
		{
			...
			if( (spellInfo->m_attributes & ATTRIBUTES_PASSIVE || (spellInfo->m_effect[0] == SPELL_EFFECT_LEARN_SPELL ||
															   spellInfo->m_effect[1] == SPELL_EFFECT_LEARN_SPELL ||
															   spellInfo->m_effect[2] == SPELL_EFFECT_LEARN_SPELL) 
			...
			{
				if( spellInfo->m_shapeshiftMask && !( (uint32)1 << (_player->GetShapeShift()-1) & spellInfo->m_shapeshiftMask ) )
				{
					// do nothing
				}
				else
