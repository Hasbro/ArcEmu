...
bool GameObject::CreateFromProto(uint32 entry,uint32 mapid, float x, float y, float z, float ang)
{
 ...
 //   SetUInt32Value( GAMEOBJECT_ARTKIT, 0 );		   //these must be from wdb somewhere i guess
   // SetUInt32Value( GAMEOBJECT_ANIMPROGRESS, 0 );
    SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //	SetUInt32Value( GAMEOBJECT_STATE, 1 );
	SetUInt32Value( GAMEOBJECT_DISPLAYID, pInfo->DisplayID );
	//SetUInt32Value( GAMEOBJECT_TYPE_ID, pInfo->Type );
	SetByteValue(GAMEOBJECT_BYTES_1, 1, pInfo->Type);
...
void GameObject::Update(uint32 p_time)
{
	...
	if(spell && (GetByteValue(GAMEOBJECT_BYTES_1, 0) == 1))
	{
		if(checkrate > 1)
		{
		...
		for(; it2 != iend;)
		{
			...
			if( (*itr) != m_summoner && (*itr)->IsUnit() && dist <= range)
			{
				...
				if(spell->m_implicitTargetA[0] == 16 ||
					spell->m_implicitTargetB[0] == 16)
...
void GameObject::Despawn(uint32 time)
{
	...
	if(m_spawn)
	{
        SetByteValue(GAMEOBJECT_BYTES_1, 0, m_spawn->state); // SetUInt32Value(GAMEOBJECT_STATE, m_spawn->state);
...
void GameObject::InitAI()
{
	...
	for(uint32 i=0;i<3;i++)
	{
		if(sp->m_effect[i])
		{
			float t = GetRadius(dbcSpellRadius.LookupEntry(sp->m_effectRadiusIndex[i]));
			if(t > r)
				r = t;
		}
	}
	...
	if(r < 0.1)//no range
		r = GetMaxRange(dbcSpellRange.LookupEntry(sp->m_rangeIndex));
...
bool GameObject::Load(GOSpawn *spawn)
{
	...
	SetFloatValue(GAMEOBJECT_PARENTROTATION+1,spawn->o1);
	SetFloatValue(GAMEOBJECT_PARENTROTATION+2,spawn->o2);
	SetFloatValue(GAMEOBJECT_PARENTROTATION+3,spawn->o3);
	SetUInt32Value(GAMEOBJECT_FLAGS,spawn->flags);
//	SetUInt32Value(GAMEOBJECT_LEVEL,spawn->level);
	SetByteValue(GAMEOBJECT_BYTES_1, 0, spawn->state); 
...
{
	SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
}
...
void GameObject::UseFishingNode(Player *player)
{
	...
	{
		if ( (*it) == NULL || (*it)->GetTypeId() != TYPEID_GAMEOBJECT || (*it)->GetByteValue(GAMEOBJECT_BYTES_1, 1) != GAMEOBJECT_TYPE_FISHINGHOLE)
			continue;
