...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
	...
	if( Config.MainConfig.GetBoolDefault( "DarkTide" , "DKRequiresLevel55" , false ) && !has_level_55_char 
		&& ( class_ == DKNIGHT ) )
	{	
		pNewChar->ok_to_remove = true;
		delete pNewChar;
		...
		data.SetOpcode(SMSG_CHAR_CREATE);
		//int val;
		//val = INVALID_CHARACTER_NAME;
		//val = Config.MainConfig.GetIntDefault( "DarkTide" , "DEC" , 10 );
		//	Log.Notice( "       " , "CURRENTLY ERR = %u " , _TEMP_ERR_CREATE_CODE );
		//	_TEMP_ERR_CREATE_CODE ++;
		data << (uint8)56 + 1;
		SendPacket( &data );
		return;
	}
...
void WorldSession::FullLogin(Player * plr)
{
	...
	*/
    ...
    if (plr->getRace() == 1 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Human
    {
            data << uint32(81);
            SendPacket( &data );
    }

    if (plr->getRace() == 2 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Orc
    {
            data << uint32(21);
            SendPacket( &data );
    }

    if (plr->getRace() == 3 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Dwarf
    {
            data << uint32(41);
            SendPacket( &data );
    }
    if (plr->getRace() == 4 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Night Elves
    {
            data << uint32(61);
            SendPacket( &data );
    }
    if (plr->getRace() == 5 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // undead <-- WORKING thats Correct
    {
            data << uint32(2);
            SendPacket( &data );
    }
    if (plr->getRace() == 6 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Tauren
    {
            data << uint32(141);
            SendPacket( &data );
    }
    if (plr->getRace() == 7 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Gnome
    {
            data << uint32(101);
            SendPacket( &data );
    }
    if (plr->getRace() == 8 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Troll
    {
            data << uint32(121);
            SendPacket( &data );
    }

	if (plr->getRace() == 10 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Blood Elf
    {
        {
            data << uint32(162);
            SendPacket( &data );
	...
	if (plr->getRace() == 11 && plr->m_FirstLogin && !HasGMPermissions() && plr->getClass() != DKNIGHT) // Draenei
    {
        {
            data << uint32(163);
            SendPacket( &data );
	...
	if (plr->getClass() == DKNIGHT && plr->m_FirstLogin && !HasGMPermissions()) // Death Knight
    {
        {
            data << uint32(165);
            SendPacket( &data );
...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sAscent %s r%u/%s-Win-%s %s(www.darktidegaming.com)", MSG_COLOR_WHITE, BUILD_TAG,
		BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);		
#else
	_player->BroadcastMessage("Server: %sAscent %s r%u/%s-%s %s(www.darktidegaming.com)", MSG_COLOR_WHITE, BUILD_TAG,
