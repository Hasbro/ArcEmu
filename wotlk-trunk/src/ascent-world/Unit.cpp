...
//				CastingSpell->Id == itr2->origId || // seed of corruption and pyroclasm proc on self 
				CastingSpell->m_id == itr2->spellId )
			{
		...
		if( itr2->procFlags & flag )
		{
			...
			if( itr2->procFlags & PROC_ON_CAST_SPECIFIC_SPELL )
			{
				...
				if( sp->m_spellVisualID != CastingSpell->m_spellVisualID )
				{
					if( !ospinfo->m_schoolMask )
						continue;
					if( ospinfo->m_schoolMask != CastingSpell->m_schoolMask )
						continue;
					if( CastingSpell->m_implicitTargetA[0] == 1 || 
						CastingSpell->m_implicitTargetA[1] == 1 || 
						CastingSpell->m_implicitTargetA[2] == 1) //Prevents school based procs affecting caster when self buffing
						continue;
				}
				else
					if( sp->m_spellVisualID == 1 )
						continue;
			}
			...
			SM_FIValue( SM_FChanceOfSuccess, (int32*)&proc_Chance, ospinfo->m_spellClassMask );
				...
				if( IsPlayer() )
				{
					...
					switch( spellId )
					{
						...
						case 16959: //Primal Fury
						{
							if( !this->IsPlayer() || 
								!CastingSpell || 
								CastingSpell->m_id == 14189 ||
								CastingSpell->m_id == 16953 ||
								CastingSpell->m_id == 16959)
								continue;
							if( CastingSpell->m_effect[0] != 80 &&
								CastingSpell->m_effect[1] != 80 &&
								CastingSpell->m_effect[2] != 80 &&
						...
						case 17106: //druid intencity
						{
							if( CastingSpell == NULL )
								continue;
							if( CastingSpell->m_id != 5229 )//enrage
								continue;
						}break;
						case 31616:
						{
...
/*						//disabled by zack until finished : this needs to get trigered on trap trigger and not trap cast
						...
						case 12654:
						{
							if( CastingSpell == NULL )
								continue;
							if( CastingSpell->m_schoolMask != SCHOOL_FIRE )
								continue;
							SpellEntry* spellInfo = dbcSpell.LookupEntry( spellId ); //we already modified this spell on server loading so it must exist
							SpellDuration* sd = dbcSpellDuration.LookupEntryForced( spellInfo->m_durationIndex );
							uint32 tickcount = GetDuration( sd ) / spellInfo->m_effectAmplitude[0] ;
							dmg_overwrite = ospinfo->m_effectBasePoints[0] * dmg / (100  * tickcount );
						}break;
						//druid - Primal Fury
						case 37116:
						case 37117:
					...
					case 27285:
						{
							...
							if( flag & PROC_ON_SPELL_HIT_VICTIM )
							{
								...
								if( (int32)(*itr2).procCharges >= ospinfo->m_effectBasePoints[ 1 ] && //if charge built up
						...
						case 18371:
							{
								...
								dmg_overwrite = ( ospinfo->m_effectBasePoints[2] + 1 ) * GetUInt32Value( UNIT_FIELD_MAXPOWER1 ) / 100;
							}break;
						// warlock - Unstable Affliction
						case 43523:
							{
								//null check was made before like 2 times already :P
								dmg_overwrite = ( ospinfo->m_effectBasePoints[0] + 1 ) * 9;
							}break;
						//warlock soul link
						case 25228:
							{
						...
						case 18118:
							{
								if( CastingSpell == NULL )
									continue;//this should not ocur unless we made a fuckup somewhere
								//only trigger effect for specified spells
								skilllinespell* skillability = objmgr.GetSpellSkill(CastingSpell->m_id);
								if( !skillability )
									continue;
						...
						case 30300:
							{
								...
								if(!(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))
									continue;
								if( CastingSpell->m_schoolMask!=SCHOOL_FIRE &&
									CastingSpell->m_schoolMask!=SCHOOL_SHADOW)
									continue;
							}break;
						//warlock - Soul Leech
						...
						case 30294:
							{
								...
								switch( CastingSpell->NameHash )
								{
									...
									{
										amount = CastingSpell->m_effectBasePoints[0]+1;
									}break;
									case SPELL_HASH_SHADOWBURN: //Shadowburn
									{
										amount = CastingSpell->m_effectBasePoints[1]+1;
									}break;
									default:
										amount=0;
								}
								...
								spell->Heal(amount*(ospinfo->m_effectBasePoints[0]+1)/100);
								delete spell;
								continue;
							}break;
						//warlock - pyroclasm
						...
						case 28682:
							{
								...
								//only trigger effect for specified spells
								if( !( CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING)
									|| CastingSpell->m_schoolMask != SCHOOL_FIRE )
									continue;
						...
						case 15258:
							{
								if( CastingSpell == NULL )
									continue;//this should not ocur unless we made a fuckup somewhere
								if( CastingSpell->m_schoolMask != SCHOOL_SHADOW || !( CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING ) ) //we need damaging spells for this, so we suppose all shadow spells casted on target are dmging spells = Wrong
									continue;
							}break;
						//priest - Inspiration
						case 15363:
                        ...
                        case 27818:
                            {
								...
								if (!parentproc || !spellInfo)
									continue;
								int32 val = parentproc->m_effectBasePoints[0] + 1;
                                Spell *spell = new Spell(this, spellInfo ,true, NULL);
						...
						case 25505:
							{
								...
								uint32 AP_owerride=GetAP() + spellInfo->m_effectBasePoints[0]+1;
						...
						case 14181:
							{
								...
								if( CastingSpell->c_is_flags & SPELL_FLAG_IS_FINISHING_MOVE && IsPlayer())
									proc_Chance = static_cast< Player* >( this )->m_comboPoints*ospinfo->m_effectBasePoints[1];
								else continue;
								if(!Rand(proc_Chance))
									continue;
							}break;
						...
						case 15269:
							{
								if( CastingSpell == NULL )
									continue;//this should not ocur unless we made a fuckup somewhere
								if( CastingSpell->m_schoolMask != SCHOOL_SHADOW || !( CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING ) )
									continue;
							}break;
						...
						case 29077:
							{
								if( CastingSpell == NULL )
									continue;
								if( CastingSpell->m_schoolMask!=SCHOOL_FIRE && CastingSpell->m_schoolMask!=SCHOOL_FROST) //fire and frost criticals
									continue;
								dmg_overwrite = CastingSpell->m_manaCost * ( ospinfo->m_effectBasePoints[0] + 1 ) / 100;
							}break;
						...
						case 34720:
							{
								if( CastingSpell == NULL )
									continue;
								dmg_overwrite = CastingSpell->m_manaCost * 40 / 100;
							}break;
						...
						case 33619:
							{
								...
								int tdmg = abs * ( ospinfo->m_effectBasePoints[0] + 1 ) / 100;
						...
						case 31893:
							{
								...
								int32 healthtoloose = ospinfo->m_effectBasePoints[1] * GetUInt32Value( UNIT_FIELD_BASE_HEALTH ) / 100;
						...
						case 25997:
							{
								...
								if(!(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))
									continue;
								dmg_overwrite = ( dmg *  (ospinfo->m_effectBasePoints[0] + 1 )) / 100 ; //only half dmg
						...
						case 31828:
							...
							case 31930:
							{
								...
								dmg_overwrite = spellInfo->m_manaCost / 2 ; //only half dmg
								//printf("is there a seal on the player ? %u \n",dmg_overwrite);
							}break;
						//Energized
						case 43751:
						...
						case 39805:
							{
								...
								{
									spellId = CastingSpell->m_id;
									dmg_overwrite = (CastingSpell->m_effectBasePoints[0] + 1) / 2; //only half dmg
								}
								else continue;
							}break;
						...
						case 37379:
							{
								if (!CastingSpell || CastingSpell->m_schoolMask != SCHOOL_SHADOW || !(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))
									continue;
							}break;
						case 37378:
							{
								if (!CastingSpell || CastingSpell->m_schoolMask != SCHOOL_FIRE || !(CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING))
									continue;
							}break;
						case 39950:
							{
						...
						case 37517:
							{
								if (!CastingSpell || CastingSpell->m_id == 37517 || CastingSpell->NameHash != SPELL_HASH_REVENGE)
									continue; 
							}break;
						//SETBONUSES END
		...
		if(iter2->second.count)
		{
			if((iter2->second.ProcFlag & flag))
			{
				...
				if(CastingSpell)
				{
					...
					SpellCastTime *sd = dbcSpellCastTime.LookupEntry(CastingSpell->m_castingTimeIndex);
					if(!sd) continue; // this shouldnt happen though :P
					//if we did not proc these then we should not remove them
					if( CastingSpell->m_id == iter2->second.spellId)
						continue;
...
//							if( CastingSpell->School!=SCHOOL_NATURE||(!sd->CastTime||sd->CastTime>10000)) continue;
							if( CastingSpell->m_schoolMask!=SCHOOL_NATURE||(sd->CastTime==0)) continue;
						}break;
					case 16166:
						{
							if(!(CastingSpell->m_schoolMask==SCHOOL_FIRE||CastingSpell->m_schoolMask==SCHOOL_FROST||CastingSpell->m_schoolMask==SCHOOL_NATURE))
								continue;
						}break;
...
//		double Reduction = double(pVictim->GetResistance(0)) / double(pVictim->GetResistance(0)+400+(85*getLevel()));
		...
		else if(getLevel() > 59 && getLevel() < 80) Reduction = double(pVictim->GetResistance(0) - ArmorReduce) / double(pVictim->GetResistance(0)-22167.5+(467.5*getLevel()));
		//
	...
	else
	{
		...
		AverageResistance = (float)(RResist) / (float)(getLevel() * 5) * 0.75f;
		  ...
		  if (!ability || !(ability->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
		  {
...
uint32 Unit::GetSpellDidHitResult( Unit* pVictim, uint32 weapon_damage_type, SpellEntry* ability )
{
	...
	if( ability && ability->m_spellClassMask )
	{
		SM_FFValue( SM_FHitchance, &hitchance, ability->m_spellClassMask );
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		float spell_flat_modifers=0;
		SM_FFValue(SM_FHitchance,&spell_flat_modifers,ability->m_spellClassMask);
		if(spell_flat_modifers!=0 )
			printf("!!!!!spell resist mod flat %f,  spell resist bonus %f, spell group %u\n",spell_flat_modifers,hitchance,ability->m_spellClassMask);
#endif
	}
	//==========================================================================================
	//==============================One Roll Processing=========================================
...
//==========================================================================================
	...
	if(ability)
		dmg.school_type = ability->m_schoolMask;
	else
	{
...
//--------------------------------by skill difference---------------------------------------
	...
	if(ability && ability->m_spellClassMask)
	{
		SM_FFValue(SM_CriticalChance,&crit,ability->m_spellClassMask);
		SM_FFValue(SM_FHitchance,&hitchance,ability->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		float spell_flat_modifers=0;
		SM_FFValue(SM_CriticalChance,&spell_flat_modifers,ability->m_spellClassMask);
		if(spell_flat_modifers!=0)
			printf("!!!!spell critchance mod flat %f ,spell group %u\n",spell_flat_modifers,ability->m_spellClassMask);
#endif
	}
//--------------------------------by ratings------------------------------------------------
...
//--------------------------------by damage type and by weapon type-------------------------
	...
	if( ability != NULL && ability->m_attributes & ATTRIBUTES_CANT_BE_DPB )
	{
		dodge = 0.0f;
		parry = 0.0f;
		block = 0.0f;
...
//--------------------------------state proc initialization---------------------------------
			...
			else
			{
				...
				if(ability && ability->m_id==3018 && IsPlayer() && getClass()==HUNTER)
...
//--------------------------------base damage calculation-----------------------------------
			...
			else
			{
				if( weapon_damage_type == MELEE && ability )
					dmg.full_damage = CalculateDamage( this, pVictim, MELEE, ability->m_spellClassMask, ability );
				else			
			...
			if(ability && ability->m_spellClassMask)
			{	
				SM_FIValue(((Unit*)this)->SM_FDamageBonus,&dmg.full_damage,ability->m_spellClassMask);
				SM_PIValue(((Unit*)this)->SM_PDamageBonus,&dmg.full_damage,ability->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
				int spell_flat_modifers=0;
				int spell_pct_modifers=0;
				SM_FIValue(((Unit*)this)->SM_FDamageBonus,&spell_flat_modifers,ability->m_spellClassMask);
				SM_FIValue(((Unit*)this)->SM_PDamageBonus,&spell_pct_modifers,ability->m_spellClassMask);
				if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
					printf("!!!!!spell dmg bonus mod flat %d , spell dmg bonus pct %d , spell dmg bonus %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,dmg.full_damage,ability->m_spellClassMask);
#endif
			}
			...
			if( ability && ability->m_mechanic == MECHANIC_BLEEDING )
				disable_dR = true; 
			...
			case 5:
				{
					...
					if(ability && ability->m_spellClassMask)
					{
						...
						SM_FIValue(SM_PCriticalDamage,&dmg_bonus_pct,ability->m_spellClassMask);
...
//==========================================================================================
	...
	if( !ability )
	{
		...
		if(HITSTATUS_UNK2)		//without this "r" shit I could do this a proper way... should work tho
		{
			data << uint32(0);
		}

		data << (uint8)vstate;			 // new victim state		 // new victim state
		data << (uint32)0x03e8;					// can be 0,1000 or -1
		data << (uint32)0;				  // unknown

		if(HITSTATUS_UNK3) //same as with the UNK2 stuff
		{
			data << uint32(0);			// would like to know wtf this is
		}
	...
	else
	{
		if( realdamage > 0 )//FIXME: add log for miss,block etc for ability and ranged
		{
			...
			SendSpellNonMeleeDamageLog( this, pVictim, ability->m_id, realdamage, dmg.school_type, dmg.resisted_damage, 0, false, blocked_damage, ( ( hit_status & HITSTATUS_CRICTICAL ) != 0 ), true );
		}
	...
	{	
		SendSpellLog(this,pVictim,ability->m_id,SPELL_LOG_RESIST);
	}
//==========================================================================================
//==============================Damage Dealing==============================================
//==========================================================================================
...
{
	if ( m_mapId != 530 || m_mapId != 530 )
	{
		for( uint32 i = 0; i < 3; ++i )
		{
			// Can't use flying auras in non-outlands.
			if( aur->GetSpellProto()->m_effectAura[i] == 208 || aur->GetSpellProto()->m_effectAura[i] == 207 )
			{
				delete aur;
				return;
			}
	...
	if( aur->GetSpellProto()->m_schoolMask && SchoolImmunityList[aur->GetSpellProto()->m_schoolMask] )
	{
		delete aur;
		return;
	}
    ...
    if( !aur->IsPassive() )
	{
		...
		uint32 maxStack = aur->GetSpellProto()->m_cumulativeAura;
		if( aur->GetSpellProto()->m_procCharges > 0 )
			maxStack=aur->GetSpellProto()->m_procCharges;
		...
		if( !info->always_apply )
		{
			...
			for( uint32 x = 0; x < MAX_AURAS; x++ )
			{
				if( m_auras[x] )
				{
					if(	m_auras[x]->GetSpellProto()->m_id != aur->GetSpellId() && 
						( aur->pSpellId != m_auras[x]->GetSpellProto()->m_id ) //if this is a proc spell then it should not remove it's mother : test with combustion later
						)
					{
					...
					else if( m_auras[x]->GetSpellId() == aur->GetSpellId() ) // not the best formula to test this I know, but it works until we find a solution
					{
						...
						//if(maxStack > 1)
						{
							...
							if(maxStack <= 1)
							{
								...
								data << GetNewGUID() << m_auras[x]->m_visualSlot << uint32(m_auras[x]->GetSpellProto()->m_id) << uint32(aur->GetDuration()) << uint32(aur->GetDuration());
	...
	////////////////////////////////////////////////////////
	if(aur->GetSpellProto()->m_spellClassMask && m_objectTypeId == TYPEID_PLAYER)
	{
		int32 speedmod=0;
		SM_FIValue(SM_FSpeedMod,&speedmod,aur->GetSpellProto()->m_spellClassMask);
		if(speedmod)
		{
	...
	{
        if((aur->m_spellProto->m_attributesEx & 1024))
        {
            aur->AddAuraVisual();
        }
		for(uint32 x=MAX_AURAS;x<MAX_AURAS+MAX_PASSIVE_AURAS;x++)
		...
		if(aur->m_auraSlot==255)
		{
			sLog.outError("Aura error in passive aura. removing. SpellId: %u", aur->GetSpellProto()->m_id);
			// for next loop.. lets kill the fucker
			aur->Remove();
			return;
		}
...
void Unit::RemoveNegativeAuras()
{
	for(uint32 x=MAX_POSITIVE_AURAS;x<MAX_AURAS;x++)
	{
		if(m_auras[x])
		{
            if(m_auras[x]->GetSpellProto()->m_attributesExC & CAN_PERSIST_AND_CASTED_WHILE_DEAD)
                continue;
            else
            {
			    m_auras[x]->Remove();
...
void Unit::RemoveAllAuraType(uint32 auratype)
{
    for(uint32 x=0;x<MAX_AURAS;x++)
    {
		if(m_auras[x])
		{
			...
			if(proto->m_effectAura[0]==auratype || proto->m_effectAura[1]==auratype || proto->m_effectAura[2]==auratype)
...
int32 Unit::GetSpellDmgBonus(Unit *pVictim, SpellEntry *spellInfo,int32 base_dmg, bool isdot)
{
	int32 plus_damage = 0;
	Unit* caster = this;
	uint32 school = spellInfo->m_schoolMask;
...
//------------------------------by cast duration--------------------------------------------
	...
	else
	{
		...
		else if( spellInfo->fixed_hotdotcoef >= 0 && isdot )
		{
			...
			{
				int durmod = 0;
				SM_FIValue(caster->SM_FDur, &durmod, spellInfo->m_spellClassMask);
		...
		else
		{
			...
			float td = float( GetDuration( dbcSpellDuration.LookupEntry( spellInfo->m_durationIndex ) ));
	...
	if(spellInfo->m_baseLevel > 0 && spellInfo->m_maxLevel > 0)
	{
		float downrank1 = 1.0f;
		if(spellInfo->m_baseLevel < 20)
		    downrank1 = 1.0f - (20.0f - float (spellInfo->m_baseLevel) ) * 0.0375f;
		float downrank2 = ( float(spellInfo->m_maxLevel + 5.0f) / float(static_cast< Player* >(caster)->getLevel()) );
...
//==========================================================================================
	...
	{
		SM_FIValue(caster->SM_FPenalty, &bonus_damage, spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		int spell_flat_modifers=0;
		int spell_pct_modifers=0;
		SM_FIValue(caster->SM_FPenalty,&spell_flat_modifers,spellInfo->m_spellClassMask);
		SM_FIValue(caster->SM_PPenalty,&spell_pct_modifers,spellInfo->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!spell dmg bonus(p=24) mod flat %d , spell dmg bonus(p=24) pct %d , spell dmg bonus %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,bonus_damage,spellInfo->m_spellClassMask);
#endif
		SM_FIValue(caster->SM_FDamageBonus, &bonus_damage, spellInfo->m_spellClassMask);
		int dmg_bonus_pct=0;
		SM_FIValue(caster->SM_PPenalty, &dmg_bonus_pct, spellInfo->m_spellClassMask);		
		SM_FIValue(caster->SM_PDamageBonus,&dmg_bonus_pct,spellInfo->m_spellClassMask);
		bonus_damage += base_dmg*dmg_bonus_pct/100;
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		spell_flat_modifers=0;
		spell_pct_modifers=0;
		SM_FIValue(caster->SM_FDamageBonus,&spell_flat_modifers,spellInfo->m_spellClassMask);
		SM_FIValue(caster->SM_PDamageBonus,&spell_pct_modifers,spellInfo->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!spell dmg bonus mod flat %d , spell dmg bonus pct %d , spell dmg bonus %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,bonus_damage,spellInfo->m_spellClassMask);
#endif
	}
//------------------------------by school----------------------------------------------
	...
	summaryPCTmod += pVictim->ModDamageTakenByMechPCT[spellInfo->m_mechanic];
...
void Unit::RemoveAllAreaAuras()
{
	...
	{
		it1 = itr++;
		if(((*it1)->m_spellProto->m_effect[0] == SPELL_EFFECT_APPLY_AREA_AURA ||
			(*it1)->m_spellProto->m_effect[1] == SPELL_EFFECT_APPLY_AREA_AURA ||
			(*it1)->m_spellProto->m_effect[2] == SPELL_EFFECT_APPLY_AREA_AURA) && (*it1)->GetCaster() != this)
		{
			(*it1)->Remove();
			m_auras.erase(it1);
		} 
...
void Unit::RemoveAurasByInterruptFlag(uint32 flag)
{
	...
	for(uint32 x=0;x<MAX_AURAS;x++)
	{
		...
		if((a->m_spellProto->m_auraInterruptFlags & flag) && !(a->m_spellProto->m_procTypeMask & PROC_REMOVEONUSE))
		{
			a->Remove();
			m_auras[x] = NULL;
		}
...
{
	for(uint32 x=0;x<MAX_AURAS+MAX_PASSIVE_AURAS;x++)
	if(m_auras[x] && m_auras[x]->GetSpellProto()->m_spellVisualID==visualid)
	{
		return true;
	}
...
void Unit::DropAurasOnDeath()
{
	for(uint32 x=0;x<MAX_AURAS;x++)
    {
        if(m_auras[x])
        {
            if(m_auras[x] && m_auras[x]->GetSpellProto()->m_attributesExC & CAN_PERSIST_AND_CASTED_WHILE_DEAD)
                continue;
            else
	        {
		        m_auras[x]->Remove();
...
void Unit::CastSpell(Unit* Target, SpellEntry* Sp, bool triggered)
{
	...
	if(IsPlayer())
	{
		SpellEntry *spellInfo = dbcSpell.LookupEntryForced(Sp->m_id);
		if(!spellInfo || !sHookInterface.OnCastSpell(static_cast<Player*>(this), spellInfo))
		{
			sLog.outError("Unit.cpp(4655): unknown spell id %i\n", Sp->m_id);
			return;
		}
		else
			sLog.outError("WORLD: player or gtfo");
	}
...
void Unit::CastSpell(Unit* Target, uint32 SpellID, bool triggered)
{
	...
	if(IsPlayer())
	{
		...
		if(!spellInfo || !sHookInterface.OnCastSpell(static_cast<Player*>(this), spellInfo))
		{
			sLog.outError("Unit.cpp(4686): unknown spell id %i\n", SpellID);
			return;
		}
		else
			sLog.outError("WORLD: player or gtfo");
...
void Unit::CastSpell(uint64 targetGuid, SpellEntry* Sp, bool triggered)
{
	...
	if(IsPlayer())
	{
		...
		if(!spellInfo || !sHookInterface.OnCastSpell(static_cast<Player*>(this), spellInfo))
		{
			sLog.outError("Unit.cpp(4706): unknown spell id %i\n", Sp->m_id);
			return;
		}
		else
			sLog.outError("WORLD: player or gtfo");
...
//		if(m_auras[x] && m_auras[x]->GetSpellProto()->NameHash == name_hash && m_auras[x]->GetCaster()==caster)
		if(m_auras[x] && m_auras[x]->GetSpellProto()->NameHash == name_hash)
		{
			...
			resp.Misc = m_auras[x]->GetSpellProto()->m_id;
...
//	if(aur->GetSpellProto()->NameHash == name_hash && aur->GetCaster()==caster)
	...
	{
		// we've got an aura with the same name as the one we're trying to apply
		resp.Misc = aur->GetSpellProto()->m_id;
...
void Unit::RemoveAurasByInterruptFlagButSkip(uint32 flag, uint32 skip)
{
	...
	for(uint32 x=0;x<MAX_AURAS;x++)
	{
		...
		//some spells do not get removed all the time only at specific intervals
		if((a->m_spellProto->m_auraInterruptFlags & flag) && (a->m_spellProto->m_id != skip) && a->m_spellProto->proc_interval==0)
		{
			//the black sheeps of sociaty
			if(a->m_spellProto->m_auraInterruptFlags & AURA_INTERRUPT_ON_CAST_SPELL)
			{
				switch(a->GetSpellProto()->m_id)
				{
...
int Unit::HasAurasWithNameHash(uint32 name_hash)
{
	...
	{
		if(m_auras[x] && m_auras[x]->GetSpellProto()->NameHash == name_hash)
			return m_auras[x]->m_spellProto->m_id;
	}
...
bool Unit::HasNegativeAura(uint32 spell_id)
{
	for(uint32 x = MAX_POSITIVE_AURAS; x < MAX_AURAS; ++x)
	{
		if(m_auras[x] && m_auras[x]->GetSpellProto()->m_id == spell_id)
			return true;
	}
...
void Unit::RemoveAurasOfSchool(uint32 School, bool Positive, bool Immune)
{
	for(uint32 x = 0; x < MAX_AURAS; ++x)
	{
		if(m_auras[x]  && m_auras[x]->GetSpellProto()->m_schoolMask == School && (!m_auras[x]->IsPositive() || Positive) && (!Immune && m_auras[x]->GetSpellProto()->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
			m_auras[x]->Remove();
	}
}
...
bool Unit::IsDazed()
{
	for(uint32 x = 0; x < MAX_AURAS; ++x)
	{
		if(m_auras[x])
		{
			if(m_auras[x]->GetSpellProto()->m_mechanic == MECHANIC_ENSNARED)
			return true;
			for(uint32 y=0;y<3;y++)
			if(m_auras[x]->GetSpellProto()->m_effectMechanic[y]==MECHANIC_ENSNARED)
				return true;
		}
	}
...
void CombatStatusHandler::ClearHealers()
{
	...
	if( m_Unit->IsInWorld() && m_Unit->GetMapMgr() )
	...
	for(uint32 x = ( HostileOnly ? MAX_POSITIVE_AURAS : 0 ) ; x < MAX_AURAS ; x++ ) // If HostileOnly = 1, then we use aura slots 40-56 (hostile). Otherwise, we use 0-56 (all)
		{
			...
			if( m_auras[x] )
			{
					if( m_auras[x]->GetSpellProto()->m_mechanic == MechanicType ) // Remove all mechanics of type MechanicType (my english goen boom)
					{
					...
					else if( MechanicType == MECHANIC_ENSNARED ) // if got immunity for slow, remove some that are not in the mechanics
					{
						...
						{
							// SNARE + ROOT
							if( m_auras[x]->GetSpellProto()->m_effectAura[i] == SPELL_AURA_MOD_DECREASE_SPEED || m_auras[x]->GetSpellProto()->m_effectAura[i] == SPELL_AURA_MOD_ROOT )
							{
								m_auras[x]->Remove();
								break;
							}
