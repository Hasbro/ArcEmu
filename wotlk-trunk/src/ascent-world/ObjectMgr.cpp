...
void ObjectMgr::LoadPlayerCreateInfo()
{
	...
	do
	{
		...
		pPlayerCreateInfo->index = fields[0].GetUInt8() + ind_shift;
	...
	if(result)
	{
		...
		do
		{
			...
			if( sf_spellId )
			{
				...
				{
					if( sf_procFlags )
						sp->m_procTypeMask = sf_procFlags;

					if( sf_SpellGroupType )
						sp->m_spellClassMask = sf_SpellGroupType;

					if( sf_procChance )
						sp->m_procChance = sf_procChance;

					if ( sf_procCharges )
						sp->m_procCharges = sf_procCharges;
				}
			}
		}while(result->NextRow());
		delete result;
	...
	if(result)
	{
		do
		{
			...
			if( seo_SpellId )
			{
				...
				{
					if( seo_Disable )
						sp->m_effect[seo_EffectId] = 0;

					if( seo_Effect )
						sp->m_effect[seo_EffectId] = seo_Effect;

					if( seo_BasePoints )
						sp->m_effectBasePoints[seo_EffectId] = seo_BasePoints;

					if( seo_ApplyAuraName )
						sp->m_effectAura[seo_EffectId] = seo_ApplyAuraName;

					if( seo_SpellGroupRelation )
						sp->m_effectItemType[seo_EffectId] = seo_SpellGroupRelation;

					if( seo_MiscValue )
						sp->m_effectMiscValue[seo_EffectId] = seo_MiscValue;

					if( seo_TriggerSpell )
						sp->m_effectTriggerSpell[seo_EffectId] = seo_TriggerSpell;
					
					if( seo_ImplicitTargetA )
						sp->m_implicitTargetA[seo_EffectId] = seo_ImplicitTargetA;

					if( seo_ImplicitTargetB )
						sp->m_implicitTargetB[seo_EffectId] = seo_ImplicitTargetB;
				}
			}
			...
			do
			{
				...
				if( CastSpellID != 0 )
				{
					...
					if( ts.pCastSpell )
					{
						for(int k=0;k<3;k++)
						{
							if(ts.pCastSpell->m_effect[k]==SPELL_EFFECT_LEARN_SPELL)
							{
								ts.pCastRealSpell = dbcSpell.LookupEntryForced(ts.pCastSpell->m_effectTriggerSpell[k]);
...
uint32 ObjectMgr::GetPetSpellCooldown(uint32 SpellId)
{
	...
	return sp->m_categoryRecoveryTime + sp->m_startRecoveryTime;
}
