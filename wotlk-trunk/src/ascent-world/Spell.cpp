...
bool Spell::IsStealthSpell()
{
	...
	if( m_spellInfo->m_effectAura[0] == 16 ||
		m_spellInfo->m_effectAura[1] == 16 ||
		m_spellInfo->m_effectAura[2] == 16 )
		return true;
	return false;
}
...
bool Spell::IsInvisibilitySpell()
{
	...
	if( m_spellInfo->m_effectAura[0] == 18 ||
		m_spellInfo->m_effectAura[1] == 18 ||
		m_spellInfo->m_effectAura[2] == 18 )
		return true;
	return false;
}
...
void Spell::FillSpecifiedTargetsInArea(uint32 i,float srcx,float srcy,float srcz, float range, uint32 specification)
{
    ...
    for(std::set<Object*>::iterator itr = m_caster->GetInRangeSetBegin(); itr != m_caster->GetInRangeSetEnd(); itr++ )
    {
        ...
        if( m_spellInfo->m_targetCreatureType)
        {
            if((*itr)->GetTypeId()!= TYPEID_UNIT)
                continue;
            CreatureInfo *inf = ((Creature*)(*itr))->GetCreatureName();
            if(!inf || !(1<<(inf->Type-1) & m_spellInfo->m_targetCreatureType))
                continue;
        }
        ...
        if(IsInrange(srcx,srcy,srcz,(*itr),r))
        {
            ...
            if( m_spellInfo->m_maxTargets)
            {
                if( m_spellInfo->m_maxTargets >= tmpMap->size())
                    return;
            }
        }
    }
...
uint64 Spell::GetSinglePossibleEnemy(uint32 i,float prange)
{
	...
	else
	{
		...
		if( m_spellInfo->m_spellClassMask && u_caster)
		{
			SM_FFValue(u_caster->SM_FRadius,&r,m_spellInfo->m_spellClassMask);
			SM_PFValue(u_caster->SM_PRadius,&r,m_spellInfo->m_spellClassMask);
		}
	}
...
uint8 Spell::DidHit(uint32 effindex,Unit* target)
{
	...
	/**************************************
	* Of course we'll have dummy effects. *
	**************************************/
	if( u_victim )
	{
		switch( m_spellInfo->m_id )
		{
			case 48721:
			case 49939:
			case 49949:
			...
			bool has_disease = false;
				...
				{
					if( u_victim->m_auras[x] && u_victim->m_auras[x]->m_spellProto->m_dispelType == DISPEL_DISEASE )
					{
						has_disease = true;
						break;
					}
	...
	/************************************************************************/
	if(u_victim->SchoolImmunityList[m_spellInfo->m_schoolMask])
		return SPELL_DID_HIT_IMMUNE;
	
	/*************************************************************************/
	/* Check if the target is immune to this mechanic                        */
	/*************************************************************************/
	if(u_victim->MechanicsDispels[m_spellInfo->m_mechanic])
	{
	...
	for( int i = 0 ; i <= 2 ; i ++ )
		{
			if( u_victim->MechanicsDispels[MECHANIC_ROOTED] && m_spellInfo->m_effectAura[i] == SPELL_AURA_MOD_ROOT )
				return SPELL_DID_HIT_IMMUNE;
			if( u_victim->MechanicsDispels[MECHANIC_ENSNARED] && m_spellInfo->m_effectAura[i] == SPELL_AURA_MOD_DECREASE_SPEED )
				return SPELL_DID_HIT_IMMUNE;
		}
	...
	if( m_spellInfo->is_melee_spell || m_spellInfo->is_ranged_spell )
	{
		...
		{
			if (m_spellInfo->m_attributesExC & 0x1000000)
				_type =  OFFHAND;
			else
				_type = MELEE;
		}
	...
	/************************************************************************/
	if( m_spellInfo->m_schoolMask==0  || m_spellInfo->is_ranged_spell ) // all ranged spells are physical too...
		return SPELL_DID_HIT_SUCCESS;
	...
	if( m_spellInfo->m_mechanic<27)
	{
		if(p_victim)
			resistchance += p_victim->MechanicsResistancesPCT[m_spellInfo->m_mechanic];
		else 
			resistchance += u_victim->MechanicsResistancesPCT[m_spellInfo->m_mechanic];
	}
	//rating bonus
	if( p_caster != NULL )
	{
	...
	if( this->m_spellInfo->m_effect[effindex] == SPELL_EFFECT_DISPEL && m_spellInfo->m_spellClassMask && u_caster)
	{
		SM_FFValue(u_caster->SM_FRezist_dispell,&resistchance,m_spellInfo->m_spellClassMask);
		SM_PFValue(u_caster->SM_PRezist_dispell,&resistchance,m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		int spell_flat_modifers=0;
		int spell_pct_modifers=0;
		SM_FIValue(u_caster->SM_FRezist_dispell,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
		SM_FIValue(u_caster->SM_PRezist_dispell,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!spell dipell resist mod flat %d , spell dipell resist mod pct %d , spell dipell resist %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,resistchance,m_spellInfo->m_spellClassMask);
#endif
	}
	...
	{
		float hitchance=0;
		SM_FFValue(u_caster->SM_FHitchance,&hitchance,m_spellInfo->m_spellClassMask);
		resistchance -= hitchance;
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		float spell_flat_modifers=0;
		SM_FFValue(u_caster->SM_FHitchance,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
		if(spell_flat_modifers!=0 )
			printf("!!!!!spell to hit mod flat %f, spell resist chance %f, spell group %u\n",spell_flat_modifers,resistchance,m_spellInfo->m_spellClassMask);
#endif
	}

	if (m_spellInfo->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY)
		resistchance = 0.0f;
...
void Spell::GenerateTargets(SpellCastTargets *store_buff)
{
	...
	for(uint32 i=0;i<3;i++)
		...
		{
			if(j==0)
				cur = m_spellInfo->m_implicitTargetA[i];
			else // if(j==1)
				cur = m_spellInfo->m_implicitTargetB[i];		
			switch(cur)
			{
				...
				case EFF_TARGET_TOTEM_FIRE:// Totem
					{
						if( p_caster != NULL )
						{
							uint32 slot = m_spellInfo->m_effect[i] - SPELL_EFFECT_SUMMON_TOTEM_SLOT1;
...
uint8 Spell::prepare( SpellCastTargets * targets )
{
	...
	{
		m_castTime = GetCastTime( dbcSpellCastTime.LookupEntry( m_spellInfo->m_castingTimeIndex ) );

		if( m_castTime && m_spellInfo->m_spellClassMask && u_caster != NULL )
		{
			SM_FIValue( u_caster->SM_FCastTime, (int32*)&m_castTime, m_spellInfo->m_spellClassMask );
			SM_PIValue( u_caster->SM_PCastTime, (int32*)&m_castTime, m_spellInfo->m_spellClassMask );
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
			int spell_flat_modifers=0;
			int spell_pct_modifers=0;
			SM_FIValue(u_caster->SM_FCastTime,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
			SM_FIValue(u_caster->SM_PCastTime,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
			if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
				printf("!!!!!spell casttime mod flat %d , spell casttime mod pct %d , spell casttime %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,m_castTime,m_spellInfo->m_spellClassMask);
#endif
		}
	...
	if( !m_triggeredSpell && p_caster != NULL && p_caster->CooldownCheat )
		p_caster->ClearCooldownForSpell( m_spellInfo->m_id );
	...
	else
	{
		...
		// aura state removal
		if( m_spellInfo->m_casterAuraState )
			u_caster->RemoveFlag( UNIT_FIELD_AURASTATE, m_spellInfo->m_casterAuraState);
	}

	//instant cast(or triggered) and not channeling
	if( u_caster != NULL && ( m_castTime > 0 || m_spellInfo->m_channelInterruptFlags ) && !m_triggeredSpell )	
	{
...
void Spell::cancel()
{
	...
	{
		if( u_caster != NULL )
			u_caster->RemoveAura(m_spellInfo->m_id);
		...
		if(m_timer > 0 || m_Delayed)
		{
			if(p_caster && p_caster->IsInWorld())
			{
				...
				if(pTarget)
				{
					pTarget->RemoveAura(m_spellInfo->m_id, m_caster->GetGUID());
				}
...
void Spell::cast(bool check)
{
	...
	sLog.outDebug("Spell::cast %u, Unit: %u", m_spellInfo->m_id, m_caster->GetLowGUID());
	...
	if(cancastresult == SPELL_CANCAST_OK)
	{
		...
		{
			sQuestMgr.OnPlayerCast(p_caster,m_spellInfo->m_id,m_targets.m_unitTarget);
		}
		if( m_spellInfo->m_attributes & ATTRIBUTE_ON_NEXT_ATTACK)
		{
			if(!m_triggeredSpell)
			{
		...
		for(uint32 i=0;i<3;i++)
        {
			if( m_spellInfo->m_effect[i] && m_spellInfo->m_effect[i] != SPELL_EFFECT_PERSISTENT_AREA_AURA)
				 FillTargetMap(i);
        }
		...
		if( p_caster )
		{
			...
			if( p_caster->IsStealth() && !(m_spellInfo->m_attributesEx & ATTRIBUTESEX_NOT_BREAK_STEALTH) )
			{
				/* talents procing - don't remove stealth either */
				if (!(m_spellInfo->m_attributes & ATTRIBUTES_PASSIVE) && 
					!( pSpellId && dbcSpell.LookupEntry(pSpellId)->m_attributes & ATTRIBUTES_PASSIVE ) )
				{
		...
		/*SpellExtraInfo* sp = objmgr.GetSpellExtraData(m_spellInfo->m_id);
		if(sp)
		{
		...
		if(!(m_spellInfo->m_attributes & ATTRIBUTE_ON_NEXT_ATTACK  && !m_triggeredSpell))//on next attack
		{
			SendSpellGo();
			...
			if (m_spellInfo->m_attributesExC & 0x8000 && m_caster->IsPlayer() && m_caster->IsInWorld())
			{
                /// Part of this function contains a hack fix
                /// hack fix for shoot spells, should be some other resource for it
                //p_caster->SendSpellCoolDown(m_spellInfo->m_id, m_spellInfo->RecoveryTime ? m_spellInfo->RecoveryTime : 2300);
				WorldPacket data(SMSG_SPELL_COOLDOWN, 14);
				data << m_spellInfo->m_id;
				data << p_caster->GetNewGUID();
				data << uint32(m_spellInfo->m_recoveryTime ? m_spellInfo->m_recoveryTime : 2300);
				p_caster->GetSession()->SendPacket(&data);
			}
			else
			{			
				if( m_spellInfo->m_channelInterruptFlags != 0 && !m_triggeredSpell)
				{
					/*
			...
			for(int j=0;j<3;j++)
			{
				switch(m_spellInfo->m_implicitTargetA[j])
				{
					case 6:
					case 22:
					case 24:
			...
			//uint32 spellid = m_spellInfo->m_id;
			...
			if(u_caster && !m_triggeredSpell && !m_triggeredByAura)
				u_caster->RemoveAurasByInterruptFlagButSkip(AURA_INTERRUPT_ON_CAST_SPELL, m_spellInfo->m_id);
			...
			if(!IsReflected())
			{
				for(uint32 x=0;x<3;x++)
				{
					...
					if( m_spellInfo->m_effect[x])
					{
						isDuelEffect = isDuelEffect ||  m_spellInfo->m_effect[x] == SPELL_EFFECT_DUEL;
						if( m_spellInfo->m_effect[x] == SPELL_EFFECT_PERSISTENT_AREA_AURA)
                        {
						...
						else if( m_spellInfo->m_effect[x] == SPELL_EFFECT_TELEPORT_UNITS)
                        {
							HandleEffects(m_caster->GetGUID(),x);
                        }
						else if( m_spellInfo->m_effect[x] == SPELL_EFFECT_SUMMON_WILD)
                        {
							HandleEffects(m_caster->GetGUID(),x);
                        }
					}
				...
				if( m_spellInfo->m_effectAura[0] != 0 || m_spellInfo->m_effectAura[1] != 0 ||
				   m_spellInfo->m_effectAura[2] != 0)
				{
				...
				if(p_caster && p_caster->IsInWorld())
				{
					for(i= UniqueTargets.begin();i != UniqueTargets.end();i++)
					{
						...
						Target->RemoveFlag(UNIT_FIELD_AURASTATE,m_spellInfo->m_casterAuraState);
					}
				}
			}
		...
		else //this shit has nothing to do with instant, this only means it will be on NEXT melee hit
		{
			...
			SendCastResult(cancastresult);
			if( u_caster != NULL )  
				u_caster->SetOnMeleeSpell(m_spellInfo->m_id);
...
void Spell::AddTime(uint32 type)
{
	...
	{
		if( m_spellInfo->m_interruptFlags & CAST_INTERRUPT_ON_DAMAGE_TAKEN)
		{
			cancel();
			return;
		}
		...
		{
			float ch=0;
			SM_FFValue(u_caster->SM_PNonInterrupt,&ch,m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
			float spell_pct_modifers=0;
			SM_FFValue(u_caster->SM_PNonInterrupt,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
			if(spell_pct_modifers!=0)
				printf("!!!!!spell interrupt chance mod pct %f , uninterrupt chance %f, spell group %u\n",spell_pct_modifers,ch,m_spellInfo->m_spellClassMask);
#endif
			if(Rand(ch))
				return;
		}
...
//				sEventMgr.ModifyEventTimeLeft(p_caster,EVENT_ATTACK_TIMEOUT,PLAYER_ATTACK_TIMEOUT_INTERVAL,true);
		...
		else if( m_spellInfo->m_channelInterruptFlags != 48140)
		{
...
//		float(m_castTime)/float(m_timer) >= 2.0f		&&
	...
	{	
		if( u_caster != NULL )
		{
			if(u_caster->HasNoInterrupt() == 0 && m_spellInfo->m_effectMechanic[1] != 14)
			{
				cancel();
				return;
			}
...
void Spell::finish()
{
	...
	if( u_caster != NULL )
	{
		...
		if(m_usesMana && (m_spellInfo->m_channelInterruptFlags == 0 && !m_triggeredSpell) && u_caster->GetPowerType()==POWER_TYPE_MANA)
		{
			/*
			Five Second Rule
		...
		// Death KNight: deplete all Runic Power. This is done here because TakePower() is done very early in the function.
		if( !GetSpellFailed() && cancastresult == SPELL_CANCAST_OK && m_spellInfo->m_powerType == POWER_TYPE_RUNIC_POWER )
		{
			u_caster->SetUInt32Value( UNIT_FIELD_POWER7 , 0 );
		}
	}
	...
	if( p_caster != NULL )
	{
		if (m_spellInfo->m_attributes & ATTRIBUTES_STOP_ATTACK)
		{
	...
	if( m_spellInfo->m_effect[0] == SPELL_EFFECT_SUMMON_OBJECT ||
		m_spellInfo->m_effect[1] == SPELL_EFFECT_SUMMON_OBJECT ||
		m_spellInfo->m_effect[2] == SPELL_EFFECT_SUMMON_OBJECT)
		if( p_caster != NULL )
			p_caster->SetSummonedObject(NULL);		
	/*
	...
	*/
	...
	if( u_caster != NULL )
	{
		if(!m_triggeredSpell && (m_spellInfo->m_channelInterruptFlags || m_castTime>0))
			u_caster->SetCurrentSpell(NULL);
...
void Spell::SendCastResult(uint8 result)
{
	...
	case SPELL_FAILED_REQUIRES_SPELL_FOCUS:
		Extra = m_spellInfo->m_requiresSpellFocus;
		break;

	case SPELL_FAILED_REQUIRES_AREA:
		Extra = m_spellInfo->m_requiredAreaID;
		break;

	case SPELL_FAILED_TOTEMS:
		Extra = m_spellInfo->m_totem[1] ? m_spellInfo->m_totem[1] : m_spellInfo->m_totem[0];
		break;

	//case SPELL_FAILED_TOTEM_CATEGORY: seems to be fully client sided.
	}

	//plr->SendCastResult(m_spellInfo->m_id, result, extra_cast_number, Extra);
	if( Extra )
	{
		packetSMSG_CASTRESULT_EXTRA pe;
		pe.SpellId = m_spellInfo->m_id;
...
void Spell::SendSpellStart()
{
	...
	if( !m_caster->IsInWorld() || m_spellInfo->m_attributes & 64 || m_triggeredSpell )
		return;
    ...
    // hacky yeaaaa
    if( m_spellInfo->m_id == 8326 ) // death
        cast_flags = 0x0F;
	...
	if( GetType() == SPELL_DMG_TYPE_RANGED )
	{
        ...
        if( m_spellInfo->m_id == SPELL_RANGED_THROW ) // throw
        {
			if( p_caster != NULL )
			{
        ...
        else if( m_spellInfo->m_attributesExC & FLAGS4_PLAYER_RANGED_SPELLS )
        {
			if( p_caster != NULL )
...
void Spell::SendSpellGo()
{
    ...
    // no need to send this on passive spells
    if( !m_caster->IsInWorld() || m_spellInfo->m_attributes & 64 )
        return;
	...
	{
		ItemPrototype* ip = NULL;
		if( m_spellInfo->m_id == SPELL_RANGED_THROW )
		{
			if( p_caster != NULL )
			{
	    ...
	    if (CAST_FLAG_UNKNOWN4)                // unknown wotlk
    {
        data << float(0);
        data << uint32(0);
    }
    ...
    if (CAST_FLAG_UNKNOWN5)                    // unknown wotlk
    {
        data << uint32(0);
        data << uint32(0);
    }
...
void Spell::SendChannelStart(uint32 duration)
{
	...
	if( u_caster != NULL )
		u_caster->SetUInt32Value(UNIT_CHANNEL_SPELL,m_spellInfo->m_id);
...
bool Spell::HasPower()
{
	...
	switch(m_spellInfo->m_powerType)
	{
		...
		default:{
		sLog.outDebug("unknown power type %u", m_spellInfo->m_powerType);
	...
	if( u_caster != NULL )
	{
		if( m_spellInfo->m_attributesEx & 2 ) // Uses %100 mana
		{
			m_caster->SetUInt32Value(powerField, 0);
			return true;
		}
	}
	// Death Knight: Not enough runes
	if( p_caster && m_spellInfo->m_powerType == POWER_TYPE_RUNES )
	{
	// SUPA: Death Knight Runic Power is gained from casting spells which cost Runes. The amount of runic power 
	// to be gained is in the DBCs: SpellRuneCost::RunePowerGain.
		SpellRuneCost * src;
		src = dbcSpellRuneCost.LookupEntryForced( m_spellInfo->m_runeCostID );
		if( src )
		{
			uint8 num_runes[3] = { 0 , 0 , 0 };
			for( uint8 i = 0 ; i <= 5 ; i ++ )
			{
				// take away the runes too
				if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 0 )
					num_runes[0] ++; // blood
				else if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 1 )
					num_runes[1] ++; // unholy
				else if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 2 )
					num_runes[2] ++; // frost
				if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 3 ) // death: counts as Frost, Blood, and Unholy rune
				{
					num_runes[0] ++;
					num_runes[1] ++;
					num_runes[2] ++;
			...
			// how did we get here if CanCast will block it? Well, safety first.
			if( src->m_BloodRuneCost > num_runes[0] )
			return false; // temporarily +3 until SPELL_FAILED enum is updated.
			if( src->m_UnholyRuneCost > num_runes[1] )
			return false;
			if( src->m_FrostRuneCost > num_runes[2] )
			return false;
		}
	}
	...
	int32 cost;
	if( m_spellInfo->m_manaCostPct)//Percentage spells cost % of !!!BASE!!! mana
	{
		if( m_spellInfo->m_powerType==POWER_TYPE_MANA)
			cost = (m_caster->GetUInt32Value(UNIT_FIELD_BASE_MANA)*m_spellInfo->m_manaCostPct)/100;
		else
			cost = (m_caster->GetUInt32Value(UNIT_FIELD_BASE_HEALTH)*m_spellInfo->m_manaCostPct)/100;
	}
	else 
	{
		cost = m_spellInfo->m_manaCost;
	}

	if((int32)m_spellInfo->m_powerType==POWER_TYPE_HEALTH)
		cost -= m_spellInfo->m_baseLevel;//FIX for life tap	
	else if( u_caster != NULL )
	{
		if( m_spellInfo->m_powerType==POWER_TYPE_MANA)
			cost += u_caster->PowerCostMod[m_spellInfo->m_schoolMask];//this is not percent!
		else
			cost += u_caster->PowerCostMod[0];
		cost +=float2int32(cost*u_caster->GetFloatValue(UNIT_FIELD_POWER_COST_MULTIPLIER+m_spellInfo->m_schoolMask));
	}

	//apply modifiers
	if( m_spellInfo->m_spellClassMask && u_caster)
	{
		SM_FIValue(u_caster->SM_FCost,&cost,m_spellInfo->m_spellClassMask);
		SM_PIValue(u_caster->SM_PCost,&cost,m_spellInfo->m_spellClassMask);
	}
...
bool Spell::TakePower()
{
	...
	switch(m_spellInfo->m_powerType)
	{
		...
		case POWER_TYPE_RUNES:{ // omg, how will this be handled
			powerField = UNIT_FIELD_POWER6;
			   }break;
		case POWER_TYPE_RUNIC_POWER:{
			powerField = UNIT_FIELD_POWER7;
			   }break;
		//case POWER_TYPE_UNHOLY:{
			//powerField = UNIT_FIELD_POWER6;
							  // }break;
		//case POWER_TYPE_FROST:{
			//powerField = UNIT_FIELD_POWER7;
							  //}break;
		default:{
			sLog.outDebug("unknown power type %u", m_spellInfo->m_powerType);
	...
	if( u_caster != NULL )
	{
		if( m_spellInfo->m_attributesEx & ATTRIBUTESEX_DRAIN_WHOLE_MANA ) // Uses %100 mana
		{
			m_caster->SetUInt32Value(powerField, 0);
			return true;
		}
	...
	if( p_caster && m_spellInfo->m_powerType == POWER_TYPE_RUNES )
	{
	// RUNES: Hackfix, incorrect implementation, but without enough client-side info this is the only way.
	...
	// to be gained is in the DBCs: SpellRuneCost::RunePowerGain.
		...
		if( src )
		{
			uint8 num_runes[3] = { 0 , 0 , 0 }, cumulative_depletion[3] = { 0 , 0 , 0 };
			cumulative_depletion[0] = src->m_BloodRuneCost;
			cumulative_depletion[1] = src->m_UnholyRuneCost;
			cumulative_depletion[2] = src->m_FrostRuneCost;
			uint32 rune_update_time;
			rune_update_time = 10000; // this can be modified by talents.
			for( uint8 i = 0 ; i <= 5 ; i ++ )
			{
				// take away the runes too
				if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 0 )
				{
					if( cumulative_depletion[0] )
					{
						cumulative_depletion[0] --;
						p_caster->SetUInt32Value( UNIT_FIELD_POWER6 + i , 4 ); // exhausted
						sEventMgr.AddEvent(p_caster, &Player::RestoreRune, i , EVENT_RESTORE_RUNE, rune_update_time, 1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
					}
					num_runes[0] ++; // blood
				}
				else if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 1 )
				{
					if( cumulative_depletion[1] )
					{
				...
				{
					if( cumulative_depletion[2] )
					{
						cumulative_depletion[2] --;
						p_caster->SetUInt32Value( UNIT_FIELD_POWER6 + i , 4 ); // exhausted
						sEventMgr.AddEvent(p_caster, &Player::RestoreRune, i , EVENT_RESTORE_RUNE, rune_update_time, 1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
					}
					num_runes[2] ++; // frost
				...
				if( p_caster->GetUInt32Value( UNIT_FIELD_POWER6 + i ) == 3 ) // death: counts as Frost, Blood, and Unholy rune
				{
					...
					num_runes[1] ++;
					num_runes[2] ++;
					for( i = 2 ; i >= 0 ; i -- ) // reverse iteration over rune types, so we use a Death Rune using a Frost slot, then Unholy, then Blood.
					{
						if( cumulative_depletion[i] )
						{
							cumulative_depletion[i] --;
							p_caster->SetUInt32Value( UNIT_FIELD_POWER6 + i , 4 ); // gone!
							sEventMgr.AddEvent(p_caster, &Player::RestoreRune, i , EVENT_RESTORE_RUNE, rune_update_time, 1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
						}
					}
				}
			...
			//Log.Notice( "Rune Power" , "Found the rune power index %u, costs %u blood, %u unholy, %u frost runes, gives %u runic power." , src->m_id , src->m_BloodRuneCost , src->m_UnholyRuneCost , src->m_FrostRuneCost , src->RunePowerGain );
			p_caster->ModUnsigned32Value( UNIT_FIELD_POWER7 , src->RunePowerGain );
			if( p_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) > p_caster->GetUInt32Value( UNIT_FIELD_MAXPOWER7 ) )
				p_caster->SetUInt32Value( UNIT_FIELD_POWER7 , p_caster->GetUInt32Value( UNIT_FIELD_MAXPOWER7 ) ); // cap at max
		}
	...
	int spell_flat_modifers=0;
	int spell_pct_modifers=0;
	SM_FIValue(u_caster->SM_FCost,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
	SM_FIValue(u_caster->SM_PCost,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
	if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
		printf("!!!!!spell cost mod flat %d , spell cost mod pct %d , spell dmg %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,cost,m_spellInfo->m_spellClassMask);
#endif
...
{   
	...
	sLog.outDebug( "WORLD: Spell effect id = %u, damage = %d", m_spellInfo->m_effect[i], damage); 
	
	if( m_spellInfo->m_effect[i]<TOTAL_SPELL_EFFECTS)
	{
		...
		(*this.*SpellEffectsHandler[m_spellInfo->m_effect[i]])(i);
	}
	else
		sLog.outError("SPELL: unknown effect %u spellid %u",m_spellInfo->m_effect[i], m_spellInfo->m_id);
}
...
void Spell::HandleAddAura(uint64 guid)
{
	...
	if( m_spellInfo->buffType > 0)
		Target->RemoveAurasByBuffType(m_spellInfo->buffType, m_caster->GetGUID(),m_spellInfo->m_id);

	uint32 spellid = 0;

	if( m_spellInfo->m_mechanic == 25 && m_spellInfo->m_id != 25771) // Cast spell Forbearance
		spellid = 25771;
	else if( m_spellInfo->m_mechanic == 16 && m_spellInfo->m_id != 11196) // Cast spell Recently Bandaged
		spellid = 11196;
	else if( m_spellInfo->m_mechanic == 19 && m_spellInfo->m_id != 6788) // Cast spell Weakened Soul
		spellid = 6788;
	else if( m_spellInfo->m_id == 45438) // Cast spell Hypothermia
		spellid = 41425;
	else if( m_spellInfo->m_id == 30451) // Cast spell Arcane Blast
		spellid = 36032;
	else if( m_spellInfo->m_id == 20572 || m_spellInfo->m_id == 33702 || m_spellInfo->m_id == 33697) // Cast spell Blood Fury
		spellid = 23230;
	...
	std::map<uint32,Aura*>::iterator itr=Target->tmpAura.find(m_spellInfo->m_id);
	if(itr!=Target->tmpAura.end())
	{
		if(itr->second)
		{
			if(itr->second->GetSpellProto()->m_procCharges>0)
			{
				Aura *aur=NULL;
				for(int i=0;i<itr->second->GetSpellProto()->m_procCharges-1;i++)
				{
					aur = new Aura(itr->second->GetSpellProto(),itr->second->GetDuration(),itr->second->GetCaster(),itr->second->GetTarget());
					Target->AddAura(aur);
					aur=NULL;
				}
				if(!(itr->second->GetSpellProto()->m_procTypeMask & PROC_REMOVEONUSE))
				{
					SpellCharge charge;
					charge.count=itr->second->GetSpellProto()->m_procCharges;
					charge.spellId=itr->second->GetSpellId();
					charge.ProcFlag=itr->second->GetSpellProto()->m_procTypeMask;
					charge.lastproc = 0;
...
{
	skilllinespell* skill = objmgr.GetSpellSkill(m_spellInfo->m_id);
...
{
	return (
		(m_spellInfo->m_id ==  2596) || (m_spellInfo->m_id ==  5118) || (m_spellInfo->m_id == 14320) || (m_spellInfo->m_id == 13159) || (m_spellInfo->m_id == 13161) || (m_spellInfo->m_id == 20190) || 
		(m_spellInfo->m_id == 20043) || (m_spellInfo->m_id == 14322) || (m_spellInfo->m_id == 14321) || (m_spellInfo->m_id == 13163) || (m_spellInfo->m_id == 14319) || (m_spellInfo->m_id == 14318) || (m_spellInfo->m_id == 13165)); 
}

bool Spell::IsSeal()
{
	return (
		(m_spellInfo->m_id == 13903) || (m_spellInfo->m_id == 17177) || (m_spellInfo->m_id == 20154) || (m_spellInfo->m_id == 20162) || (m_spellInfo->m_id == 20164) || (m_spellInfo->m_id == 20165) || 
		(m_spellInfo->m_id == 20166) || (m_spellInfo->m_id == 20287) || (m_spellInfo->m_id == 20288) || (m_spellInfo->m_id == 20289) || (m_spellInfo->m_id == 20290) || (m_spellInfo->m_id == 20291) || 
		(m_spellInfo->m_id == 20292) || (m_spellInfo->m_id == 20293) || (m_spellInfo->m_id == 20305) || (m_spellInfo->m_id == 20306) || (m_spellInfo->m_id == 20307) || (m_spellInfo->m_id == 20308) || 
		(m_spellInfo->m_id == 20347) || (m_spellInfo->m_id == 20348) || (m_spellInfo->m_id == 20349) || (m_spellInfo->m_id == 20356) || (m_spellInfo->m_id == 20357) || (m_spellInfo->m_id == 20375) || 
		(m_spellInfo->m_id == 20915) || (m_spellInfo->m_id == 20918) || (m_spellInfo->m_id == 20919) || (m_spellInfo->m_id == 20920) || (m_spellInfo->m_id == 21082) || (m_spellInfo->m_id == 21084)); 
}

uint8 Spell::CanCast(bool tolerate)
{
	uint32 i;
	if(objmgr.IsSpellDisabled(m_spellInfo->m_id))
		...
		if(m_caster->IsInWorld())
		{
			...
			if( target )
			{
				...
				// If we're attacking a friendly target, target must be undead.
				if( ( m_spellInfo->m_id == 47541 
					|| m_spellInfo->m_id == 49892 
					|| m_spellInfo->m_id == 49893 
					|| m_spellInfo->m_id == 49894
					|| m_spellInfo->m_id == 49895 ) 

				&& ( !isAttackable( u_caster , target ) 
				&& ( ( target->IsCreature() && !( static_cast<Creature*>(target)->GetCreatureName()->Family == UNDEAD ) ) || target->IsPlayer() ) ) )
				{
					return SPELL_FAILED_BAD_TARGETS;
				}
				
				if(m_spellInfo->m_id == 12699)
				{
					if(target->GetEntry() != 5307 || target->isAlive())
						return SPELL_FAILED_BAD_TARGETS;
				}

				if(m_spellInfo->m_id == 30877)
				{
					if(target->GetEntry() != 17326 && target != m_caster)
						return SPELL_FAILED_BAD_TARGETS;
				}
				
				if(m_spellInfo->m_id == 34665)
				{
					if(target->GetEntry() != 16880)
						return SPELL_FAILED_BAD_TARGETS;
				}

				if(m_spellInfo->m_id == 3607)
				{
					if(target->GetEntry() != 2530)
						return SPELL_FAILED_BAD_TARGETS;
				}

				if(m_spellInfo->m_id == 36310)
				{
	...
	if( p_caster != NULL )
	{
		...
		if( !p_caster->isAlive() )
		{
		  //Spirit of Redemption (20711) fix
            if (!(m_spellInfo->c_is_flags & SPELL_FLAG_IS_HEALING) || !p_caster->HasAura(27827)) {
		...
		if ( p_caster->m_bg && ( p_caster->m_bg->GetType() >= BATTLEGROUND_ARENA_2V2 && p_caster->m_bg->GetType() <= BATTLEGROUND_ARENA_5V5 ) && ( m_spellInfo->m_recoveryTime >= 900000 || m_spellInfo->m_categoryRecoveryTime >= 900000 ) )
			return SPELL_FAILED_SPELL_UNAVAILABLE;
		...
		if(p_caster->GetDuelState() == DUEL_STATE_REQUESTED)
		{
			for(i = 0; i < 3; ++i)
			{
				if( m_spellInfo->m_effect[i] && m_spellInfo->m_effect[i] != SPELL_EFFECT_APPLY_AURA && m_spellInfo->m_effect[i] != SPELL_EFFECT_APPLY_PET_AURA
					&& m_spellInfo->m_effect[i] != SPELL_EFFECT_APPLY_AREA_AURA)
				{
		...
		if(p_caster && m_spellInfo->m_id == 7266)
		{
		...
		{
			if( m_spellInfo->m_id != 33836) // exception for Area 52 Special
		...
		if(p_caster->m_CurrentTransporter)
		{
			...
			if( m_spellInfo->m_effectAura[0] == SPELL_AURA_MOUNTED || m_spellInfo->m_effectAura[1] == SPELL_AURA_MOUNTED || m_spellInfo->m_effectAura[2] == SPELL_AURA_MOUNTED)
		...
		{
			if( m_spellInfo->m_id == 25860) // Reindeer Transformation
				return SPELL_FAILED_ONLY_MOUNTED;
		}
		else
		{
			if (!(m_spellInfo->m_attributes & ATTRIBUTES_MOUNT_CASTABLE))
		...
		if(i_caster)
		{
			...
			if(i_caster->GetProto()->Spells[0].Charges != 0)
			{
				...
				if(i_caster->GetProto()->ItemId == m_spellInfo->m_reagent[0])
					if(p_caster->GetItemInterface()->GetItemCount(m_spellInfo->m_reagent[0]) < 1 + m_spellInfo->m_reagentCount[0])
		...
		for(i=0; i<8 ;i++)
		{
			if( m_spellInfo->m_reagent[i] == 0 || m_spellInfo->m_reagentCount[i] == 0)
				continue;

			if(p_caster->GetItemInterface()->GetItemCount(m_spellInfo->m_reagent[i]) < m_spellInfo->m_reagentCount[i])
				return SPELL_FAILED_ITEM_GONE;
		}

		// check if we have the required tools, totems, etc
		if( m_spellInfo->m_totem[0] != 0)
		{
			if(!p_caster->GetItemInterface()->GetItemCount(m_spellInfo->m_totem[0]))
				return SPELL_FAILED_TOTEMS;
		}
		if( m_spellInfo->m_totem[1] != 0)
		{
			if(!p_caster->GetItemInterface()->GetItemCount(m_spellInfo->m_totem[1]))
				return SPELL_FAILED_TOTEMS;
		}
		...
		if( m_spellInfo->m_requiresSpellFocus)
		{
			bool found = false;
            ...
            for(std::set<Object*>::iterator itr = p_caster->GetInRangeSetBegin(); itr != p_caster->GetInRangeSetEnd(); itr++ )
			{
				...
				if(info->sound1)
					...
					focusRange = GetMaxRange(dbcSpellRange.LookupEntry(m_spellInfo->m_rangeIndex));
				...
				if(info->SpellFocus == m_spellInfo->m_requiresSpellFocus)
				{
					found = true;
					break;
				}
		...
		if (m_spellInfo->m_requiredAreaID && m_spellInfo->m_requiredAreaID != p_caster->GetMapMgr()->GetAreaID(p_caster->GetPositionX(),p_caster->GetPositionY()))
			return SPELL_FAILED_REQUIRES_AREA;

		// aurastate check
		if( m_spellInfo->m_casterAuraState )
		{
			if( !p_caster->HasFlag( UNIT_FIELD_AURASTATE, m_spellInfo->m_casterAuraState ) )
	...
	if(m_targets.m_itemTarget && p_caster)
	{
		...
		if( m_targets.m_targetMask & TARGET_FLAG_TRADE_ITEM )
		{
			switch( m_spellInfo->m_effect[0] )
			{
				...
				case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY:
				{
					...
					if( m_spellInfo->m_attributesExB & FLAGS3_ENCHANT_OWN_ONLY )
						return SPELL_FAILED_BAD_TARGETS;
		...
		switch(m_spellInfo->m_effect[0])
		{
			...
			case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY:
			{
				...
				if( m_spellInfo->m_equippedItemClass != (int32)proto->Class)
					return SPELL_FAILED_BAD_TARGETS;

				if( m_spellInfo->m_equippedItemSubclass && !(m_spellInfo->m_equippedItemSubclass & (1 << proto->SubClass)))
					return SPELL_FAILED_BAD_TARGETS;

				if( m_spellInfo->m_equippedItemInvTypes && !(m_spellInfo->m_equippedItemInvTypes & (1 << proto->InventoryType)))
					return SPELL_FAILED_BAD_TARGETS;

				if (m_spellInfo->m_effect[0] == SPELL_EFFECT_ENCHANT_ITEM && 
					m_spellInfo->m_baseLevel && (m_spellInfo->m_baseLevel > proto->ItemLevel))
					return int8(SPELL_FAILED_BAD_TARGETS); // maybe there is different err code
				
				if( i_caster && i_caster->GetProto()->Flags == 2097216)
					break;

				if( m_spellInfo->m_attributesExB & FLAGS3_ENCHANT_OWN_ONLY && !(i_target->IsSoulbound()))
					return SPELL_FAILED_BAD_TARGETS;
	...
	float maxRange = GetMaxRange( dbcSpellRange.LookupEntry( m_spellInfo->m_rangeIndex ) );

	if( m_spellInfo->m_spellClassMask && u_caster != NULL )
	{
		SM_FFValue( u_caster->SM_FRange, &maxRange, m_spellInfo->m_spellClassMask );
		SM_PFValue( u_caster->SM_PRange, &maxRange, m_spellInfo->m_spellClassMask );
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		float spell_flat_modifers=0;
		float spell_pct_modifers=0;
		SM_FFValue(u_caster->SM_FRange,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
		SM_FFValue(u_caster->SM_PRange,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!spell range bonus mod flat %f , spell range bonus pct %f , spell range %f, spell group %u\n",spell_flat_modifers,spell_pct_modifers,maxRange,m_spellInfo->m_spellClassMask);
#endif
	}
	...
	if(m_targets.m_unitTarget)
	{
		...
		if(target)
		{
			...
			if( p_caster != NULL )
			{
				...
				// check aurastate
				if( m_spellInfo->m_targetAuraState )
				{
					if( !target->HasFlag( UNIT_FIELD_AURASTATE, m_spellInfo->m_targetAuraState ) )
					{
				...
				// pet training
				if( m_spellInfo->m_implicitTargetA[0] == EFF_TARGET_PET &&
					m_spellInfo->m_effect[0] == SPELL_EFFECT_LEARN_SPELL )
				{
					Pet *pPet = p_caster->GetSummon();
					// check if we have a pet
					if( pPet == NULL )
					...
					// other checks
					SpellEntry* trig = dbcSpell.LookupEntry( m_spellInfo->m_effectTriggerSpell[0] );
					if( trig == NULL )
						return SPELL_FAILED_SPELL_UNAVAILABLE;
				...
				if( m_spellInfo->m_effectAura[0]==2)//mind control
				{
					if( m_spellInfo->m_effectBasePoints[0])//got level req;
					{
						if((int32)target->getLevel() > m_spellInfo->m_effectBasePoints[0]+1 + int32(p_caster->getLevel() - m_spellInfo->m_spellLevel))
			...
			{
				// Dummy spells check
				if( m_spellInfo->m_id == 4130)// Banish Burning Exile
				{
					if(target->GetEntry()!= 2760) // target needs to be a Burning Exile
						return SPELL_FAILED_BAD_TARGETS;
				}
				if( m_spellInfo->m_id == 4131)// Banish Cresting Exile
				{
					if(target->GetEntry()!= 2761) // target needs to be a Cresting Exile
						return SPELL_FAILED_BAD_TARGETS;
				}
				if( m_spellInfo->m_id == 4132)// Banish Thundering Exile
				{
					if(target->GetEntry()!= 2762) // target needs to be a Thundering Exile
						return SPELL_FAILED_BAD_TARGETS;
				}
				if( m_spellInfo->m_id == 38177) //Blackwhelp Net
				{
				...
				**********************************************************/
				...
				{
					if( m_spellInfo->m_defenseType == SPELL_DMG_TYPE_RANGED )
			...
			if( m_spellInfo->m_effect[0] == SPELL_EFFECT_SKINNING) // skinning
			...
			if(
				m_spellInfo->m_implicitTargetA[0] == 61 || 
				m_spellInfo->m_implicitTargetA[1] == 61 || 
				m_spellInfo->m_implicitTargetA[2] == 61)
			{
			...
			// pet's owner stuff
			/*if (m_spellInfo->m_implicitTargetA[0] == 27 || 
				m_spellInfo->m_implicitTargetA[1] == 27 || 
				m_spellInfo->m_implicitTargetA[2] == 27)
			{
				if (!target->IsPlayer())
			...
			if( m_spellInfo->m_implicitTargetA[0] == 39 )//|| 
			 //m_spellInfo->m_implicitTargetA[1] == 39 || 
			 //m_spellInfo->m_implicitTargetA[2] == 39)
			{
				uint32 entry = m_spellInfo->m_effectMiscValue[0];
			...
			if( p_caster != NULL )
			{
				...
				if( m_spellInfo->m_category==1131)//Hammer of wrath, requires target to have 20- % of hp
				{
				...
				else if( m_spellInfo->m_category==672)//Conflagrate, requires immolation spell on victim
				{
					if(!target->HasAuraVisual(46))
						return SPELL_FAILED_BAD_TARGETS;
				}

				if(target->dispels[m_spellInfo->m_dispelType])
			...
			/*SpellReplacement*rp=objmgr.GetReplacingSpells(m_spellInfo->m_id);
			if(rp)
			{
	...
	if (m_spellInfo->m_mechanic == 19 && ((target) ? target->HasAura(6788) : u_caster->HasAura(6766))) //Weakened Soul
		return SPELL_FAILED_DAMAGE_IMMUNE;
	if (m_spellInfo->m_mechanic == 25 && ((target) ? target->HasAura(25771) : u_caster->HasAura(25771))) //Forbearance
		return SPELL_FAILED_DAMAGE_IMMUNE;
	if (m_spellInfo->NameHash == SPELL_HASH_ICE_BLOCK && u_caster->HasAura(41425))
		return SPELL_FAILED_DAMAGE_IMMUNE;
	if (m_spellInfo->m_mechanic == 16 && ((target) ? target->HasAura(11196) : u_caster->HasAura(11196)))
		return SPELL_FAILED_DAMAGE_IMMUNE;

	// Special State Checks (for creatures & players)
	if( u_caster )
	{
		if( u_caster->SchoolCastPrevent[m_spellInfo->m_schoolMask] ) 
		{	
			uint32 now_ = getMSTime();
			if( now_ > u_caster->SchoolCastPrevent[m_spellInfo->m_schoolMask] )//this limit has expired,remove
				u_caster->SchoolCastPrevent[m_spellInfo->m_schoolMask] = 0;
			else 
			{
				// HACK FIX
		...
		if( u_caster && u_caster->m_silenced && m_spellInfo->m_schoolMask != NORMAL_DAMAGE )// can only silence non-physical
		{
			switch( m_spellInfo->NameHash )
			{
				case SPELL_HASH_ICE_BLOCK: //Ice Block
		...
		if(target) /* -Supalosa- Shouldn't this be handled on Spell Apply? */
		{
			...
			{
				if( m_spellInfo->m_effectAura[i] == SPELL_AURA_MECHANIC_IMMUNITY )
				{
					target->RemoveAllAurasByMechanic( m_spellInfo->m_effectMiscValue[i] , -1 , true );
					// Remove all debuffs of that mechanic type.
					// This is also done in SpellAuras.cpp - wtf?
				}
				/*
				if( m_spellInfo->m_effectAura[i] == SPELL_AURA_MECHANIC_IMMUNITY && (m_spellInfo->m_effectMiscValue[i] == 12 || m_spellInfo->m_effectMiscValue[i] == 17))
				{
					for(uint32 x=MAX_POSITIVE_AURAS;x<MAX_AURAS;x++)
						if(target->m_auras[x])
							if(target->m_auras[x]->GetSpellProto()->MechanicsType == m_spellInfo->m_effectMiscValue[i])
		...
		if( u_caster->IsPacified() && m_spellInfo->m_schoolMask == NORMAL_DAMAGE ) // only affects physical damage
		{
			// HACK FIX
			switch( m_spellInfo->NameHash )
			{
		...
		if(u_caster->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT) > 0)
		{
			...
			{
				if(
					t_spellInfo->m_effectTriggerSpell[0] != m_spellInfo->m_id &&
					t_spellInfo->m_effectTriggerSpell[1] != m_spellInfo->m_id &&
					t_spellInfo->m_effectTriggerSpell[2] != m_spellInfo->m_id)
				{
					return SPELL_FAILED_SPELL_IN_PROGRESS;
				}
			}
...
void Spell::RemoveItems()
{
	...
	// Ammo Removal
	if( m_spellInfo->m_attributesExB == FLAGS3_REQ_RANGED_WEAPON || m_spellInfo->m_attributesExC & FLAGS4_PLAYER_RANGED_SPELLS)
	{
	...
	{
		if( m_spellInfo->m_reagent[i])
		{
			p_caster->GetItemInterface()->RemoveItemAmt_ProtectPointer(m_spellInfo->m_reagent[i], m_spellInfo->m_reagentCount[i], &i_caster);
		}
	}
}
...
int32 Spell::CalculateEffect(uint32 i,Unit *target)
{
	...
	// Range checks
 /*   if (m_spellInfo->m_id == SPELL_RANGED_GUN) // this includes bow and gun
	{
		if(!u_caster || !unitTarget)
			return 0;

		return ::CalculateDamage( u_caster, unitTarget, RANGED, m_spellInfo->m_spellClassMask );
	}
*/
	int32 value = 0;

	float basePointsPerLevel    = m_spellInfo->m_effectRealPointsPerLevel[i];
	float randomPointsPerLevel  = m_spellInfo->m_effectDicePerLevel[i];
	int32 basePoints = m_spellInfo->m_effectBasePoints[i] + 1;
	int32 randomPoints = m_spellInfo->m_effectDieSides[i];
	...
	if(i_caster && (int32(i_caster->GetUInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID)) < 0))
	{
		...
		for(j = 0; j < 3; ++j)
		{
			if(si->enchantments[j] != 0)
			{
				...
				for(k = 0; k < 3; ++k)
				{
					if(ent->spell[k] == m_spellInfo->m_id)
					{
						if(si->prefixes[k] == 0)
							goto exit;
...
exit:
	...
	if( u_caster != NULL )
	{
		int32 diff = -(int32)m_spellInfo->m_baseLevel;
		if (m_spellInfo->m_maxLevel && u_caster->getLevel()>m_spellInfo->m_maxLevel)
			diff +=m_spellInfo->m_maxLevel;
		else
	...
	{	
		...
		int32 comboDamage = (int32)m_spellInfo->m_effectPointsPerCombo[i];
		...
		SpellOverrideMap::iterator itr = p_caster->mSpellOverrideMap.find(m_spellInfo->m_id);
	...
	if( u_caster != NULL )
	{
		...
		SM_FIValue(u_caster->SM_FSPELL_VALUE,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
		SM_FIValue(u_caster->SM_PSPELL_VALUE,&spell_pct_modifers,m_spellInfo->m_spellClassMask);

		SM_FIValue(u_caster->SM_FEffectBonus,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
		SM_FIValue(u_caster->SM_PEffectBonus,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
	...
	{	
		...
		if( item_creator != NULL )
		{
			...
			SM_FIValue(item_creator->SM_FSPELL_VALUE,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
			SM_FIValue(item_creator->SM_PSPELL_VALUE,&spell_pct_modifers,m_spellInfo->m_spellClassMask);

			SM_FIValue(item_creator->SM_FEffectBonus,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
			SM_FIValue(item_creator->SM_PEffectBonus,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
			if(spell_flat_modifers!=0 || spell_pct_modifers!=0 || spell_pct_modifers2!=0)
				printf("!!!!ITEMCASTER ! : spell value mod flat %d , spell value mod pct %d, spell value mod pct2 %d , spell dmg %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,spell_pct_modifers2,value,m_spellInfo->m_spellClassMask);
#endif
			value = value + value*(spell_pct_modifers+spell_pct_modifers2)/100 + spell_flat_modifers;
		}
	}
	...
	// SUPA: Death Knight: Runic Power and Attack Power Coefficient.

	// RUNIC POWER:
	// The effect of any ability that costs Runic Power is affected by the Death Knight's current Runic Power
	// at the time of cast. 100 RP means the spell does 100% effect, whereas 0 RP (technically uncastable) means the spell does 0% effect.
	// Runic Power is gained by casting spells that require Runes to cast. At the moment of writing, all spells give 10 Runic Power.

	// (NOTE: Runic Power, like the warrior Rage it emulates, is handled from a 0-1000 range.)
	
	// ATTACK POWER COEFFICIENT:
	// Death Knights are intended to rely completely on Attack Power to augment the power of their spell effects.
	// For example, Icy Touch (Rank 5) [49909] has an Attack Power Coefficient of 0.2 and a minimum damage of 509.
	// If the Death Knight casting this spell has 1000 attack power, 200 damage will be added to the effect.
	// This means the Icy Touch now has a minimum damage of 709.
	//Log.Line();
	//Log.Notice( "CalculateEffect" , "Calculating Effect for %s (%u)" , m_spellInfo->m_name_lang[0] , m_spellInfo->m_id );
	//Log.Notice( "RP" , "RunicPower = %u/%u" , u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) , u_caster->GetUInt32Value( UNIT_FIELD_MAXPOWER7 ) );
	
	// This will ONLY apply to damage.
	if( m_spellInfo->m_effect[i] == SPELL_EFFECT_SCHOOL_DAMAGE 
		|| m_spellInfo->m_effect[i] == SPELL_EFFECT_WEAPON_DAMAGE_NOSCHOOL
		|| m_spellInfo->m_effect[i] == SPELL_EFFECT_WEAPON_PERCENT_DAMAGE
		|| m_spellInfo->m_effect[i] == SPELL_EFFECT_DUMMY
		|| ( m_spellInfo->m_effect[i] == SPELL_EFFECT_APPLY_AURA &&
			( m_spellInfo->m_effectAura[i] == SPELL_AURA_PERIODIC_DAMAGE )
			)
		)
	{
		if( m_spellInfo->m_powerType == POWER_TYPE_RUNIC_POWER )
		{
			//Log.Notice( "CalculateEffect" , "Value before Runic Power mod: %u" , value );
			//value *= ( u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) / u_caster->GetUInt32Value( UNIT_FIELD_MAXPOWER7 ) ) + 1;
			//Log.Notice( "Runic power:" , "RP/1000 = %f" , (float)u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) / 1000.0f );
			value = float2int32( (float)u_caster->GetUInt32Value( UNIT_FIELD_POWER7 ) / 1000.0f * (float)value );
			//Log.Notice( "CalculateEffect" , "Value after Runic Power mod: %u" , value );
		}
		//Log.Line();
		//Log.Notice( "Attack Power Coefficient" , "Apcoef = %f" , m_spellInfo->fixed_apcoefficient );
		if( m_spellInfo->fixed_apcoefficient > 0 )
		{
			//Log.Notice( "CalculateEffect" , "Value before Attack Power mod: %u" , value );
			value += float2int32( (float)u_caster->GetUInt32Value( UNIT_FIELD_ATTACK_POWER ) * m_spellInfo->fixed_apcoefficient );
			//Log.Notice( "CalculateEffect" , "Value after Attack Power mod: %u" , value );
		}
		//Log.Line();
	}
	return value;
...
void Spell::HandleTeleport(uint32 id, Unit* Target)
{
    ...
    // predefined behavior 
	if (m_spellInfo->m_id == 8690 || m_spellInfo->m_id == 556 || m_spellInfo->m_id == 39937)// 556 - Astral Recall ; 39937 - Ruby Slippers
	{
...
/*void Spell::_DamageRangeUpdate()
{
	if(unitTarget)
	{
		...
		{
			m_caster->SpellNonMeleeDamageLog(unitTarget,m_spellInfo->m_id, damageToHit);
		}
		else
		{	if( u_caster != NULL )
...
void Spell::SendHealSpellOnPlayer(Object* caster, Object* target, uint32 dmg,bool critical)
{
	...
	data << (pSpellId ? pSpellId : m_spellInfo->m_id);
...
void Spell::Heal(int32 amount, bool ForceCrit)
{
	...
	{
		//Downranking
		if(p_caster && p_caster->IsPlayer() && m_spellInfo->m_baseLevel > 0 && m_spellInfo->m_maxLevel > 0)
		{
			float downrank1 = 1.0f;
			if (m_spellInfo->m_baseLevel < 20)
				downrank1 = 1.0f - (20.0f - float (m_spellInfo->m_baseLevel) ) * 0.0375f;
			float downrank2 = ( float(m_spellInfo->m_maxLevel + 5.0f) / float(p_caster->getLevel()) );
		...
		//Basic bonus
		bonus += u_caster->HealDoneMod[m_spellInfo->m_schoolMask];
		bonus += unitTarget->HealTakenMod[m_spellInfo->m_schoolMask];

		//Bonus from Intellect & Spirit
		if( p_caster != NULL )  
		{
			for(uint32 a = 0; a < 6; a++)
				bonus += float2int32(p_caster->SpellHealDoneByAttribute[a][m_spellInfo->m_schoolMask] * p_caster->GetUInt32Value(UNIT_FIELD_STAT0 + a));
		}
		...
		critchance = float2int32(u_caster->spellcritperc + u_caster->SpellCritChanceSchool[m_spellInfo->m_schoolMask]);

		if(m_spellInfo->m_spellClassMask)
		{
			int penalty_pct = 0;
			int penalty_flt = 0;
			SM_FIValue( u_caster->SM_PPenalty, &penalty_pct, m_spellInfo->m_spellClassMask );
			bonus += amount*penalty_pct/100;
			SM_FIValue( u_caster->SM_FPenalty, &penalty_flt, m_spellInfo->m_spellClassMask );
			bonus += penalty_flt;
			SM_FIValue( u_caster->SM_CriticalChance,&critchance,m_spellInfo->m_spellClassMask);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
			int spell_flat_modifers=0;
			int spell_pct_modifers=0;
			SM_FIValue(u_caster->SM_FPenalty,&spell_flat_modifers,m_spellInfo->m_spellClassMask);
			SM_FIValue(u_caster->SM_PPenalty,&spell_pct_modifers,m_spellInfo->m_spellClassMask);
			if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
				printf("!!!!!HEAL : spell dmg bonus(p=24) mod flat %d , spell dmg bonus(p=24) pct %d , spell dmg bonus %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,bonus,m_spellInfo->m_spellClassMask);
#endif
		}
		
		amount += float2int32( float( bonus ) * healdoneaffectperc ); //apply downranking on final value ?
		amount += amount*u_caster->HealDonePctMod[m_spellInfo->m_schoolMask]/100;
		amount += float2int32( float( amount ) * unitTarget->HealTakenPctMod[m_spellInfo->m_schoolMask] );

		if (m_spellInfo->m_spellClassMask)
			SM_FIValue(u_caster->SM_PDamageBonus,&amount,m_spellInfo->m_spellClassMask);
		
		if( critical = Rand(critchance) || ForceCrit )
		{
			/*int32 critbonus = amount >> 1;
			if( m_spellInfo->m_spellClassMask)
					SM_PIValue(static_cast<Unit*>(u_caster)->SM_PCriticalDamage, &critbonus, m_spellInfo->m_spellClassMask);
			amount += critbonus;*/

			int32 critical_bonus = 100;
			if( m_spellInfo->m_spellClassMask )
				SM_FIValue( u_caster->SM_PCriticalDamage, &critical_bonus, m_spellInfo->m_spellClassMask );
	...
	if (p_caster)
	{
		p_caster->m_casted_amount[m_spellInfo->m_schoolMask]=amount;
...
bool Spell::Reflect(Unit *refunit)
{
	...
	for(int i=0; i<3; i++)
    {
		if( m_spellInfo->m_effect[i] == 6 && (m_spellInfo->m_effectAura[i] == 74 || m_spellInfo->m_effectAura[i] == 28))
			return false;
    }
	...
	{
		if((*i)->school == -1 || (*i)->school == (int32)m_spellInfo->m_schoolMask)
		{
...
void Spell::SendCastSuccess(const uint64& guid)
{
	...
	*(uint32*)&buffer[c] = swap32(m_spellInfo->m_id);         c += 4;
#else
	*(uint32*)&buffer[c] = m_spellInfo->m_id;                 c += 4;
#endif
...
bool IsBeneficSpell(SpellEntry *sp)
{
	...
	for(uint32 i=0;i<3;i++)
		...
		{
			if(j==0)
				cur = sp->m_implicitTargetA[i];
			else // if(j==1)
				cur = sp->m_implicitTargetB[i];		
			switch(cur)
			{
				case EFF_TARGET_SELF:
				case EFF_TARGET_PET:
