...
Aura::Aura( SpellEntry* proto, int32 duration, Object* caster, Unit* target )
{
	...
	for( int x = 0 ; x <= 2; x ++ )
	m_amplitude[x] = GetSpellProto()->m_effectAmplitude[x];
	...
	pSpellId = 0;
	periodic_target = 0;
	//sLog.outDetail("Aura::Constructor %u (%s) from %u.", m_spellProto->m_id, m_spellProto->Name, m_target->GetLowGUID());
...
void Aura::Remove()
{
	...
	if( !IsPassive() || IsPassive() && m_spellProto->m_attributesEx & 1024 )
		RemoveAuraVisual();
	...
	{
		if( !m_spellProto->m_effect[x] )
			break;

		if( m_spellProto->m_effect[x] == SPELL_EFFECT_TRIGGER_SPELL && !m_spellProto->always_apply )
		{
			// Im not sure about this! FIXME!!
			if ( dbcSpell.LookupEntryForced( GetSpellProto()->m_effectTriggerSpell[x] )->m_durationIndex < m_spellProto->m_durationIndex )
			m_target->RemoveAura(GetSpellProto()->m_effectTriggerSpell[x]);
		}
		else if( m_spellProto->m_effect[x] == SPELL_EFFECT_APPLY_AREA_AURA && m_casterGuid == m_target->GetGUID())
		{
			RemoveAA();
		}
	}

	if( m_spellProto->m_procCharges > 0 && m_spellProto->proc_interval == 0 )
	{
	...
	if( GetSpellProto()->m_spellClassMask && m_target->GetTypeId() == TYPEID_PLAYER )
	{
		int32 speedmod = 0;
		SM_FIValue( m_target->SM_FSpeedMod, &speedmod, m_spellProto->m_spellClassMask );
		if( speedmod )
		{
	...
	*************************************************************/
	if( caster != NULL && caster->IsPlayer() && caster->IsInWorld() && m_spellProto->c_is_flags & SPELL_FLAG_IS_REQUIRECOOLDOWNUPDATE )
	{
		...
		data << m_spellProto->m_id << caster->GetGUID();
		caster->SendMessageToSet( &data, true );*/

		packetSMSG_COOLDOWN_EVENT data;
		data.spellid = m_spellProto->m_id;
	...
	if( caster != NULL && caster->IsPlayer() && caster->IsInWorld() )
	{
		int j;
		for( j = 0; j < 3; ++j )
			if( m_spellProto->m_effect[j] == SPELL_EFFECT_ADD_FARSIGHT )
				break;
...
void Aura::AddMod( uint32 t, int32 a, uint32 miscValue, uint32 i )
{
	...
	if( m_modcount >= 3 )
	{
		sLog.outString("Tried to add >3 (%u) mods to spellid %u [%u:%u, %u:%u, %u:%u]", m_modcount+1, this->m_spellProto->m_id, m_modList[0].m_type, m_modList[0].m_amount, m_modList[1].m_type, m_modList[1].m_amount, m_modList[2].m_type, m_modList[2].m_amount);
		return;
	}
...
{ 
	...
	for( uint32 x = 0; x < m_modcount; x++ )
	{
		...
		sLog.outDebug( "WORLD: target = %u , Spell Aura id = %u, SpellId  = %u, i = %u, apply = %s, duration = %u, damage = %d",
			m_target->GetLowGUID(),mod->m_type, m_spellProto->m_id, mod->i, apply ? "true" : "false",GetDuration(),mod->m_amount); 
...
/*
	...
	{
		for(uint32 x=0; x<3; x++)
			if(GetSpellProto()->m_effectAura[x] == SPELL_AURA_PROC_TRIGGER_SPELL||GetSpellId()==974||GetSpellId()==32593||GetSpellId()==32594)
...
void Aura::ModAmplitude( float amp_factor )
{
	// This is used for spell haste, etc.
	for( int x = 0 ; x <= 2 ; x ++ )
	{
		m_amplitude[x] = float2int32( (float)m_amplitude[x] * amp_factor );
	}
}
void Aura::AddAuraVisual()
{
	...
	m_visualSlot = m_target->AddAuraVisual(m_spellProto->m_id, 1, IsPositive());

	/*m_target->SetUInt32Value(UNIT_FIELD_AURA + slot, m_spellProto->m_id);
	...
	data << m_target->GetNewGUID() << m_visualSlot << uint32(m_spellProto->m_id) << uint32(m_duration) << uint32(m_duration);
...
{
	m_target->RemoveAuraVisual(m_spellProto->m_id, 1);
...
void Aura::EventUpdateAA(float r)
{
	...
	{
		if(!plr->HasActiveAura(m_spellProto->m_id))
		{
		 ...
		 )
	{
		...
		if( summon->isAlive() && summon->GetDistanceSq(u_caster) <= r && !summon->HasActiveAura( m_spellProto->m_id ))
		{
...
//			sEventMgr.AddEvent(((Unit*)summon), &Unit::EventRemoveAura, m_spellProto->m_id, EVENT_DELETE_TIMER, 10, 1,0);
		}
	}
	...
	if(group && group->GetMemberCount() > 0 )
	{
		...
		for(; itr != group->GetGroupMembersEnd(); ++itr)
		{
			if((*itr) && (*itr)->m_loggedInPlayer && (*itr)->m_loggedInPlayer != plr && (*itr)->m_loggedInPlayer->GetDistanceSq(u_caster) <= r)
			{
				...
				if(!(*itr)->m_loggedInPlayer->HasActiveAura(m_spellProto->m_id) &&
					(*itr)->m_loggedInPlayer->GetInstanceID() == plr->GetInstanceID() && (*itr)->m_loggedInPlayer->isAlive())
				{
					...
					for(i = 0; i < m_modcount; ++i)
					{
						...
						if(m_spellProto->m_effect[m_modList[i].i] == SPELL_EFFECT_APPLY_AREA_AURA)
						{
							if(!aura)
							{
		...
		if(!iplr || iplr->GetDistanceSq(u_caster) > r || iplr->GetInstanceID() != plr->GetInstanceID())
		{
			...
			if(iplr)
			{
                ...
                if(iplr->GetInstanceID() != plr->GetInstanceID())
					sEventMgr.AddEvent(((Unit*)iplr), &Unit::EventRemoveAura, m_spellProto->m_id, EVENT_DELETE_TIMER, 10, 1,0);
				else
					iplr->RemoveAura(m_spellProto->m_id);
			}
...
void Aura::RemoveAA()
{
		 ...
		 )
	{
		Unit *summon = plr->GetSummon();
		if( summon->isAlive() )
			summon->RemoveAura( m_spellProto->m_id );
	}
	...
	if(apply)
	{
		...
		switch(m_spellProto->m_id)
		{
			...
			case 12654:
			{
				...
				if (c && c->IsPlayer())
				{
					dmg = float2int32(static_cast< Player* >(c)->m_casted_amount[SCHOOL_FIRE]*parentsp->m_effectBasePoints[0]/100.0f);
				}
				else
				{
					if (!dmg)
					...
					for(int i=0;i<3;i++)
					{
					  //dmg +=parentsp->m_effectBasePoints[i]*m_spellProto->m_effectBasePoints[0];
						dmg +=spell->CalculateEffect(i,m_target->IsUnit()?(Unit*)m_target:NULL)*parentsp->m_effectBasePoints[0]/100;
					}
					delete spell;
				}
			}
		...
		uint64 gr = GetSpellProto()->m_spellClassMask;
		if(gr)
		{
			Unit*c=GetUnitCaster();
			if(c)
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicDamage,(uint32)dmg, 
			EVENT_AURA_PERIODIC_DAMAGE,m_amplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);

		/*static_cast< Player* >( c )->GetSession()->SystemMessage("dot will do %u damage every %u seconds (total of %u)", dmg,m_spellProto->m_effectAmplitude[mod->i],(GetDuration()/m_spellProto->m_effectAmplitude[mod->i])*dmg);
		printf("dot will do %u damage every %u seconds (total of %u)\n", dmg,m_spellProto->m_effectAmplitude[mod->i],(GetDuration()/m_spellProto->m_effectAmplitude[mod->i])*dmg);*/
		SetNegative();
...
void Aura::EventPeriodicDamage(uint32 amount)
{
	...
	if(!m_target->isAlive())
		return;
	if(m_target->SchoolImmunityList[GetSpellProto()->m_schoolMask])
		return;
	float res = float(amount);
	int bonus = 0;
	uint32 school = GetSpellProto()->m_schoolMask;
	Unit * c = GetUnitCaster();
	...
	if(m_target->GetGUID()!=m_casterGuid)//don't use resist when cast on self-- this is some internal stuff
	{
		if(c)
		{
			...
			int amp = m_spellProto->m_effectAmplitude[mod->i];
			if( !amp ) 
		...
		if(res > 0 && c && m_spellProto->m_mechanic != MECHANIC_BLEEDING)
		{
		...
		SendPeriodicAuraLog(m_casterGuid, m_target, GetSpellProto()->m_id, school, float2int32(res), abs_dmg, dmg.resisted_damage, FLAG_PERIODIC_DAMAGE);
	...
	case 32594:
		{
			...
			{
			ProcTriggerSpell pts;
			pts.origId = GetSpellProto()->m_id;
			pts.caster = m_casterGuid;
			pts.spellId=GetSpellProto()->m_id;
			if(!pts.spellId)
				return;
			pts.procChance = GetSpellProto()->m_procChance;
			pts.procFlags = GetSpellProto()->m_procTypeMask;
			pts.procCharges = GetSpellProto()->m_procCharges;
			pts.LastTrigger = 0;
			pts.deleted = false;
			m_target->m_procSpells.push_front(pts);
			}
...
//					pts.procFlags = GetSpellProto()->procFlags;
	...
	case 33763:		// lifebloom
		{
			...
			//pCaster->Heal( m_target, m_spellProto->m_id, mod->m_amount );
		}break;
	...
	if ( TamingSpellid && ! GetTimeLeft() )
	{
		...
		Quest* tamequest = QuestStorage.LookupEntry( triggerspell->m_effectMiscValue[1] );
		...
		{	
			...
			{
				p_caster->SendCastResult( triggerspell->m_id,SPELL_FAILED_TRY_AGAIN,0,0 );
			}
		}
		else
		{
			p_caster->SendCastResult( triggerspell->m_id,SPELL_FAILED_BAD_TARGETS,0,0 );
		}
		TamingSpellid = 0;
	}
}
...
void Aura::SpellAuraModCharm(bool apply)
{
	...
	if( apply )
	{
		...
		if( caster->GetSession() ) // crashfix
		{
			...
			target->SetEnslaveSpell(m_spellProto->m_id);
		}
	}
	else
	{
...
void Aura::SpellAuraPeriodicHeal( bool apply )
{
	...
	{
		SetPositive();
		sEventMgr.AddEvent( this, &Aura::EventPeriodicHeal,(uint32)mod->m_amount, EVENT_AURA_PERIODIC_HEAL, GetSpellProto()->m_effectAmplitude[mod->i], 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT );
		...
		if( GetSpellProto()->NameHash == SPELL_HASH_REJUVENATION || GetSpellProto()->NameHash == SPELL_HASH_REGROWTH )
		{
			...
			{
				sEventMgr.AddEvent( m_target, &Unit::EventAurastateExpire, (uint32)AURASTATE_FLAG_REJUVENATE, EVENT_REJUVENATION_FLAG_EXPIRE, GetSpellProto()->m_effectAmplitude[mod->i], 1, 0 );
			}
			else
			{
				sEventMgr.ModifyEventTimeLeft( m_target,EVENT_REJUVENATION_FLAG_EXPIRE, GetSpellProto()->m_effectAmplitude[mod->i], 0 );
			}
		}
	}
}
...
void Aura::EventPeriodicHeal( uint32 amount )
{
	...
	if( c != NULL && c->IsPlayer() )
	{
		for(uint32 a = 0; a < 6; a++)
			bonus += float2int32( static_cast< Player* >( c )->SpellHealDoneByAttribute[a][m_spellProto->m_schoolMask] * static_cast< Player* >( c )->GetUInt32Value( UNIT_FIELD_STAT0 + a) );

		bonus += c->HealDoneMod[GetSpellProto()->m_schoolMask] + m_target->HealTakenMod[m_spellProto->m_schoolMask];
		...
		else
		{
			...
			if( m_spellProto->fixed_hotdotcoef >= 0 )
			{
				...
				{
					int durmod = 0;
					SM_FIValue(c->SM_FDur, &durmod, m_spellProto->m_spellClassMask);
	...
	if( c != NULL && m_spellProto->m_spellClassMask )
	{
		int penalty_pct = 0;
		int penalty_flt = 0;
		SM_FIValue( c->SM_PPenalty, &penalty_pct, GetSpellProto()->m_spellClassMask );
		bonus += bonus * ( penalty_pct / 100 );
		SM_FIValue( c->SM_FPenalty, &penalty_flt, GetSpellProto()->m_spellClassMask );
		bonus += penalty_flt;
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
		int spell_flat_modifers=0;
		int spell_pct_modifers=0;
		SM_FIValue(c->SM_FPenalty,&spell_flat_modifers,GetSpellProto()->m_spellClassMask);
		SM_FIValue(c->SM_PPenalty,&spell_pct_modifers,GetSpellProto()->m_spellClassMask);
		if(spell_flat_modifers!=0 || spell_pct_modifers!=0)
			printf("!!!!!HEAL : spell dmg bonus(p=24) mod flat %d , spell dmg bonus(p=24) pct %d , spell dmg bonus %d, spell group %u\n",spell_flat_modifers,spell_pct_modifers,bonus,GetSpellProto()->m_spellClassMask);
#endif
	}
    ...
    if( c != NULL && c->IsPlayer() )
    {
		if( m_spellProto->m_baseLevel > 0 && m_spellProto->m_maxLevel > 0 )
		{
            float downrank1 = 1.0f;
            if( m_spellProto->m_baseLevel < 20 )
                downrank1 = 1.0f - ( 20.0f - float( m_spellProto->m_baseLevel ) ) * 0.0375f;

            float downrank2 = ( float(m_spellProto->m_maxLevel + 5.0f) / float(c->getLevel()) );
	...
	if( c != NULL )
	{
		add += float2int32( add * ( m_target->HealTakenPctMod[m_spellProto->m_schoolMask]+ c->HealDonePctMod[GetSpellProto()->m_schoolMask] / 100.0f));
		if (m_spellProto->m_spellClassMask)
			SM_PIValue(c->SM_PDOT,&add,m_spellProto->m_spellClassMask);
	}
	...
	if( GetSpellProto()->m_auraInterruptFlags & AURA_INTERRUPT_ON_STAND_UP )
	{
...
void Aura::SpellAuraModStun(bool apply)
{
	...
	if(m_spellProto->m_id == 38554) return; // Absorb Eye of Grillok
...
void Aura::SpellAuraDamageShield(bool apply)
{
	if(apply)
	{
		...
		ds.m_spellId = GetSpellProto()->m_id;
		ds.m_school = GetSpellProto()->m_schoolMask;
...
void Aura::SpellAuraModStealth(bool apply)
{
	...
	else 
	{
		...
		if( m_target->GetTypeId() == TYPEID_PLAYER )
		{
			...
			/*WorldPacket data(12);
			data.SetOpcode(SMSG_COOLDOWN_EVENT);
			data << (uint32)GetSpellProto()->m_id << m_target->GetGUID();
			static_cast< Player* >( m_target )->GetSession()->SendPacket (&data);*/
			packetSMSG_COOLDOWN_EVENT cd;
			cd.guid = m_target->GetGUID();
			cd.spellid = m_spellProto->m_id;
...
{
	SetPositive();
	if(m_spellProto->m_effect[mod->i] == 128)
		return;
...
void Aura::SpellAuraModInvisibilityDetection(bool apply)
{
	...
	assert(mod->m_miscValue < INVIS_FLAG_TOTAL+1);
	if(apply)
	{
...
void Aura::SpellAuraModTotalHealthRegenPct(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHealPct,(float)mod->m_amount,
			EVENT_AURA_PERIODIC_HEALPERC,	GetSpellProto()->m_effectAmplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicHealPct(float RegenPct)
{
	...
	SendPeriodicAuraLog(m_casterGuid, m_target, m_spellProto->m_id, m_spellProto->m_schoolMask, add, 0, 0, FLAG_PERIODIC_HEAL);
...
void Aura::SpellAuraModTotalManaRegenPct(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicManaPct,(float)mod->m_amount,
			EVENT_AURA_PERIOCIC_MANA,	GetSpellProto()->m_effectAmplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicManaPct(float RegenPct)
{
	...
	//SendPeriodicAuraLog(m_target, m_casterGuid, GetSpellProto()->m_id, FLAG_PERIODIC_HEAL, add,true);
	//SendPeriodicAuraLog(m_target, m_casterGuid, GetSpellProto()->m_id, FLAG_PERIODIC_HEAL, add);
...
{
	if(m_spellProto->m_effectTriggerSpell[mod->i] == 0)
		return;
	...
	if(IsPassive() &&
		...
		m_spellProto->m_id != 8145 && 
		m_spellProto->m_id != 8167 &&
		m_spellProto->m_id != 8172)
	{
		Unit * target = (m_target != 0) ? m_target : GetUnitCaster();
		if(target == 0 || !target->IsPlayer())
			return; //what about creatures ?

		SpellEntry *proto = dbcSpell.LookupEntry( m_spellProto->m_effectTriggerSpell[mod->i] );

		if( apply )
			static_cast< Player* >( target )->AddOnStrikeSpell( proto, m_spellProto->m_effectAmplitude[mod->i] );
		else
	...
	if(apply)
	{
		...
		uint32 sp = GetSpellProto()->m_effectTriggerSpell[mod->i];
		...
		if(m_caster->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT))
		{
			...
			EVENT_AURA_PERIODIC_TRIGGERSPELL,GetSpellProto()->m_effectAmplitude[mod->i], 0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
...
void Aura::EventPeriodicTriggerSpell(SpellEntry* spellInfo)
{
	...
	if( spellInfo->m_implicitTargetA[0] == 18 )			// Hellfire, if there are any others insert here
	{
	...
	if(spellInfo->m_spellIconID == 225 ) // this is arcane missles to avoid casting on self
		if(m_casterGuid == pTarget->GetGUID())
			return;

	// set up our max Range
	float maxRange = GetMaxRange( dbcSpellRange.LookupEntry( spellInfo->m_rangeIndex ) );

	if( spellInfo->m_spellClassMask )
	{
		SM_FFValue( m_caster->SM_FRange, &maxRange, spellInfo->m_spellClassMask );
		SM_PFValue( m_caster->SM_PRange, &maxRange, spellInfo->m_spellClassMask );
	}
...
void Aura::SpellAuraPeriodicEnergize(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicEnergize,(uint32)mod->m_amount,(uint32)mod->m_miscValue,
			EVENT_AURA_PERIODIC_ENERGIZE,GetSpellProto()->m_effectAmplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicEnergize(uint32 amount,uint32 type)
{
	...
	SendPeriodicAuraLog( m_casterGuid, m_target, m_spellProto->m_id, m_spellProto->m_schoolMask, amount, 0, 0, FLAG_PERIODIC_ENERGIZE);

	if((GetSpellProto()->m_auraInterruptFlags & AURA_INTERRUPT_ON_STAND_UP) && type == 0)
	{
		m_target->Emote(EMOTE_ONESHOT_EAT);
	}
}
...
void Aura::SpellAuraModPacify(bool apply)
{
	...
	if( apply )
	{
		if( m_spellProto->m_id == 24937 || m_spellProto->NameHash == SPELL_HASH_BLESSING_OF_PROTECTION )
			SetPositive();
		else
			SetNegative();
...
void Aura::SpellAuraModDecreaseSpeed(bool apply)
{
	...
	if(apply)
	{
		...
		if(m_spellProto->m_schoolMask==SCHOOL_FROST)
		{
		...
		m_target->speedReductionMap.insert(make_pair(m_spellProto->m_id, mod->m_amount));
		//m_target->m_slowdown=this;
		//m_target->m_speedModifier += mod->m_amount;
	}
	else
	{
		map< uint32, int32 >::iterator itr = m_target->speedReductionMap.find(m_spellProto->m_id);
...
void Aura::SpellAuraModShapeshift(bool apply)
{
	...
	if( p_target->m_MountSpellId && p_target->m_MountSpellId != m_spellProto->m_id )
	...
	case FORM_TRAVEL:
		{//druid
			...
			{
				if( m_target->IsPlayer() )
					static_cast< Player* >( m_target )->m_MountSpellId = m_spellProto->m_id;
			}
			else
			{
				if( m_target->IsPlayer() )
	...
	if( apply )
	{
		...
		if( freeMovements )
		{
			for( uint32 x = MAX_POSITIVE_AURAS; x < MAX_AURAS; x++ )
			{
				...
				{
					if( m_target->m_auras[x]->GetSpellProto()->m_mechanic == 7 || m_target->m_auras[x]->GetSpellProto()->m_mechanic == 11 ) // Remove roots and slow spells
					{
					...
					else // if got immunity for slow, remove some that are not in the mechanics
					{
						for( int i = 0; i < 3; i++ )
						{
							if( m_target->m_auras[x]->GetSpellProto()->m_effectAura[i] == SPELL_AURA_MOD_DECREASE_SPEED || m_target->m_auras[x]->GetSpellProto()->m_effectAura[i] == SPELL_AURA_MOD_ROOT )
							{
								m_target->m_auras[x]->Remove();
								break;
							}
...
{
	if( m_spellProto->m_id == 24937 )
		SetPositive();

	if( m_spellProto->m_id == 23333 || m_spellProto->m_id == 23335 || m_spellProto->m_id == 34976 )
	{
		if( !apply )
		{
...
void Aura::SpellAuraModSchoolImmunity(bool apply)
{
	if( apply && ( m_spellProto->NameHash == SPELL_HASH_DIVINE_SHIELD || m_spellProto->NameHash == SPELL_HASH_ICE_BLOCK) ) // Paladin - Divine Shield
	{
		...
		for(uint32 i = MAX_POSITIVE_AURAS; i < MAX_AURAS; ++i)
		{
			...
			if( pAura != this && pAura != NULL && !pAura->IsPassive() && !pAura->IsPositive() && !(pAura->GetSpellProto()->m_attributes & ATTRIBUTES_IGNORE_INVULNERABILITY) )
			{
				pAura->Remove();
			}
		}
	...
	if(apply)
	{
		for(uint32 x=0;x<MAX_AURAS;x++)
		{
			...
			if(m_target->m_auras[x] && m_target->m_auras[x]->GetSpellId() != 41425 && m_target->m_auras[x]->GetSpellId() != 25771)
				if(m_target->m_auras[x]->GetSpellProto()->m_dispelType==(uint32)mod->m_miscValue)
					m_target->m_auras[x]->Remove();
		}
	}
}
...
void Aura::SpellAuraProcTriggerSpell(bool apply)
{
	if(apply)
	{
		...
		pts.origId = GetSpellProto()->m_id;
		pts.caster = m_casterGuid;
		if(GetSpellProto()->m_effectTriggerSpell[mod->i])
			pts.spellId=GetSpellProto()->m_effectTriggerSpell[mod->i];
		else
		{
			sLog.outDebug("Warning,trigger spell is null for spell %u",GetSpellProto()->m_id);
			return;
		}
		pts.procChance = GetSpellProto()->m_procChance;
		pts.procFlags = GetSpellProto()->m_procTypeMask;
...
/*		pts.ospinfo = m_spellProto;
		...
		m_target->m_procSpells.push_front(pts);
		sLog.outDebug("%u is registering %u chance %u flags %u charges %u triggeronself %u interval %u\n",pts.origId,pts.spellId,pts.procChance,m_spellProto->m_procTypeMask & ~PROC_TARGET_SELF,m_spellProto->m_procCharges,m_spellProto->m_procTypeMask & PROC_TARGET_SELF,m_spellProto->proc_interval);
	}
	else
	{
		for(std::list<struct ProcTriggerSpell>::iterator itr = m_target->m_procSpells.begin();itr != m_target->m_procSpells.end();itr++)
...
void Aura::SpellAuraProcTriggerDamage(bool apply)
{
	if(apply)
	{
		...
		ds.m_spellId = GetSpellProto()->m_id;
		ds.m_school = GetSpellProto()->m_schoolMask;
		ds.m_flags = m_spellProto->m_procTypeMask;
		ds.owner = (void*)this;
		m_target->m_damageShields.push_back(ds);
		sLog.outDebug("registering dmg proc %u, school %u, flags %u, charges %u \n",ds.m_spellId,ds.m_school,ds.m_flags,m_spellProto->m_procCharges);
	}
	else
	{
		for(std::list<struct DamageProc>::iterator i = m_target->m_damageShields.begin();i != m_target->m_damageShields.end();i++)
...
//		SM_FIValue(m_target->SM_FSPELL_VALUE, &amt, GetSpellProto()->m_spellClassMask);
		if(apply)
		{
			if(amt<0)
				SetNegative();
...
void Aura::SpellAuraModCritPerc(bool apply)
{
	if (m_target->IsPlayer())
	{
		if(apply)
		{
			...
			md.wclass = GetSpellProto()->m_equippedItemClass;
			md.subclass = GetSpellProto()->m_equippedItemSubclass;
...
void Aura::SpellAuraPeriodicLeech(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicLeech,amt,
			EVENT_AURA_PERIODIC_LEECH,	GetSpellProto()->m_effectAmplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicLeech(uint32 amount)
{
	...
	if(m_target->isAlive() && m_caster->isAlive())
	{
		...
		data << m_spellProto->m_id;
		data << uint32(1);
		data << uint32(FLAG_PERIODIC_HEAL);
		data << uint32(Amount);
		m_target->SendMessageToSet(&data,true);

		SendPeriodicAuraLog(m_target, m_target, m_spellProto->m_id, m_spellProto->m_schoolMask, Amount, 0, 0, FLAG_PERIODIC_LEECH);

		//deal damage before we add healing bonus to damage
		m_target->DealDamage(m_target, Amount, 0, 0, GetSpellProto()->m_id,true);
...
void Aura::SpellAuraTransform(bool apply)
{
   ...
   // SetPositive();
	switch( GetSpellProto()->m_id )
	{
		...
		case 28272:	 // Polymorph: Pig
			{
				...
				if(apply)
				{
					...
					m_target->polySpell = GetSpellProto()->m_id;
				}
				else
				{
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAYID, m_target->GetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID));
		...
		case 47241: // Metamorphsis/Demonform
		{
			if( apply )
				m_target->SetUInt32Value( UNIT_FIELD_DISPLAYID, 21069 ); //Currently the model info is in the DBCs, but not the actual display Id. Therefor, blizz is using a dreadlord for now. Ill update it when its ready. I guess.
			else
				m_target->SetUInt32Value( UNIT_FIELD_DISPLAYID, m_target->GetUInt32Value( UNIT_FIELD_NATIVEDISPLAYID ) );
		}break;
...
void Aura::SpellAuraPeriodicHealthFunnel(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHealthFunnel, amt,
			EVENT_AURA_PERIODIC_HEALTH_FUNNEL, GetSpellProto()->m_effectAmplitude[mod->i], 0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicHealthFunnel(uint32 amount)
{
	...
	if(m_target->isAlive() && m_caster->isAlive())
	{
		...
		SendPeriodicAuraLog(m_target, m_target, m_spellProto->m_id, m_spellProto->m_schoolMask, 1000, 0, 0, FLAG_PERIODIC_LEECH);
	}
}
...
void Aura::SpellAuraSchoolAbsorb(bool apply)
{
	...
	if(apply)
	{
		...
		if( plr )
		{
			...
			int flat = 0;
			SM_FIValue( plr->SM_FSPELL_VALUE, &flat, GetSpellProto()->m_spellClassMask );
			...
			if( GetSpellProto()->Dspell_coef_override >= 0 )
				val += float2int32( float( plr->HealDoneMod[GetSpellProto()->m_schoolMask] ) * GetSpellProto()->Dspell_coef_override );
			//For spells Affected by Bonus Damage we use OTspell_coef_override.
			else if( GetSpellProto()->OTspell_coef_override >= 0 )
				val += float2int32( float( plr->GetDamageDoneMod( GetSpellProto()->m_schoolMask ) ) * GetSpellProto()->OTspell_coef_override );
		}
...
void Aura::SpellAuraReflectSpellsSchool(bool apply)
{
	if(apply)
	{
		...
		if(m_spellProto->m_attributes == 0x400D0 && m_spellProto->m_attributesEx == 0)
		{
			rss->school = (int)(log10((float)mod->m_miscValue) / log10((float)2));
		}
		else
			rss->school = m_spellProto->m_schoolMask;
			...
			for(uint32 x=MAX_POSITIVE_AURAS;x<MAX_AURAS;x++)
				if(m_target->m_auras[x])
				{
					...
					else if(mod->m_miscValue == 11) // if got immunity for slow, remove some that are not in the mechanics
					{
						...
						for(int i=0;i<3;i++)
							if(m_target->m_auras[x]->GetSpellProto()->m_effectAura[i] == SPELL_AURA_MOD_DECREASE_SPEED)
							{
			...
			if(m_spellProto->m_id==42292)
			{
				...
				for(uint32 x= MAX_POSITIVE_AURAS; x < MAX_AURAS; ++x)
				{
					if(m_target->m_auras[x])
					{
						...
						{
							switch(m_target->m_auras[x]->GetSpellProto()->m_effectAura[y])
							{
							case SPELL_AURA_MOD_STUN:
							case SPELL_AURA_MOD_CONFUSE:
							case SPELL_AURA_MOD_ROOT:
...
void Aura::SpellAuraMounted(bool apply)
{
	...
	{   
		...
		p_target->m_MountSpellId = m_spellProto->m_id;
		...
		if( p_target->GetShapeShift() && !(p_target->GetShapeShift() & FORM_BATTLESTANCE | FORM_DEFENSIVESTANCE | FORM_BERSERKERSTANCE ) && p_target->m_ShapeShifted != m_spellProto->m_id )
...
[wtf did you do shady? 8)] - Supalosa */
	...
	if(m_target->IsPlayer())
	{
		...
		if(GetSpellProto()->m_equippedItemClass==-1)//does not depend on weapon
		{
			for(uint32 x=0;x<7;x++)
			{
				if (mod->m_miscValue & (((uint32)1)<<x) )
...
void Aura::SpellAuraModRegen(bool apply)
{
	...
	{
		SetPositive ();
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHeal1,(uint32)((this->GetSpellProto()->m_effectBasePoints[mod->i]+1)/5)*3,
		...
		if(m_target->GetTypeId() == TYPEID_UNIT && ((Creature*)m_target)->GetCreatureName())
		{
			if(((Creature*)m_target)->GetCreatureName()->Type != CRITTER)
			{
				if(m_target->isDead())
				{
					...
					uint32 itemid = GetSpellProto()->m_effectItemType[mod->i];
...
void Aura::SpellAuraPeriodicDamagePercent(bool apply)
{
	if(apply)
	{
		//uint32 gr = GetSpellProto()->m_spellClassMask;
		//if(gr)
		//{
		//	Unit*c=GetUnitCaster();
		//	if(c)
		...
		/*if(m_spellProto->m_id == 28347) //Dimensional Siphon
		{
		...
		else*/
		{
			...
			sEventMgr.AddEvent(this, &Aura::EventPeriodicDamagePercent, dmg, 
				EVENT_AURA_PERIODIC_DAMAGE_PERCENT,GetSpellProto()->m_effectAmplitude[mod->i],0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		SetNegative();
	}
}
...
void Aura::EventPeriodicDamagePercent(uint32 amount)
{
	...
	if(c)
		c->SpellNonMeleeDamageLog(m_target, GetSpellProto()->m_id, damage, pSpellId==0, true);
	else
		m_target->SpellNonMeleeDamageLog(m_target, GetSpellProto()->m_id, damage, pSpellId==0, true);
}
...
{
	int32 val = apply ? mod->m_amount : -mod->m_amount;
	uint64 AffectedGroups = (uint64)GetSpellProto()->m_effectItemType[mod->i] + ((uint64)GetSpellProto()->EffectSpellGroupRelation_high[mod->i] << 32);
...
void Aura::SendDummyModifierLog( std::map< SpellEntry*, uint32 >* m, SpellEntry* spellInfo, uint32 i, bool apply, bool pct )
{
	...
	int32 v = spellInfo->m_effectBasePoints[i] + 1;
	uint32 mask = spellInfo->m_effectItemType[i];
	uint8 type = spellInfo->m_effectMiscValue[i];
...
void Aura::SpellAuraAddTargetTrigger(bool apply)
{
	...
	{
		ProcTriggerSpell pts;
		pts.parentId = GetSpellProto()->m_id;
		pts.caster = m_casterGuid;
		pts.procCharges = GetSpellProto()->procCharges;
		pts.i = mod->i;
		pts.LastTrigger = 0;
...
// ----?
		...
		case 5148:
			...
			if(apply)
			{
				...
				SpellOverrideMap::iterator itr = plr->mSpellOverrideMap.find((*itrSE)->m_id);
				...
				else
				{
					...
					{
						plr->mSpellOverrideMap.insert( SpellOverrideMap::value_type( (*itrSE)->m_id, lst) );
					}
				}
			}
			else
...
//		if(GetSpellProto()->m_id == 6150)// Quick Shots
//		{
//			Unit * pCaster = GetUnitCaster();
//			if(pCaster)
...
void Aura::EventPeriodicBurn(uint32 amount, uint32 misc)
{
	...
	if(m_target->isAlive() && m_caster->isAlive())
	{
		...
		SendPeriodicAuraLog(m_target, m_target, m_spellProto->m_id, m_spellProto->m_schoolMask, newHealth, 0, 0, FLAG_PERIODIC_DAMAGE);
		m_target->DealDamage(m_target, Amount, 0, 0, GetSpellProto()->m_id);
	}  
}
...
void Aura::SpellAuraPowerBurn(bool apply)
{
	...
	if(apply)
		sEventMgr.AddEvent(this, &Aura::EventPeriodicBurn, uint32(mod->m_amount), (uint32)mod->m_miscValue, EVENT_AURA_PERIODIC_BURN, GetSpellProto()->m_effectAmplitude[mod->i], 0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
}
...
void Aura::SpellAuraIncreaseSpellDamageByAttribute(bool apply)
{
	...
	int32 val;
	val = mod->m_amount;
	SM_FIValue(pCaster->SM_FEffectBonus,&val,m_spellProto->m_spellClassMask);
	...
	{ //bit hacky but it will work with all currently available spells
		if (m_spellProto->m_effectAura[i] == SPELL_AURA_INCREASE_SPELL_HEALING_PCT)
		{
			if (m_spellProto->m_effectMiscValue[i] < 5)
				stat = m_spellProto->m_effectMiscValue[i];
			else
				return;
		}
	}
...
//printf("!!! the AffectedGroups %u ,the smt type %u,\n",AffectedGroups,mod->m_miscValue);
	...
	case SMT_SPELL_VALUE:
		if(GetSpellProto()->m_spellIconID==457) AffectedGroups=1;
		if(GetSpellProto()->m_spellIconID==86) AffectedGroups=256;
...
void Aura::SpellAuraEnableFlight(bool apply)
{
	if(apply)
	{
		...
		if(m_target->IsPlayer())
		{
			static_cast< Player* >( m_target )->flying_aura = m_spellProto->m_id;
		}
	}
	else
	{
...
void Aura::SpellAuraIncreaseRating( bool apply )
{
	...
	if( mod->m_miscValue & 1 )//weapon skill
	{
		...
		for( uint32 y = 0; y < 20; y++ )
			if( m_spellProto->m_equippedItemSubclass & ( ( ( uint32 )1 ) << y ) )
			{
