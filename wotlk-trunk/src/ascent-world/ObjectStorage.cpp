...
void ObjectMgr::LoadProfessionDiscoveries()
{
	...
	if(result)
	{
		...
		{		
			...
			pf->SkillID = f[0].GetUInt32();
			pf->SpellID = f[1].GetUInt32();
			pf->pctChance = f[2].GetFloat();
	...
	if( result != NULL )
	{
		...
		/*		if (!sp->procCountDB) 
				...
				if(sp->agent == AGENT_SPELL)
				{
					...
					if(sp->spell->m_effect[0] == SPELL_EFFECT_LEARN_SPELL || sp->spell->m_effect[1] == SPELL_EFFECT_LEARN_SPELL ||
						sp->spell->m_effect[2] == SPELL_EFFECT_LEARN_SPELL)
					{
					...
					sp->minrange = GetMinRange(dbcSpellRange.LookupEntry(sp->spell->m_rangeIndex));
					sp->maxrange = GetMaxRange(dbcSpellRange.LookupEntry(sp->spell->m_rangeIndex));
					...
					if(sp->cooldown==0xffffffff)
					{
						...
						int cooldown;
						SpellDuration *sd=dbcSpellDuration.LookupEntry(sp->spell->m_durationIndex);
						int Dur=0;
						int Casttime=0;//most of the time 0
						int RecoveryTime=sp->spell->m_recoveryTime;
						if(sp->spell->m_durationIndex)
							Dur =::GetDuration(sd);
						Casttime=GetCastTime(dbcSpellCastTime.LookupEntry(sp->spell->m_castingTimeIndex));
