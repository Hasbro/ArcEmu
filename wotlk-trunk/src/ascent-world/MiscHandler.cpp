...
void WorldSession::HandleLootReleaseOpcode( WorldPacket & recv_data )
{
	...
	{	   
        ...
        switch( pGO->GetByteValue(GAMEOBJECT_BYTES_1, 1) )
        {
                        ...
                        if( pLock->locktype[i] )
                        {
							...
							else if( pLock->locktype[i] == 2 ) //locktype;
                            {
                                ...
                                if( pLock->lockmisc[i] == LOCKTYPE_MINING || pLock->lockmisc[i] == LOCKTYPE_HERBALISM )
                                {
                                    ...
                                    if( pGO->HasLoot() )
                                    {
                                        pGO->SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value( GAMEOBJECT_STATE, 1 );
...
void WorldSession::HandleGameObjectUse(WorldPacket & recv_data)
{
	...
	uint32 type = obj->GetByteValue(GAMEOBJECT_BYTES_1, 1);
	switch (type)
	{
	...
	case GAMEOBJECT_TYPE_DOOR:
		{
			// door
			if((obj->GetByteValue(GAMEOBJECT_BYTES_1, 0) == 1) && (obj->GetUInt32Value(GAMEOBJECT_FLAGS) == 33))
				obj->EventCloseDoor();
			else
			{
				obj->SetUInt32Value(GAMEOBJECT_FLAGS, 33);
				obj->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
