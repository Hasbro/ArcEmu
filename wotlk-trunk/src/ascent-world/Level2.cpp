...
bool ChatHandler::HandleGOSpawn(const char *args, WorldSession *m_session)
{
	...
	go->SetFloatValue(GAMEOBJECT_PARENTROTATION+2, sinf(o / 2));
	go->SetFloatValue(GAMEOBJECT_PARENTROTATION+3, cosf(o / 2));
	...
	gs->o = go->GetFloatValue(GAMEOBJECT_PARENTROTATION);
	gs->o1 = go->GetFloatValue(GAMEOBJECT_PARENTROTATION+1);
	gs->o2 = go->GetFloatValue(GAMEOBJECT_PARENTROTATION+2);
	gs->o3 = go->GetFloatValue(GAMEOBJECT_PARENTROTATION+3);
	gs->scale = go->GetFloatValue(OBJECT_FIELD_SCALE_X);
	gs->x = go->GetPositionX();
	gs->y = go->GetPositionY();
	gs->z = go->GetPositionZ();
	gs->state = go->GetByteValue(GAMEOBJECT_BYTES_1, 0); //GetUInt32Value(GAMEOBJECT_STATE);
	//gs->stateNpcLink = 0;
...
bool ChatHandler::HandleGOInfo(const char *args, WorldSession *m_session)
{
	...
	sstext
		...
		<< MSG_COLOR_GREEN << "State: " << MSG_COLOR_LIGHTBLUE << GObj->GetByteValue(GAMEOBJECT_BYTES_1, 0) << "\n" //GetUInt32Value(GAMEOBJECT_STATE)<< "\n"
		...
		<< MSG_COLOR_GREEN << "Type: "  << MSG_COLOR_LIGHTBLUE << GObj->GetByteValue(GAMEOBJECT_BYTES_1, 1)  << " -- ";

	switch( GObj->GetByteValue(GAMEOBJECT_BYTES_1, 1) )
	{
...
bool ChatHandler::HandleGOActivate(const char* args, WorldSession *m_session)
{
	...
	if(GObj->GetByteValue(GAMEOBJECT_BYTES_1, 0) == 1)
	{
		// Close/Deactivate
		GObj->SetByteValue(GAMEOBJECT_BYTES_1, 0, 0); //SetUInt32Value(GAMEOBJECT_STATE, 0);
		GObj->SetUInt32Value(GAMEOBJECT_FLAGS, (GObj->GetUInt32Value(GAMEOBJECT_FLAGS)-1));
	} else {
		// Open/Activate
		GObj->SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE, 1);
		GObj->SetUInt32Value(GAMEOBJECT_FLAGS, (GObj->GetUInt32Value(GAMEOBJECT_FLAGS)+1));
	}
...
/*	sp->procCount = atoi(procCount);*/
	...
	sp->minrange = GetMinRange(dbcSpellRange.LookupEntry(dbcSpell.LookupEntry(atoi(spellId))->m_rangeIndex));
	sp->maxrange = GetMaxRange(dbcSpellRange.LookupEntry(dbcSpell.LookupEntry(atoi(spellId))->m_rangeIndex));
...
bool ChatHandler::HandleGOAnimProgress(const char * args, WorldSession * m_session)
{
	...
	uint32 ap = atol(args);
	m_session->GetPlayer()->m_GM_SelectedGO->SetByteValue(GAMEOBJECT_BYTES_1, 3, ap); //SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, ap);
