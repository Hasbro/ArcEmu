...
void EyeOfTheStorm::OnCreate()
{
	...
	for( i = 0; i < 2; ++i )
	{
		...
		if( !m_bubbles[i]->CreateFromProto( 184719, m_mapMgr->GetMapId(), EOTSBubbleLocations[i][0], EOTSBubbleLocations[i][1], EOTSBubbleLocations[i][2], EOTSBubbleLocations[i][3] ) )
		{
			...
			m_bubbles[i]->SetByteValue(GAMEOBJECT_BYTES_1, 0, 1); //SetUInt32Value(GAMEOBJECT_STATE,1);
	...
	m_standFlag->SetFloatValue( GAMEOBJECT_PARENTROTATION+2, 0.662620f );
	m_standFlag->SetFloatValue( GAMEOBJECT_PARENTROTATION+3, -0.748956f );
	...
	m_dropFlag->SetFloatValue( GAMEOBJECT_PARENTROTATION+2, 0.885448f );
	m_dropFlag->SetFloatValue( GAMEOBJECT_PARENTROTATION+3, -0.464739f );
