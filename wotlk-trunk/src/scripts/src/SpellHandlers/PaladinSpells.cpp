...
bool SealOfRighteousness(uint32 i, Aura* pAura, bool apply)
{
	...
	switch(pAura->m_spellProto->m_id)
	{
	...
	{
		int32 value = 0;
		float randomPointsPerLevel = m_spellInfo->m_effectDicePerLevel[2];
		int32 basePoints = m_spellInfo->m_effectBasePoints[2] + 1;
		int32 randomPoints = m_spellInfo->m_effectDieSides[2];
		if(u_caster)
		...
		((Player*)u_caster)->AddOnStrikeSpellDamage(pAura->m_spellProto->m_id, min_dmg, max_dmg);

		// set the seal business
		if(u_caster->GetTypeId() == TYPEID_PLAYER)
		{
				((Player*)u_caster)->judgespell = applyId;
				((Player*)u_caster)->Seal = pAura->m_spellProto->m_id;
		}
		u_caster->SetFlag(UNIT_FIELD_AURASTATE, 16);
	}
	else
	{
			((Player*)u_caster)->RemoveOnStrikeSpellDamage(pAura->m_spellProto->m_id);
			// set the seal business
			if(u_caster->GetTypeId() == TYPEID_PLAYER)
			{
...
bool HolyShock(uint32 i, Spell *pSpell)
{
	...
	{
		switch(pSpell->m_spellInfo->m_id)
		{
		case 20473:
			newspell = 25912;
			break;
