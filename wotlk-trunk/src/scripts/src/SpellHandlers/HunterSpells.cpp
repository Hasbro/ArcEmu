...
bool Readiness(uint32 i, Spell * pSpell)
{
    if(!pSpell->p_caster)
		return true;
    pSpell->p_caster->ClearCooldownsOnLine(50 , pSpell->m_spellInfo->m_id);//Beast Mastery
    pSpell->p_caster->ClearCooldownsOnLine(163, pSpell->m_spellInfo->m_id);//Marksmanship
    pSpell->p_caster->ClearCooldownsOnLine(51 , pSpell->m_spellInfo->m_id);//Survival
    return true;
}
