...
#ifdef WIN32
	_player->BroadcastMessage("Powered by: %sArcEmu %s r%u/%s-Win-%s %s(STABLE-Milestone: 0.1 Fire in the skys!)", MSG_COLOR_WHITE, BUILD_TAG,
		BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);		
#else
	_player->BroadcastMessage("Powered by: %sArcEmu %s r%u/%s-%s %s(STABLE-Milestone: 0.1 Fire in the skys!)", MSG_COLOR_WHITE, BUILD_TAG,
